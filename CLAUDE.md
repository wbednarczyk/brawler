...
Prefix every shell/file command with `rtk` (`rtk git`, `rtk grep`, `rtk read`, `rtk ls`, `rtk cargo`, `rtk npm`, `rtk rad`, `rtk make`, `rtk npx playwright`) — it compresses output before it reaches context. `rtk proxy <cmd>` runs raw; avoid for normal work. Run `rtk trust` once per checkout so `.rtk/` filters apply. Use **repoctx** for structural code questions (definition, callers, outline, impact, changed) instead of grep/find/whole-file reads — full reference in the `repoctx` skill; fall back to `rtk grep` for prose/string-literal scans. Read targeted ranges; never re-read a file just edited. Reserve the strongest model for hard reasoning; prefer cheaper models for routine edits/lookups. Batch independent tool calls in one turn. **Delegation (owner standing rule):** the strong model orchestrates and quality-gates; implementation slices go to cheaper-model subagents from written contracts (tests-that-redden enumerated, red evidence pasted), **up to 3 subagents concurrently** for independent slices — but never stacking heavy builds (rules: [testing.md § Resource discipline](docs/testing.md)). Every subagent uses rtk + repoctx too, and never runs `git checkout`/`git restore` on tracked files — surgical edits only (a written rule; Codex peers sit outside the Claude-only `git-boundaries` hook and must follow it as written).
...
- **Any product, UI, analysis, or planning work: the `investing-domain` skill first** (`.claude/skills/investing-domain/SKILL.md` — how the investor thinks; surfaced by the session hook so every agent, including Codex peers, judges features from the user's side, owner 2026-09-07).
- **Any frontend UI work: [docs/ui-authoring.md](docs/ui-authoring.md) first** — it is the one home of primitive-first ([ADR 0037](docs/adr/0037-ui-component-framework-and-authoring-contract.md)), the pre-write self-check, mockup-first, and the i18n/gallery rules; apply them, don't restate them.
...
- Prefer small, reviewable changes. Commit + push to the feature branch at slice-green checkpoints without asking (owner 2026-09-08); pushing/merging to master, force pushes, `--no-verify`, and discarding tracked changes are hook-denied — [engineering-workflow.md § Local Developer Commands](docs/engineering-workflow.md#local-developer-commands).
...
- No in-app AI ([ADR 0084](docs/adr/0084-retire-in-app-ai-layer.md), [ADR 0111](docs/adr/0111-retire-video-transcription.md)): intelligence arrives through the MCP port; no AI dependency or provider credential remains. Future in-app inference re-enters only via an eval-gated ADR that beats the deterministic baseline on real data (the ADR 0080 bar).
...
- **Every behavior has a test that reddens when it breaks** — the full rule (what counts, "hard to test" is no excuse, lean suites) lives in [testing.md § Strategy](docs/testing.md).
...
- **Validate against the maintainer's real database** for every new feature; for matching/ranking/dedup features real-data validation **precedes** the approach — rule and recipe: [testing.md § Real-data validation](docs/testing.md).
- Data transforms, hot paths and new IPC commands carry the ADR 0049 obligations (invariants + goldens, scale gate, fidelity-corpus step — headless/MCP-only commands exempt): [testing.md § Data-transform correctness](docs/testing.md).
