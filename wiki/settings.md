...
- **Data storage** — `Parallel work` / `Wait when busy` / `Wait to start`
  ...
  autopilot tasks run at once (applies after restart); **Backups** lists the
  local copies of your data (a copy is taken before every upgrade), lets you
  `Create backup` now and `Restore` one — the restore applies when the app
  restarts.
