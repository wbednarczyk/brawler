...
| Qualitative verdicts | `set_qualitative_verdicts` | every result carries `"citationsJson":"[{\"evidenceType\":\"notebook_entry\",\"evidenceId\":\"<noteId>\"}]"` — types: `feed_item` \| `notebook_entry` \| `claim` \| `company_event` \| `research_question` \| `company_signal` \| `decision_entry`; every ref must resolve to an EXISTING row (#343) — one dead/fabricated ref refuses the whole batch |
