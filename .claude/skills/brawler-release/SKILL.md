...
9. **Post-merge mechanics only**: task issues auto-close via the PR's `Closes` tags; `gh issue close <epic> --reason completed` with a short closure comment; the project automation moves the board (state lives ONLY in the board `Status` field, never `state:*` labels). `make sync-rad` mirrors asynchronously — the owner's convenience, not a closure step. **An epic never closes with open sub-issues** (owner rule 2026-08-01, harvested from epic #40 closing at 6/11): before closing — and after any `Closes`-keyword auto-close — verify `sub_issues_summary` on the epic reads complete; every remaining child is finished, re-parented, or explicitly dropped by the owner first. A closed epic with open children is reopened on sight.
