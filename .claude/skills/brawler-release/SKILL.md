...
description: Complete a Brawler epic under the continuous-release model — a PRE-MERGE gate (retro, ADR audit, human docs, live-drive §G, check-epic ratchets) executed as commits on the epic branch, because merging the epic PR IS the release. Use when wrapping up or closing an epic; after the merge only issue/board mechanics remain.
---
...
6. **`make check-epic` green under Nix, on the branch** — the closure gate: full mandatory gate + the ratchets CI cannot run (coverage; real-data honesty on the owner's DB copy — refresh the copy first per `private/realdata/README.md`). Evidence = the gate's own `EXIT=` line, never a wrapper's exit code. (+ `make bench` if a hot kernel changed.) Triage every failure: fix on the branch or file a tracked issue.
