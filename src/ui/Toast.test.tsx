...
// into internals. Transient action feedback is the ONLY variant (ADR 0097) —
// the persistent attention variant is retired with system-event toasts.
...
describe("Toast", () => {
  ...
  it("never renders an assertive role=alert region (ADR 0097: no persistent variant)", () => {
    renderHarness();
    fireEvent.click(screen.getByRole("button", { name: "delete" }));
    fireEvent.click(screen.getByRole("button", { name: "one" }));
