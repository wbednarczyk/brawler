...
/// report's **disclosure date** ([`storage::ReportDocument::disclosure_key`]), never `created_at`:
...
/// actually-newer report; still tied (same disclosure key, same
/// structuredness), the smaller `id` wins, mirroring
/// [`crate::fundamentals::extraction::classify::canonical_reports_per_period`]'s
/// `Reverse(document_id)` determinism rule — so the winner never depends on
/// input order.
...
fn candidate_rank(document: &storage::ReportDocument) -> (String, bool, std::cmp::Reverse<&str>) {
    (
        document.disclosure_key(),
        is_structured_document(document),
        std::cmp::Reverse(document.id.as_str()),
    )
}
