...
/// ```
#[test] // no-assert-ok: prints a real-data verdict for human eyeballing, never asserted
...
fn extractability_follows_the_sniffed_container_not_the_pdf_name() {
    ...
    assert!(
        ...
        "a non-iXBRL markup render is not machine-extractable post-ADR 0095"
    );
    // Markup that IS inline-XBRL under a `.pdf` name: the ESEF tier reads
    // it as its own instance.
    assert!(document_is_extractable(
            &state,
            &seed(
                "instance.pdf",
                b"<?xml version=\"1.0\"?><html xmlns:ix=\"http://www.xbrl.org/2013/inlineXBRL\"><ix:nonFraction/></html>",
                "xml"
            )
        ));
...
/// pre-fix `enqueue_history_sweep` created the sweep row and enqueued its
/// job as two separate autocommit statements — poisoning the job insert
/// used to leave a `queued` sweep row behind with no job to ever drive it.
/// The fix wraps both writes in one IMMEDIATE transaction
/// (`create_history_sweep_with_job`), so the poisoned insert must roll the
/// whole thing back: no partial sweep row survives.
#[test]
fn enqueue_history_sweep_leaves_no_sweep_row_when_the_job_insert_fails() {
    let s = state();
    let c = company(&s);
    ...
    connection
        .execute_batch(
            "CREATE TRIGGER poison_history_sweep_job BEFORE INSERT ON job_queue
                 WHEN NEW.kind = 'history_sweep'
                 BEGIN SELECT RAISE(ABORT, 'history_sweep job poisoned for test'); END;",
        )
        .expect("install poison trigger");
    drop(connection); // avoid a pool deadlock across the call below (#360/#376)

    let result = enqueue_history_sweep(&s, &c, "manual");
    assert!(
        result.is_err(),
        "the poisoned job insert must surface as an error, not succeed silently"
    );

    let latest = s
        .history_sweeps()
        .get_latest_history_sweep(&c)
        .expect("read latest sweep (the same public read the coverage panel uses)");
    assert!(
        latest.is_none(),
        "no partial sweep row must remain after the job insert fails: {latest:?}"
    );
}
...
fn fetched_file_report(
    ...
    state
        .mark_report_document_fetched(
            ...
            // — so a placeholder hash here would make an already-fetched
...
fn non_ixbrl_xhtml_canonical_falls_back_to_pdf_sibling_post_retirement() {
    ...
    // render with no `ix:` tags — no tier reads it since ADR 0095.
    fetched_file_report(
        &s,
        &c,
    ...
    assert_eq!(
        ...
        "ADR 0095: the non-iXBRL XHTML canonical is no longer extractable, \
             so the sweep falls back to the PDF sibling"
    );
...
fn run_sweep(state: &AppState, company_id: &str, trigger: &str) -> crate::storage::HistorySweep {
    let sweep = state
        .history_sweeps()
