...
/// Test I (#508, decision 6): the concrete 4 → 5 bump this change makes — a
/// terminal couldn't-extract run whose `kpi_delta_json` is the EXACT
/// version-4 gap delta re-arms once under the running build (now 5, one
/// statement basis per document, true basis stored), pinning THIS migration
/// rather than only the relative "lower version" claim the sibling tests
/// already cover.
#[test]
fn a_version_4_couldnt_extract_run_re_arms_under_version_5_a_version_5_run_does_not() {
    assert_eq!(
        crate::jobs::structured_extraction::EXTRACTION_PIPELINE_VERSION,
        5,
        "this test pins the concrete #508 bump (4 -> 5); update the literals \
         here deliberately on the next bump"
    );

    let dir_v4 = unique_temp_dir("rearm-v4-to-v5-v4");
    ...
    assert!(
        ...
        "a run stamped pipelineVersion 4 must re-arm once under the running version 5"
    );

    let dir_v5 = unique_temp_dir("rearm-v4-to-v5-v5");
    std::fs::create_dir_all(&dir_v5).expect("temp dir");
    let state_v5 = AppState::with_data_dir(open_in_memory_database().expect("db"), dir_v5.clone());
    let stamped_v5 = serde_json::json!({
        "extractionAvailable": false,
        "reason": crate::jobs::structured_extraction::reason::NO_DETERMINISTIC_TIER,
        "pipelineVersion": 5,
    });
    let run_v5 = seed_terminal_unavailable_run(&state_v5, &dir_v5, "run_v5", stamped_v5);
    assert!(
        !terminal_run_should_rearm(&state_v5, &run_v5),
        "a run already stamped pipelineVersion 5 must settle, never re-arm"
    );
}
