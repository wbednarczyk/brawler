...
/// note-reference (a small bare integer preceding grouped/decimal value
/// columns). Polish statements print the current period in the first value
/// column.
fn current_period_value(tokens: &[NumToken]) -> Option<Decimal> {
    let has_value_shaped = tokens.iter().any(|t| t.grouped || t.decimal);
    let mut iter = tokens.iter().peekable();
    if has_value_shaped {
        // Drop leading bare small integers (note refs) until the first
        // value-shaped token.
