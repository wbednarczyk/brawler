...
//! implementations simply return ready results. [`Classifier`] is the only
//! capability with a live consumer (ESPI signal classification,
//! `storage/signals.rs`); the similarity, matcher, and semantic-search
//! contracts were removed with their implementations and re-enter through a
//! fresh eval-gated ADR if a consumer ever arrives (ADR 0080).
