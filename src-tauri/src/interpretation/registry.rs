...
//!
//! Each capability resolves to an active *strategy*. The legacy persisted
//! `embedding` value is tolerated at the settings read (mapped to `static`,
//! see `storage::settings`). Consumers build a capability through these
//! factories instead of constructing a concrete implementation, so a future
//! strategy requires no consumer change.
