...
#[test]
fn proposal_confirm_applies_and_reject_leaves_null() {
    let state = AppState::new(open_in_memory_database().expect("db"));
    let company = tracked_company(&state);
    let store = state.ownership();
    store
        .append_snapshot(stake(&company.id, "ULTRO S.a.r.l.", "2025-06-30"))
        .expect("append");
    let normalized = "ULTRO S.A.R.L.";

    // Upsert is idempotent per (company, holder): a re-run refreshes in place.
    assert!(store
        .upsert_holder_type_proposal(NewHolderTypeProposal {
            company_id: company.id.clone(),
            holder_name_normalized: normalized.to_owned(),
    ...
    assert!(store
        .upsert_holder_type_proposal(NewHolderTypeProposal {
            company_id: company.id.clone(),
            holder_name_normalized: normalized.to_owned(),
            proposed_type: "parent_company".to_owned(),
            confidence: Some(0.9),
            rationale: Some("refined".to_owned()),
            provider_id: Some("test".to_owned()),
            model: Some("m".to_owned()),
        })
        .expect("upsert refresh"));
    let pending = store
        .list_holder_type_proposals(&company.id, true)
        .expect("pending");
    assert_eq!(pending.len(), 1, "one proposal per holder");
    assert_eq!(
        pending[0].proposed_type, "parent_company",
        "refreshed in place"
    );

    // Pending does not stamp.
    assert!(store
        .current_state(&company.id)
        .expect("state")
        .iter()
        .all(|s| s.holder_type.is_none()));

    // Confirm applies the type; a confirmed proposal is not disturbed by re-upsert.
    store
        .confirm_holder_type_proposal(&pending[0].id)
        .expect("confirm");
    assert!(!store
        .upsert_holder_type_proposal(NewHolderTypeProposal {
            company_id: company.id.clone(),
            holder_name_normalized: normalized.to_owned(),
            proposed_type: "tfi".to_owned(),
            confidence: Some(0.1),
            rationale: None,
            provider_id: None,
            model: None,
        })
        .expect("upsert after confirm"));
    assert_eq!(
        store.current_state(&company.id).expect("state")[0]
            .holder_type
            .as_deref(),
        Some("parent_company"),
        "confirmed type applied and preserved"
    );
}
