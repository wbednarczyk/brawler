...
mod stored_fact_set;
pub(super) use stored_fact_set::{stored_fact_set, stored_fact_set_for_cross_check};
...
/// grouped in memory.
///
/// Equivalence contract (the refactor's oracle), when `basis` is `None`:
/// `result[k]` is byte-identical to what `metric_history(company_id, k,
/// exclude_fiscal_year, exclude_period_type)` would return — same values,
/// same order (the company fact scan preserves the `created_at DESC, id`
/// order each single read sees, and filtering to one definition preserves
/// that relative order). Every requested key is present; a key with no
/// history maps to an empty vector, exactly as the single read returns `[]`.
/// Final-preferred, one value per period — the same ADR 0093 dec. 2
/// slot-once collapse [`metric_history`] applies, kept byte-identical
/// between the two paths by the equivalence contract above. `Some(basis)`
/// restricts to that basis only (#508); `metric_history` stays basis-blind.
pub(super) fn metric_histories(
    ...
    for fact in facts {
        ...
        // #508: like-for-like only, no fallback to the other basis.
