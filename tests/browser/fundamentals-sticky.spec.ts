...
for (const [tierName, size] of Object.entries(TIERS)) {
  test(`facts matrix: sticky scroller is bounded, cells opaque, header outranks body @ ${tierName}`, async ({
    ...
    // The #5 fix: a bounded, genuinely-scrollable 2-axis container.
    const scrollerStyle = await page.locator(".facts-matrix-scroll").evaluate((el) => {
      const style = getComputedStyle(el);
      return { overflowY: style.overflowY, maxHeight: style.maxHeight };
    });
    expect(scrollerStyle.overflowY, "scroller is a vertical scroll container").toBe("auto");
    expect(scrollerStyle.maxHeight, "scroller has a bounded height").not.toBe("none");
  ...
  test(`Pozycje × okresy: sticky scroller is bounded, cells opaque, header outranks body @ ${tierName}`, async ({
    ...
    const scrollerStyle = await page.locator(".fundamentals-periods-scroll").evaluate((el) => {
      const style = getComputedStyle(el);
      return { overflowY: style.overflowY, maxHeight: style.maxHeight };
    });
    expect(scrollerStyle.overflowY, "scroller is a vertical scroll container").toBe("auto");
