...
// it lives in the Today stream plus the sidebar Today badge. Supersedes the
// persistent-overflow-cap spec (the capped toast wall this guarded against no
// longer exists as a surface at all). The `attention-overflow` overlay seeds
// ~20 unseen urgent events — the exact batch shape that used to raise the
// bottom-left toast wall (owner screenshot, issue #330).
test.describe("Ambient attention — no toasts, Today badge (ADR 0097)", { tag: "@clickable" }, () => {
  test("a ~20-event unseen batch raises ZERO toasts; the stream renders and the sidebar stays clickable", async ({ page }) => {
    ...
    // The events land as stream rows — this is genuinely "many unseen events",
    // not an empty screen…
