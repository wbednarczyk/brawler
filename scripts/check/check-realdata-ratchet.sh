#!/usr/bin/env bash
# Self-test for the real-data honesty ratchet (epic #40 S4, ADR 0091 dec. 4-5).
...
cat >"$baseline" <<'JSON'
{ "specificity_pct": 60.9, "orphaned_evidence": 27, "filename_as_statement": 0 }
JSON
...
cat >"$metrics" <<'JSON'
{ "specificity_pct": 75.0, "orphaned_evidence": 27, "filename_as_statement": 0 }
JSON
expect_exit 2 "uncommitted specificity improvement"

cat >"$metrics" <<'JSON'
{ "specificity_pct": 60.9, "orphaned_evidence": 12, "filename_as_statement": 0 }
JSON
...
cat >"$metrics" <<'JSON'
{ "specificity_pct": 60.9, "orphaned_evidence": 27 }
JSON
