...
- **`UNIQUE(run_id)`** — one receipt per run. Idempotent replay of a committed manifest returns this stored row rather than re-executing the commit write primitives (dec. 5: the primitives answer differently on replay — `Created` → `Reobserved` — only the receipt is a stable answer; replay against a TERMINAL run is #363's job — a normal re-call today dies on `InvalidRunTransition` inside `begin_committing` before this insert is ever reached, so `CommitReceiptAlreadyRecorded` is unreachable from a repeat production call until #363 adds a replay path that re-enters `committing`). A second insert for the same run maps the `UNIQUE` violation to a typed `CommitReceiptAlreadyRecorded` (Conflict).
