...
- The read model (`storage/activity_reads.rs`) binds status/window in SQL, collapses by `activity_key` (`MAX(id)`) before the 40-task cap, and takes the final status of a run/sweep/batch from its domain row.
