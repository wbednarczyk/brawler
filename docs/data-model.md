...
`kpi_ingest_commit_receipts` (migration 0138, ADR 0098 dec. 5, card #359): the immutable per-run commit outcome, append-only — zero `UPDATE` path in the store (the `valuation_runs` idiom).

- **`UNIQUE(run_id)`** — one receipt per run. Idempotent replay of a committed manifest returns this stored row rather than re-executing the commit write primitives (dec. 5: the primitives answer differently on replay — `Created` → `Reobserved` — only the receipt is a stable answer). A second insert for the same run maps the `UNIQUE` violation to a typed `CommitReceiptAlreadyRecorded` (Conflict).
- Written by `record_commit_receipt`, a **connection-level free fn** (`storage::kpi_extraction::record_structured_fact` pattern) that #363's commit transaction calls under its own externally-owned `&Connection`/transaction — never opens one itself, so it rolls back with the caller's transaction on any failure.
