...
- **Arming (`arm`)**: `pending` → noop (attempts/backoff preserved); `running` → typed error — the **single-armer invariant**: with the 1-worker lane, reconciliation running before the pools, and no external armers, a running row of the generation being armed is unreachable; the error is PROPAGATED (a handler's own row retries; reconciliation refuses to start the lane), never logged-and-ignored. **#353 must revisit this invariant when it lands the first concurrent armer (invalidate/stage tools)** — together with making the terminal hook's generation-check + `mark_failed` pair atomic (today nothing can invalidate/re-stage between them). Missing/terminal rows are `reschedule`d — arming is always a fresh intent, never a restart loop; `reschedule` follows the caller's kind/payload/budget (the id IS the identity, so a kind-drifted row heals on re-arm).
