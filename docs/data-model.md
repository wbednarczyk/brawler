...
- `ReportTaggedFactStore::coverage_counts(company_id)` — the Coverage panel's compact `TaggedFactCoverageCounts { rawStored, projected, comparative, dimensional, noteLevel, awaitingName, conflicting, unparsed, repeated }`. `dimensional` and `unparsed` are direct row counts; the projection buckets re-run `project_period` per `(report_document, period_end)`. `projected` covers ONLY each filing's own period — the derived-period cache's declared reporting date when one exists, else the latest tagged date — while earlier periods' candidates land in `comparative` (decision 7: captured, never written). `conflicting` counts contributing raw rows (fact-level), and `repeated` counts equal-value repeats beyond the first plus shorter duration windows superseded by the cumulative figure — so every raw number has a stated bucket. `projected` still means "the deterministic rule selected it", **not** "it is live in `financial_facts` right now" — the validation gate downstream may refuse a candidate; the UI label says "Selected for Fundamentals" for exactly this reason.
