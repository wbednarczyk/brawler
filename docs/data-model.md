...
- **Domain recency is the domain date, never `created_at`.** Selecting or ranking "newest / latest / most recent" of anything (report, feed item, event, signal, fact) must order by the **domain date** — publication / period-end / event / signal date — and never by `created_at`/`updated_at`/ingestion order. `created_at` is the local insert time; a history backfill or re-ingest gives an **old** record a **newer** `created_at`, so created-order silently diverges from chronological order. Any such selection ships a test against **real backfilled data where `created_at` order ≠ domain-date order** (real-data validation). Guardrail from `d60305c` (autopilot detection fired on a 3-year-old report by ranking on `created_at`); policy [ADR 0045](adr/0045-guardrail-harvest-loop.md). Enforced by the `recency` source-tree guard (`src-tauri/src/source_tree_guards/recency.rs`): every leading-`created_at` `ORDER BY` site **and every Rust comparator on `created_at`** (`sort_by`/`max_by`/`min_by`/`Reverse`/`.cmp`) is pinned with a reason; `Debt` entries carry their issue. The rule binds Rust-side ranking exactly as SQL (#496 closed `history_sweep`, the KPI-table cell and the report-diff representative, which the SQL scan could not see). The one fact preference for "which fact represents a slot" is `storage::financials::CANONICAL_FACT_PREFERENCE_ORDER` (SQL) mirrored by `canonical_fact_rank` (Rust); the period key is `IFNULL(period_end_date, fiscal_year || '-12-31')`.
...
- `scope`: `canonical` (app-owned, global) | `sector` (shared within a `sector`) | `company` (bespoke, set `company_id`) | `user` (user-owned, global custom metric — added in `v0.44.0` for quality frameworks, [ADR 0046](adr/0046-quality-frameworks-quantitative.md)). A `sector` row's `sector` value lives on **two axes** (#383): the curated statement packs use the `statement_type` vocabulary (`banking`, `insurance`, …), while a runtime-created row may carry a raw directory sector (`"Gry"`); the resolver and pinned-commit eligibility accept both (§ KPI Ingest Validation → Resolver), raw first.
...
- **Every metric key an extraction mapper can emit must be seeded**, or `record_structured_fact` drops the fact at its defensive `NoDefinition` skip and nothing surfaces. Gate: `every_emittable_metric_key_has_a_canonical_definition` (`storage/tests/migration_safety.rs`) scans the dictionary tuples and (since #309) the **whole `classify` body** of the cover-note mapper (rather than the `Some("literal")` return shape, which was blind to `Some(if … { "a" } else { "b" })` and let three keys ship unseeded), plus — for the ESEF path — iterates `fundamentals::extraction::ifrs_crosswalk::entries()`. A seed landing after `0129`/`0130` sets `origin`/`statement_group` **at insert**: those backfills are one-shot and never revisit a later row.
- **`ifrs_crosswalk` is the naming authority for the ESEF/iXBRL path** (ADR 0100 decision 2, epic #398, `fundamentals/extraction/ifrs_crosswalk.rs`, seed migration `0143_ifrs_crosswalk_kpi_definitions.sql`): a curated, sorted, deduplicated table mapping a taxonomy concept's local name to `metric_key` + `label` + `value_kind` + `statement_group` + `period_nature`, replacing hand-maintained match arms as the source `esef::concept_to_metric_key` moves onto. **Reuse before mint**: a concept maps onto an existing key whenever one already holds facts for the same figure (`EquityAttributableToOwnersOfParent` → `wdf_equity_parent`, `ProfitLossBeforeTax` → `wdf_pretax_profit`, `Inventories` → `inventories` never `inventory`, etc.) — never a fresh mint when one fits. It grows through a concept-harvest command (`ReportTaggedFactStore::harvest_uncrosswalked_concepts`, `storage::tests::real_data_extraction::concept_harvest_report`, `make realdata-concept-harvest`) that ranks Layer 1 (`report_tagged_facts`) concepts absent from the crosswalk by distinct-company count — the input for the next seed migration. Curated in-repo; nothing is minted at runtime.
- **Curated aliases** (ADR 0100 decision 12, epic #398, `fundamentals/kpi_aliases.rs`): a catalog key that exists but has never held a fact is redirected to the live key it duplicates — today only `inventory` → `inventories` (0 facts vs 771 across 44 companies; both canonical, seeded by `0048` and `0084`). `resolve_kpi_definition` applies the alias **only while the source key holds zero facts for the company being written** (one per-company `EXISTS` probe per aliased resolve — per company, never database-global, so one company's legacy series cannot flip routing for another's), so a writer reaching the dead key files into the live series with no caller change — while on a database where the source key already carries a series (import, older schema, manual entry) the redirect never fires; the alias is never applied in reverse and never chained. **One-sided by rule, enforced at runtime**: a redirect can never merge or split two real series — a pair where both sides carry facts is a *merge*, needing its own migration and its own evidence, never an alias entry. This is why an alias is not a repaint (next bullet): nothing is renamed, and a key that was always empty simply stops being a place a fact can land. Gates: `no_derived_formula_references_an_alias_source` (a derived formula reading a dead key would never compute for a real company — this is exactly how the seeded `quick_ratio` formula shipped broken from `0048` until migration `0147` repointed it, invisible to the computability gate, which seeds a fact for every definition including the dead ones) and `every_alias_names_two_seeded_catalog_keys`, both in `storage/tests/quality_frameworks.rs`.
...
- `period_nature` (ADR 0100 decision 6, epic #398, migration `0141_kpi_definition_period_nature.sql`): `instant` | `duration` (the DEFAULT). Whether a metric is measured at a point in time (a balance, a share/asset count, a quote) or accumulated over a span — a DIFFERENT axis than TTM (trailing-twelve-months) eligibility, which `is_ttm_eligible` (`fundamentals::metrics`) also weighs `value_kind` for: a ratio is duration-reported yet never TTM-eligible, so one predicate cannot serve both questions (the conflation `is_flow_key` used to make). Two pure functions read the axis: `measure_window_for(period_nature, profile_version)` for manifest `period.window_kind_mismatch` validation and ingest-context default-slot candidates (instant → `point_in_time`; duration → `cumulative` under a `gpw_interim@`-prefixed acquisition profile, ADR 0098 dec. 3, else `flow`), and `is_ttm_eligible(period_nature, value_kind)` for `_ttm`/`ttm()` window resolution. Either axis absent (no resolved catalog definition) keeps the pre-existing default: duration/eligible. Validated at the write boundary (`normalize_kpi_definition_period_nature`, the `normalize_currency` pattern); optional on every live writer, like `statement_group` — nothing forces a value. Backfill (`0141`) maps every row (**any scope**, unlike `origin`/`statement_group`) whose `metric_key` is in the retired `STOCK_METRIC_KEYS` const (`fundamentals::metrics`) to `instant` — no bare-id/scope guard, because the runtime predicate it replaces never respected scope either. `STOCK_METRIC_KEYS` itself stays as the migration's authoring source and the backfill gate's (`every_stock_metric_key_backfills_to_instant_period_nature`, `storage/tests/quality_frameworks.rs`) reference list — every real reader is retired. `financial_facts.measure_window`'s write-boundary default (ADR 0100 decision 6's 2nd paragraph) is covered below under `financial_facts`.
...
- `currency`: NULL (unit-less) or exactly a 3-letter ISO-4217 code, upper-cased and **enforced at every write** since #93 (`normalize_currency`, `storage/financials.rs`) — a non-currency unit is a typed `InvalidFinancialsValue` error, never a stored row. The ESEF projection writes NULL for every non-monetary crosswalk `value_kind` (share counts) and the numerator measure for `monetary` ones (ADR 0100 dec. 4, #509); a fact the guard still refuses is held back per fact, never a run abort (`rejectedFacts` below). Migration `0120_repair_eps_shares_currency.sql` repairs the 76 EPS facts (38 `eps_basic` + 38 `eps_diluted`) the ESEF divide-unit bug stored as `'shares'` (the `iso4217:PLN / xbrli:shares` denominator won); the parser now resolves a divide unit to its **numerator**.
- Dimensions: `currency`, `statement_basis` (`consolidated`/`standalone` — the ESEF structured writer stores the ONE basis it selected for the document, the instance's true label (ADR 0100 dec. 4 amendment 2026-09-14, #508: consolidated when the package has a consolidated primary statement, else standalone); every other writer keeps the plain `None`/`'consolidated'` default, which `slot_dims`'s `or_default` resolves; readers pick among bases through the one canonical preference (final › reported › consolidated › attribution, `storage/financials/fact_preference.rs`); the retired positional tier's document-evidence basis derivation `positional_statement_basis` is REMOVED with it — ADR 0095, 2026-08-05, and the migration that once re-stamped its proven rows was superseded by `0135`'s blanket `pdf`-tier fact deletion), `attribution` (`total`/`owners_of_parent`/`nci`), `variant` (`reported`/`adjusted`/`constant_currency`/`continuing`/`discontinued`/`net_of_cancellations`/`lifo_ccs`), `measure_window` (`flow`/`point_in_time`/`trailing`/`cumulative`/`duration` — ADR 0100 decision 6 2nd paragraph, epic #398: a `None` caller value derives from the resolved definition's `period_nature` (`instant` → `point_in_time`, else → `flow`, via `fundamentals::metrics::measure_window_for`), not a hard `'flow'` default; an EXPLICIT value whose flow/stock class contradicts `period_nature` — `point_in_time` on a `duration` metric, or any non-`point_in_time` token on an `instant` one — is a typed `MeasureWindowPeriodNatureMismatch` refusal, never a silent override (`cumulative`/`trailing` over a `duration` metric are legitimate and accepted). Both resolved in `slot_dims`/`resolve_measure_window`, `storage/financials.rs`, the shared boundary every writer funnels through. Migration `0144_repair_instant_measure_window.sql` moved every pre-existing `flow` fact on an `instant`-nature definition to `point_in_time` in the same change (ids preserved; a destination-slot collision keeps the already-correct `point_in_time` row: an equal-value flow twin is deleted as a re-observation after repointing its mutable references, while a DIVERGENT flow row is preserved in place under its original `measure_window` with a `diagnostic_events` row for manual resolution — a migration never deletes an observation two sources disagree about; immutable `kpi_ingest_commit_receipts.outcomes_json` is never rewritten, so a historical `factId` of a deleted equal-value twin may no longer resolve)), `data_quality` (`final`/`preliminary`/`estimated` — canonical since [ADR 0093](adr/0093-agent-acquisition-tier-and-preliminary-lifecycle.md): `final` = reported-final (default), `preliminary` = issuer-published pre-report figures (wstępne wyniki), `estimated` = third-party/derived; normalized and **enforced at every write** by `normalize_data_quality` (`storage/financials.rs`, the `normalize_currency` pattern — `estimate`→`estimated`, unknown token = typed error). Preliminary and final coexist in the slot; every reader prefers `final` and counts a slot once — `stored_fact_set_filtered` (the cross-check prior AND the completeness/recall FactSet loader — a preliminary-only metric still counts as covered, exactly once, no FactSet type change needed), `facts_coverage_by_period` (a `ROW_NUMBER() OVER (PARTITION BY … ORDER BY data_quality)` per-slot rank, `rn = 1`), and `metric_history`/`metric_histories` (one value per period, never double-weighted in the plausibility median) all converged on this ADR 0093 T4, epic #285), `reporting_standard` (override).
...
- **RETIRED (ADR 0095, 2026-08-05):** `pdf` (formerly the tier-3b positional sub-tier, `financial_facts.extraction_method='html_positional'`) is a legacy READ value only. Migration `0135` deleted every stored `pdf`-tier fact; new writes are refused (guards below). **RETIRED for writes (ADR 0098 dec. 7, #365):** `structured_xhtml` joined `pdf` as a legacy read-only tier — no live producer constructs it and new APPLICATION-level provenance writes are refused by the same mechanism (legacy rows remain readable/orderable, and a whole-database backup restore may intentionally reintroduce historical rows — the refusal is about new ingest stamping, not database-level impossibility).
- **Tier↔method coherence** (bug #324): `source_tier` and `extraction_method` are two axes of the same write and must always describe the SAME route (`SourceTier::matches_extraction_method`, `fundamentals/extraction/mod.rs`), enforced at RUNTIME in BOTH provenance write seams — the structured-path wrapper (`write_fact_provenance_fields`, `storage/kpi_extraction.rs`) and the raw shared upsert it delegates to (`fundamentals_provenance::set_fact_provenance`) — as a typed `IncoherentFactProvenance`/`RetiredSourceTier` error (the error names the refused tier), never a `debug_assert`. See [ADR 0095](adr/0095-retire-html-positional-tier.md) for the `pdf` tier's retirement.
...
- **Invalidation axis 2 = content provenance** (migration `0140`, #385): "a document is immutable once ingested" holds for the bytes, but **recapture** replaces the row's file/hash without touching this cache — so a cache hit additionally requires `cached.content_hash == document.content_hash`, both present and equal (`None == None` never counts). A legacy NULL-hash row or a recaptured document re-derives and overwrites on next read (same self-healing shape as the version axis). The MCP context read model's `derivedPeriod` hint applies the same predicate against the RUN's frozen `source_content_hash` — and never derives or writes (pure read): any mismatch is simply `null`.
- **Write guard**: only a **fetched** document with a stored file is cached — a not-yet-ingested document's transient `None` is never written, so it is not poisoned into never being re-derived once fetched. The stamped hash comes from the same document snapshot the derivation read, never a re-query of the mutable row. Writes are best-effort (a cache-write failure never fails the derivation); reads tolerate a missing row (a period predating this table is simply re-derived and cached).
...
- **Review read model** (`expectation_review`, `v0.52.0` J2): composed on read, never stored. `facts_available` is true once any facts exist for the resolved period; each metric's `actual_value` is the **canonical confirmed** `financial_facts` row for that metric+period (joined via `kpi_definitions.metric_key`, exact `period_type` match; the one preference order `storage::financials::CANONICAL_FACT_PREFERENCE_ORDER` — `final` › `reported` › `consolidated` › `total` › `owners_of_parent`, then id — never the latest-inserted; #496 fixed CBF/GPW/VRC H1-2026 `total_equity` reviews that compared against the owners-of-parent figure), else null → `unknown`. The read stamps `frozen_at` like `list` does.
- Owner-durable and retention-exempt; unified-bundle export is a `v0.67` dependency.
...
- `trigger` — `detection` (refresh-completion hook) | `manual` (user-initiated re-run, including a [Version-Aware Re-extraction](#version-aware-re-extraction) batch's re-arm) | `history_sweep` (enqueued by a history sweep, [History Sweeps](#history-sweeps) / [ADR 0077](adr/0077-trusted-extraction-foundations.md) §3; widened by migration `0062`). The `extract` stage is now **deterministic-only** for every trigger ([ADR 0084](adr/0084-retire-in-app-ai-layer.md) decision 4 — tier-4 OCR is retired): when no deterministic tier emits, the run records `{ extractionAvailable: false, reason: "no_deterministic_tier" }` (`reason: "witness_fallback"` appears on **legacy** deltas only — the ADR 0085 aggregator gap-fill is retired by [ADR 0086](adr/0086-aggregator-primary-fundamentals.md); stored fallback deltas stay honest gaps and re-armable) and is **flagged** with its completion notification — never a silent skip, never a guess. The AI-era budget gate no longer runs. **`pipelineVersion` is stamped on EVERY terminal delta, emitted or not** (epic #398 Item B blocker 2 fix, 2026-08-17 — before it, only the `extractionAvailable:false` gap delta stamped it, so a successful rerun that only re-observed an already-landed period left no version record at all). **"Emitted" (`extractionAvailable:true`) means a successful accepted ESEF observation** — `producedFactIds` OR `skippedFactIds` (re-observations) nonempty, NOT "at least one newly-created fact" (blocker 1 fix): a re-extraction of a landed period legitimately re-observes every fact, and that is still a genuine success, never miscoded as a gap.
...
- **A stage-enqueue failure is never swallowed** (#458): the first stage's own enqueue and every stage's hand-off to its successor used to log a warn-only message and leave the run dangling with no live job under it. Now a durable-queue write failure at either point finalizes the run `failed` with a typed `last_error` naming the stage (`"stage <x> could not be enqueued: …"`), visible on the run card.

### History Sweeps

The `history_sweeps` table (migration `0062`, [ADR 0077](adr/0077-trusted-extraction-foundations.md) §3) is the durable record of a history sweep — the backfill/manual counterpart to the refresh-time detection sweep. A sweep enqueues a full `autopilot_run` (`trigger='history_sweep'`) for every **canonical periodic report whose period still lacks accepted facts**, through the shared `enqueue_extraction_run` (dedup + re-arm identical to detection). Driven by `storage::HistorySweepStore` + the `history_sweep` durable-queue job (autopilot lane). **The sweep row and its durable job are created in one IMMEDIATE transaction** (`create_history_sweep_with_job`, #458) — a crash/error between the two used to be able to commit a `queued` sweep with no job ever able to drive it.
...
### Version-Aware Re-extraction

The `pipeline_reextraction_batches` table (migration `0146`, epic #398 Item B) is the durable record of a version-aware re-extraction batch — the explicit, user-triggered counterpart to the history sweep for the population the sweep deliberately never touches: runs that already **emitted** facts. `terminal_run_should_rearm` (jobs::autopilot) correctly never re-arms an emitted run (it stops the sweep from re-milling every finished report on every pass) — so a widened crosswalk/projection (this epic) never reaches an already-landed filing on its own. A batch selects the company's **successful ESEF-tier runs** (`extractionAvailable:true`, `tier:"esef"` on `kpi_delta_json`) whose stored `pipelineVersion` is below `EXTRACTION_PIPELINE_VERSION`, and re-arms each through the SAME primitives the sweep's own re-arm uses (`autopilot().rearm_run` + `autopilot::enqueue_first_stage`) — but a SEPARATE table and candidate selector, so the sweep's own skip rule is never touched. Driven by `storage::PipelineReextractionStore` + the `pipeline_reextraction` durable-queue job (autopilot lane, 2 attempts so a genuine process crash — job left `running`, reclaimed on the next startup — resumes instead of dead-lettering).

Fields (same shape as `history_sweeps`, minus the sweep-only `trigger`/`skipped_existing`/`skipped_reason`):

- `id` — primary key (`pipeline_reextraction:{company}:{nanos}`, collision-checked).
- `company_id` — → the company being re-extracted.
- `status` — `queued` (default) | `running` | `completed` | `failed`.
- `candidates_total` — successful ESEF-tier runs found with a stale stored pipeline version.
- `runs_enqueued` — candidates successfully re-armed.
- `runs_failed` — candidates a storage error prevented re-arming; a batch with `runs_failed > 0` still `completed`s (the count records the partial failure, never swallows it).
- `enqueued_run_ids_json` — the `autopilot_run` ids this batch re-armed (JSON array), so progress derives per-run status without a parallel query.
- `error` — a storage-level abort that failed the whole batch.
- `created_at`, `updated_at`.

Rules:

- **NOT gated on automation mode** — unlike the history sweep, a re-extraction reprocesses already-stored documents on explicit request (the per-period "Try again" posture, `rerun_extraction_outcome`), not new automation; it runs regardless of `company_autopilot_settings.mode`.
- **Re-armed runs are stamped `trigger='manual'`** — reuses the existing generic user-triggered bucket rather than widening `autopilot_run.trigger`'s CHECK for one more value with identical semantics.
- **Idempotent by construction**: a second batch re-lists the company's runs and finds none below the (now-current) stamped version — a genuine no-op, not a special case. Requires blocker 2's `pipelineVersion`-on-emitted-success fix (above): every run that emitted before that fix has NO stamped version at all (reads as `0`), so it is a candidate exactly once, after which its fresh delta stamps the current version.
- Access path: latest batch per company (`idx_pipeline_reextraction_batches_company_created`). Append-only, idempotent, self-healing migration (`CREATE TABLE IF NOT EXISTS`).
...
Fields (migration 0035): `id`, `company_id` → `companies(id)`, `period_id` → `financial_periods(id)` (nullable), `source_type` (`espi_attachment` | `user_url` | `article`), `origin_ref` (feed item / evidence id), `url` (original source URL), `local_path` (relative path under the app data dir; null when no file is stored), `content_type`, `content_hash` (sha256, optional dedup), `byte_size`, `title`, `attribution`, `fetch_status`, `fetch_error`, `fetched_at`, `created_at`, `updated_at`. `financial_facts.source_document_ref` is a soft reference to `report_documents.id`. **List order** (`list_by_company`, every reader of a company's documents): newest **disclosure key** first — `ReportDocument::disclosure_key()`: the `/emitent/YYYY-MM/` URL month (→ `YYYY-MM-01`; the article's month, correct even under a reused foreign filename), else `fetched_at`, else `created_at`; ties by id. Never insert order (#496: 36 of 50 owner companies showed a backfilled old report on top). Autopilot, the history sweep, the coverage read model and the report-diff representative all rank by the same key.
...
- **Mis-association guard + repair** (card 45fcece): tag-listing ingestion (bankier-company-komunikaty) stamps the tag's company onto every attachment. Before a row is created, an attachment that classifies as a **periodic report** (`doc_kind` `periodic_ssf`/`periodic_jsf` — the only kind that can win a period's canonical slot) is **rejected** when its filename names a *different* tracked issuer and no owner mention (name/alias/ticker over the article title, filename, and URL) is found; rejections are logged, no row. An idempotent startup repair (`repair_misassociated_report_documents`, wired in `lib.rs` setup, mirroring the `reclassify_report_documents` precedent) re-scans `espi_attachment` rows with the **same predicate** (no drift) and deletes the mis-associated ones (FK `CASCADE`/`SET NULL` clean their derived rows). Deliberately conservative — foreign detection uses the filename's full company-name/alias phrase only (never a bare ticker, never the URL hosting path), so counterparty/shareholder mentions and Bankier folder names never delete a legitimate filing (validated against the owner DB: the naive url/ticker rule would have wrongly deleted 36 rows; the scoped rule deletes 0).
  ...
  - **Incomplete attachment links** (`attachment_link_incomplete`, #460): a Bankier attachment `href` without a directory (a bare filename, `./x`, `../x`, `dir/x`) is the source's own broken link, not something to fetch from a guess — `bankier_links::resolve_attachment_href` joins it against the article page URL (RFC 3986) and the attachment registers `metadata_only` with `fetch_error = attachment_link_incomplete`, never fetched. An idempotent startup repair (`ReportDocumentStore::repair_incomplete_attachment_links`, wired in `lib.rs` setup beside the mis-association repair) downgrades pre-fix rows glued onto a bogus host (`https://www.bankier.pl_...`) the same way — statuses only, the URL is never rewritten and no row is ever deleted.
...
- `fetch_status` is `pending | fetched | failed | metadata_only` (the `metadata_only` value is additive over migration 0035 — no new migration). It is the single source of truth for whether bytes exist locally; a failed fetch records `fetch_error`, stays retryable, and never blocks feed-item ingestion. **`fetched` is never silently downgraded** (#455): `mark_failed` is an atomic guarded update (`UPDATE … WHERE NOT (fetch_status = 'fetched' AND local_path IS NOT NULL AND local_path <> '')`, mirroring `mark_metadata_only`'s guard) — a fetch attempt that races a concurrent successful capture declines the downgrade, and the caller re-reads and reports the existing fetched document rather than "no file". Before any fetch, capture/refetch entry points (`capture_report_document`, `fetch_report_document`) check the row's **verified identity** against disk — file present, size and `content_hash` match the stored row — rather than trusting "a file exists at this path": a `fetched` row that still matches needs no refetch; a `failed` row whose file matches is healed back to `fetched` (clearing `fetch_error`); a `fetched` row whose file is missing or no longer matches is reset to `pending` (`local_path` cleared) and refetched. The final file is published from a **unique per-attempt temp name** (`<local_path>.<pid>-<nanos>.part`, fsynced) via a **no-clobber publish** (`hard_link`, not `rename` — it fails rather than silently overwriting, so a slower concurrent attempt can never clobber a faster one's already-published bytes); a crash mid-write leaves only the `.part` file, never a truncated file at the path the row claims. **Every status transition that follows a prior inspection is compare-and-set on that inspected identity, not read-then-write** (#487): the heal write, the fetched→pending reset, and the fetched-row publish itself each guard their `UPDATE` on the exact `fetch_status`/`local_path`/`content_hash` the caller inspected (or, for a fresh publish, on the row not already being `fetched` with a *different* hash) — a concurrent capture that published between the inspection and the write always wins and is never erased, and the loser re-reads and reports the winning row instead. **Captures are serialized per document in-process** (`report_documents_capture::doc_lock`, #487 P1 r3): `capture_report_document`/`fetch_report_document`/`fetch_pending_attachments` hold one document's lock for the whole capture body, closing a window compare-and-set alone cannot — a second caller starting after the file is published but before the row write, which would otherwise see a `pending` row next to a fresh file and misread it as crash residue; the compare-and-set guards above remain as defence in depth.
...
- Retention reuses the feed-retention protection model ([ADR 0033](adr/0033-feed-retention-policy.md)): a document referenced by a confirmed `financial_fact`, linked as research evidence (`notebook_entry_origins` or a `management_claims` evidence link), backing a confirmed signal derivation, referenced by any durable `kpi_ingest_runs` row in ANY state — including failed/cancelled — ([ADR 0098](adr/0098-mcp-native-kpi-acquisition-lifecycle.md) dec. 3, migration 0138, epic #352 card #359), **or carrying a `report_tagged_fact_extractions` row, also in ANY state** ([ADR 0100](adr/0100-two-layer-tagged-fact-capture-and-ifrs-vocabulary.md) dec. 8, migration 0142, epic #398 — Layer 1 is derived data rebuilt from the document's bytes, so a pruned document could never be rebuilt) — is **protected** and never pruned, so a run's page/table citations (and a rebuildable tagged-fact generation) stay verifiable/possible even after the run/extraction itself terminates. Unprotected full files past the retention window have their **bytes** pruned (file deleted, row downgraded to `metadata_only`); the metadata row itself is never deleted. Implemented as `storage::report_documents::document_bytes_are_protected` — five semantic legs realized as six physical `EXISTS` clauses (the research-evidence leg splits into `notebook_entry_origins` and `management_claims`) — and enforced **atomically** in `mark_metadata_only`: a single `UPDATE … WHERE NOT (<protection>)`, never read-then-write; a protected document's downgrade attempt refuses with a typed `ReportDocumentBytesProtected` error rather than silently no-op'ing. On-disk report-document size and the retention window are surfaced in Settings → Data retention alongside the feed controls.
...
### Report Tagged Facts (Layer 1)

`report_tagged_facts` + `report_tagged_fact_roles` + `report_tagged_fact_extractions` (migration `0142_report_tagged_facts.sql`, [ADR 0100](adr/0100-two-layer-tagged-fact-capture-and-ifrs-vocabulary.md) decisions 1, 8, 9, epic #398) — the raw Layer 1 tagged-fact capture that sits under the curated catalog. Every tagged fact in an ESEF/iXBRL instance is written 1:1, unconditionally: no naming decision, no crosswalk lookup, no interpretation. A concept with no crosswalk entry is a **row**, not a diagnostic that evaporates. Populated by `fundamentals::extraction::esef::layer1::extract_tagged_facts` (a namespace-aware second pass over the instance bytes, independent of the unchanged Layer 2 `parse_esef`), wired from `jobs::structured_extraction::capture_layer1_tagged_facts` — runs alongside Layer 2 for every ESEF-bearing document, best-effort (a failure here never fails the Layer 2 run).

**Layer 2 projection** (`fundamentals::extraction::esef::projection::project_period`, decision 4) is now the ESEF tier's fact production, replacing the pre-epic `parse_esef` + `dedup_longest_duration` + `fact_set_for_period` chain for this tier only (other tiers keep those collapses unchanged). It runs over the SAME freshly-extracted `NewTaggedFact` generation `capture_layer1_tagged_facts` writes (`jobs::structured_extraction::compute_layer1_generation`, computed once per run — never a DB round-trip) and applies the required order (ADR 0100 decisions 3/4 as amended after the sol review): dimensionless-only → CONCEPT-level primary-statement role filter (a presentation linkbase relates concepts to roles, so occurrences of one concept are indistinguishable by role — no per-occurrence statement ranking exists) → namespace-gated crosswalk resolution (only a standard-IFRS-namespace concept may resolve to a global key; an issuer extension reusing a standard local name stays uncrosswalked) → full write-slot duplicate resolution grouped by (statement basis derived per package instance, metric key), with the longest duration window winning over shorter spans sharing its end date (cumulative GPW interim convention), never first-wins by document order: identical normalized value+currency is a deterministic **repeat** (one fact, linking every contributing `fact_identity`); a genuine disagreement is a typed conflict, recorded, never projected. `fundamentals::extraction::pipeline::run_pipeline`'s ESEF branch validates the projected candidate set through the same `validate`/`validate_tier` gate as before (decision 5) — projection only decides which candidates exist, never bypasses the gate — and separately (read-only, never written) projects the comparative period for the cross-check, so that check is order-independent too. Only the filing's own declared period is ever projected/written (decision 7); a comparative period stays in Layer 1.

**No-linkbase fallback (regression fix).** A bare iXBRL instance (not inside a ZIP package) carries no `*_pre.xml` to attach roles from — `compute_layer1_generation` computes `has_presentation_linkbase = false` for it (and for a package whose linkbase failed to parse), threaded through `PipelineInput`/`project_period`. The strict role filter above cannot evaluate with no evidence, so `project_period` skips it entirely for that document: every dimensionless, valued row proceeds straight to crosswalk resolution (the pre-epic selection for this tier), never silently projecting zero facts. A package that DOES carry a linkbase is unaffected. The fallback's use is never silent: `report_tagged_fact_extractions.no_linkbase_fallback_count` (below) records how many facts it affected.

Same doctrine as `document_derived_periods`/`report_document_sections` above: a **disposable, derived index**, never a source of truth — every row is computed from a `report_documents` row's stored bytes. Dropping the tables loses zero canonical data, it only forces re-extraction, which is exactly why the extraction record joins the report-bytes retention protection (§ Report Document Model retention, below): a raw-only package whose bytes were pruned could never be rebuilt.

`report_tagged_fact_extractions` — one row per extracted document:

- `report_document_id` (PK, FK → `report_documents`, `ON DELETE CASCADE`).
- `source_content_hash`, `extractor_version` — **freshness is the pair, not `extractor_version` alone** (decision 8): recapture replaces a document's bytes without changing its id, the same reason migration `0140` added `content_hash` to the derived-period cache. `ReportTaggedFactStore::is_extraction_current` mirrors `report_sections.rs`'s skip predicate.
- `state` — free-text outcome token; deliberately **no DB `CHECK`**, mirroring `report_document_extractions.extraction_state`'s own undocumented-at-the-DB-level shape. `capture_layer1_tagged_facts` writes `extracted` (≥1 instance parsed, walked to the end), `truncated` (≥1 instance parsed but a mid-document XML reader error aborted a walk, or a package entry was skipped unread — the counts cover only what was walked, and `encountered = stored` is not evidence of completeness; sol review finding 4), or `no_instance` (a route that carries an ESEF instance yielded none, e.g. a ZIP with only decoy xhtml); a route with no instance at all (PDF/positional/unsupported) writes no row.
- `encountered_count`/`stored_count`/`dimensional_count` — the epic's ship-gate counters (decision 9: "encountered = stored", never "successfully normalized"); `dimensional_count` is the `is_dimensional = 1` subset (stored but not yet projectable — `financial_facts` has no dimension axis).
- `no_linkbase_fallback_count` (migration `0145`) — the no-linkbase-fallback regression fix's visible, never-silent counter: `0` when the document carried presentation-linkbase evidence, else the count of dimensionless/valued facts that used the fallback selection (see above). `TAGGED_FACT_EXTRACTOR_VERSION` bumped 1 → 2 in the same change so every pre-existing generation rebuilds and reports it instead of a stale `0`.
- `extracted_at`.

`report_tagged_facts` — one row per tagged fact **occurrence**, written 1:1 from a filing:

- `id` (`rtf_{32 hex}` — the `generate_observation_id` id policy, collision-safe and non-deterministic; the real business-key invariant is the `UNIQUE` below, not this id), `report_document_id` → `report_documents(id) ON DELETE CASCADE`, `company_id` → `companies(id) ON DELETE CASCADE`.
- `package_entry_path` — the instance's path inside the package (a package legitimately carries several instances); `''` for a non-package document.
- `fact_identity` + `identity_kind` (`xml_id` | `occurrence`) — `xml_id:<the ix fact's id attribute>` where present (measured: 3439/3439 sampled facts carry a unique id), else a deterministic source locator. **Non-null on purpose**: a nullable column would let SQLite admit many NULLs under the unique index below.
- `concept_namespace_uri` + `concept_local_name` — the **expanded** namespace URI, never the QName prefix (a prefix is a document-local alias).
- `context_ref`, `period_type` (`instant` | `duration`), `period_start` (nullable), `period_end`, `unit_ref`, `unit_measure`.
- `value_raw` (exactly as printed) and the normalized `value_numeric` (decimal-exact TEXT, the `valuation_runs`/`kpi_staged_observations` convention), `scale`, `sign`, `decimals` — all nullable except `value_raw`.
- `is_dimensional` + `dimensions_json` — dimension members, when present; not (yet) projectable.
- `parse_status` (`ok` | `unparsed_value` | `unresolved_context` | `unsupported_unit` | `unsupported_continuation`) + `parse_error` — **decision 9: every supported occurrence gets a row even when normalization fails**, nullable `value_numeric`, typed status, never a silent drop. `unsupported_continuation` marks a numeric fact carrying `continuedAt`: its local text fragment can parse cleanly while being only PART of the disclosed number, so no value is ever stored for it (decision 9 as amended after the sol review).
- `created_at`/`updated_at`.
- `UNIQUE (report_document_id, package_entry_path, fact_identity)` — admits the **same** concept + context tagged 2–3× within one document (a real shape, decision 4: XTB `ProfitLoss` 3×) as long as `fact_identity` differs, and two instances inside one package (differing `package_entry_path`) never collide on the same `fact_identity`; only rejects a true duplicate identity.
- Indexed for "all rows for a document" (`report_document_id`) and concept lookups (`concept_namespace_uri`, `concept_local_name`).

`report_tagged_fact_roles` — the fact↔presentation-role **relation** (decision 3, as amended): a concept participates in several roles (income statement, comprehensive income, cash-flow reconciliation, …), so a scalar column cannot hold its role membership. The relation is CONCEPT-level evidence: the linkbase relates concepts to roles, so every occurrence of one concept carries the same role set — occurrences are NOT distinguishable by role, which is why projection resolves same-concept repeats by value equality. `fact_id` → `report_tagged_facts(id) ON DELETE CASCADE`, `role_uri`, `role_kind` (the classified family: `balance` | `income` | `comprehensive_income` | `cash_flow` | `equity_changes` | `other` — an unrecognised role classifies as `other` explicitly, never by guess). `PRIMARY KEY (fact_id, role_uri)` — a pure junction table needs no surrogate id (the `report_document_sections` composite-PK idiom). `esef_package::extract_presentation_roles` reads every `*_pre.xml` in the package and matches on concept **local name** only (the same carve-out decision 1 makes for the existing 22-concept mapping); `esef_package::classify_role` classifies each role URI's terminal segment through the three bounded rules of ADR 0100 decision 3 (as amended 2026-09-12, #511): an exact standard-IFRS role-number allowlist, exact normalized vendor names (Polish and English), and the `Rnn_<token>` abbreviation pattern — never a substring match, never inference. The stored `role_kind` is the classification at capture time; a classifier change bumps `TAGGED_FACT_EXTRACTOR_VERSION` so `is_extraction_current` rebuilds every stored generation on its next run (no migration).

`esef_package::extract_all_instances` enumerates every real iXBRL instance in a package (filtered by the same `is_inline_xbrl` sniff the container router uses, so a decoy `.xhtml` never counts as one) — the multi-instance fix decision 1 calls for; Layer 2's `extract_instance` (largest-instance-under-`reports/`) is unchanged.

`ReportTaggedFactStore::replace_tagged_facts(document_id, company_id, extraction)` mirrors `report_sections.rs`'s replace: delete the document's existing rows + extraction record, insert the new generation + record, all in **one transaction** (`Immediate`) — a failed rebuild never leaves a half-generation.

**Coverage read model + owner promotion** (ADR 0100 decision 10, epic #398 final slice) — no schema change; both reuse `report_tagged_facts` plus the existing `kpi_definitions`/`financial_facts` tables:

- `ReportTaggedFactStore::coverage_counts(company_id)` — the Coverage panel's compact `TaggedFactCoverageCounts { rawStored, projected, comparative, dimensional, noteLevel, awaitingName, conflicting, unparsed, repeated, otherBasis }`. `dimensional` and `unparsed` are direct row counts; the projection buckets re-run `project_period` per `(report_document, period_end)`. `projected` covers ONLY each filing's own period — the derived-period cache's declared reporting date when one exists, else the latest tagged date — while earlier periods' candidates land in `comparative` (decision 7: captured, never written). `conflicting` counts contributing raw rows (fact-level), and `repeated` counts equal-value repeats beyond the first plus shorter duration windows superseded by the cumulative figure — `otherBasis` counts the occurrences of the statement basis the document's projection did not select (the standalone instance of a package that also carries a consolidated one) plus instances whose entry path names both bases — kept as Layer 1 evidence, never written; so every raw number has a stated bucket. `projected` still means "the deterministic rule selected it", **not** "it is live in `financial_facts` right now" — the validation gate downstream may refuse a candidate; the UI label says "Selected for Fundamentals" for exactly this reason.
- `ReportTaggedFactStore::harvest_uncrosswalked_concepts_for_company(company_id)` — the per-company variant of `harvest_uncrosswalked_concepts` above: every uncrosswalked concept **this company** captured (dimensionless only — Layer 1 has no dimension-aware projection), each with the SAME global `company_count` ranking signal, this company's own `occurrence_count`, and a `statement_group`/`period_nature` derived from this company's own role/period_type rows.
- `commands::tagged_fact_promotion::promote_uncrosswalked_concept_core` — the "Show in Fundamentals" action:
  1. Ensures a `scope='company'` `kpi_definitions` row — idempotent (checked via `list_kpi_definitions` before `create_kpi_definition`). The `metric_key` is the concept's IDENTITY, not just its local name (sol rounds 3/4): a standard-taxonomy concept keeps its local name (unique within IFRS by construction, matched across taxonomy-year namespace versions), while an issuer extension mints the fixed-length `xconcept_<sha256(namespace, local) 64-hex>` — full strength, because a truncated digest is birthday-collidable and a collision would silently merge two concepts into one definition. `label`/`label_source`: an issuer extension concept (namespace not `xbrl.ifrs.org`) resolves through the package's own label linkbase (`esef_package::extract_label_linkbase`, read from that document's stored bytes at promotion time — never persisted, never re-parsed on the hot extraction path); a standard concept, or an extension with no Polish label in its linkbase, falls back to the raw technical concept name with `label_source = "technical"` — **never invented** (ADR 0100 decision 10).
  2. Groups the company's dimensionless, valued Layer 1 rows for that concept by `period_end`, resolving repeat-vs-conflict the same way decision 4 does (identical value+currency → write once; a genuine disagreement → skipped, never resolved by document order), then writes each surviving period through `record_structured_fact` (`source_tier = "esef"`, `extraction_method = "api"`, `validation_status = "unreviewed"` — no `validate_tier` gate runs over a single custom company metric; the same honesty precedent as the MCP single-fact write path).
  3. Never mints a canonical crosswalk entry (decision 2 stays intact) and never re-arms itself off the harvest output — a promoted concept still appears there (decision 10's third bullet), so a later seed migration can lift it into the shared vocabulary deliberately.
- MCP registry: the two reads are classified `read` (unexposed — an owner-facing trust view, not core investor content); `promote_uncrosswalked_concept` is `excluded` — permanently, per decision 10's "a machine still may not".
...
- Identity: `report_document_id` (FK → `report_documents`, `ON DELETE RESTRICT` — row-audit protection; the byte-retention downgrade to `metadata_only` stays a separate contract, #359), `company_id` (FK → `companies`, `ON DELETE CASCADE`), `profile_version` — validated at creation against the in-code extraction-profile registry ([ADR 0099](adr/0099-acquisition-mcp-surface-mechanics.md) dec. 6, § Extraction-profile registry below), format `{profile_id}@v{N}`; a value outside the registry is a typed `InvalidKpiIngestRunValue`. `period_id` (FK → `financial_periods`, `ON DELETE SET NULL`) may be absent until `commit_manifest` (#362) fills it inside the commit transaction (§ KPI Ingest Commit Receipts below) — a pre-existing pin is re-verified, never silently re-created if its row was deleted meanwhile.
- `source_content_hash` — an immutable snapshot SHA-256 of the content the run actually read, set once via `mark_source_captured` (atomically with the `discovered → source_captured` transition). Not the same signal as `report_documents.content_hash`, which is overwritten on re-fetch.
- `scope` (`standalone`/`consolidated`), `data_quality` (`final`/`preliminary`/`estimated`) — nullable in discovery; enforced non-NULL as a `begin_extracting` prerequisite (`RunTransitionPrerequisiteMissing`).
- `instruction_version` — nullable in discovery; written atomically by `begin_extracting` (the transition's own payload) as a `source_captured → extracting` prerequisite.
- `status` — the closed 11-state lifecycle (ADR 0098 dec. 6): `discovered → source_captured → extracting → staged → validation_failed | ready_to_commit → committing → complete | partial | failed | cancelled`. Enforced by a DB `CHECK`, the `KpiIngestRunState` enum, and `KpiIngestRunState::can_transition` (`storage/kpi_ingest_runs.rs`) — one exhaustive `match` over every state with no wildcard arm, so the compiler forces coverage of all 11 on any change.

**Transition table** (ADR 0098 dec. 6, card #360 — normative):

| From → To | Caller | Lease | Revision/manifest | Method |
|---|---|---|---|---|
| (create) → `discovered` | system | — | rev 0, hash `NULL` | `create_run_if_absent` |
| `discovered` → `source_captured` | agent (live lease) | required | — | `mark_source_captured(id, holder, hash)` — prerequisite: `source_content_hash` non-NULL, set-once |
| `source_captured` → `extracting` | agent (live lease) | required | — | `begin_extracting(id, holder, instruction_version)` — prerequisites: `instruction_version` (the call's own payload), `scope`+`data_quality` non-NULL, a period identity (`period_id` or a complete descriptor); over MCP the server stamps the acquisition-orchestration-contract version (`acquisition-mcp@v1`, #384) — the run records which frozen tool contract drove it, not an agent skill version (the skill records its own version in execution metadata, #388) |
...
| `ready_to_commit` → `committing` | `KpiIngestCommitStore::commit_manifest` (#362) | must be `NULL` (invariant; violation → `RunLeaseInvariantViolation`) | verifies hash+revision (stale → `StaleManifestForCommit`) | `begin_committing(conn, id, manifest_hash, revision)` — connection-level |
| `committing` → `complete`\|`partial` | `KpiIngestCommitStore::commit_manifest` (#362), same tx as the receipt | — | — | `finalize_committing(conn, id)` — connection-level; reads and verifies the receipt itself (no terminal parameter — a structural gate); missing/mismatched receipt → typed refusal |
| `committing` (crash) → `complete`\|`partial`\|`ready_to_commit` | startup reclaim | — | receipt present → finalize per `receipt.terminal_status`; no receipt → `ready_to_commit` (the commit tx rolled back, retryable) | `reclaim_ingest_runs_on_startup()` |
| `{discovered…ready_to_commit}` → `cancelled` | system/user | released | — | `cancel_run(id)` — never legal from `committing` or a terminal state |
| `{discovered…staged, validation_failed, ready_to_commit}` → `failed` | system (exhausted retries: the agent's for the pre-staging states, the validator's for `staged`, the commit job's for `ready_to_commit`) | released | — | `mark_failed(id, last_error)` — never legal from `committing` (only rollback/completion resolve it) |
| terminal → anything | — | — | — | always `InvalidRunTransition` |

Enforcement (never a pre-check outside the transaction — TOCTOU): every write is a guarded `UPDATE … WHERE id = ? AND status IN (…from){extra guard}` on an `Immediate` transaction; a 0-row result is classified by re-reading the row on the SAME transaction into either `InvalidRunTransition { id, from, to }` (wrong status) or the specific `RunTransitionPrerequisiteMissing { id, requirement }` (right status, a named prerequisite still missing). The three lease-failure shapes (wrong holder, expired, absent) all collapse to `RunLeaseNotHeld`.

- `manifest_hash`/`manifest_revision`, `attempt_count`, `expected_kpis_json` (the versioned completeness-denominator snapshot, stamped at run creation — legacy NULL rows: by first validation; § KPI Ingest Validation), `missing_reasons_json`, `progress_json`, `cost_json` (diagnostic only, never part of the trust verdict), `last_error`.
- `period_fiscal_year`/`period_type` (migration 0138, ADR 0098 dec. 3, card #359) — the run's durable **period descriptor**, a natural key mirroring `financial_periods UNIQUE(company_id, fiscal_year, period_type)`. A run legally has no `financial_periods` row until the commit transaction (`commit_manifest`, #362) creates one, so staging needs a stable period identity before that exists. **All-or-none** (`InvalidKpiIngestRunValue` on a partial pair); `period_type` vocabulary matches `financial_periods.period_type` exactly: `FY`, `H1`, `H2`, `Q1`–`Q4`, `9M`, `M01`–`M12`. When both `period_id` and the descriptor are present, `create_run_if_absent` verifies they name the same period (mismatch → typed error); an active run for the same `(document, company, profile)` triple later requested with a **different** period/descriptor is a typed `RunPeriodConflict` (Conflict), never a silent overwrite.

Lease semantics (model A, ADR 0098 dec. 8): `lease_holder`/`lease_expires_at`/`last_heartbeat_at` are all-NULL or all-set (DB `CHECK`) — never a half-filled/orphaned heartbeat. `claim_next` atomically claims the oldest claimable row whose lease is absent or expired (one `UPDATE … WHERE id = (SELECT …) RETURNING`, mirroring `jobs::claim_next`), incrementing `attempt_count` exactly once per successful claim or takeover. **`claim_run` (#384, ADR 0099 dec. 4)** is the targeted twin `start_kpi_ingest` uses: the same holder with a LIVE lease renews (expiry+heartbeat extended, NO `attempt_count` increment — the explicit keepalive); an absent/expired lease claims with the increment; a live FOREIGN lease refuses `RunTakenOver`; a non-claimable status refuses `RunNotClaimable`; the transaction reads `now` once and binds it everywhere. Lease refusals on the guarded steps (`attach_run_context`/`mark_source_captured`/`begin_extracting`) classify three ways: the caller's own expired lease → `RunLeaseExpired` (retry via `start_kpi_ingest(runId)`), a live foreign lease → `RunTakenOver` (abandon), anything else → `RunLeaseNotHeld`. **`attach_run_context` (#384)** fills the run's NULL context columns set-once under a live lease in the claimable states (same value idempotent; a differing value → `RunContextValueConflict`; an attached period descriptor is cross-checked against an existing `period_id` pin). Claimable states: `discovered`, `source_captured`, `extracting`, `validation_failed` — `staged`/`ready_to_commit` wait on deterministic validation/commit, not the agent worklist, and `committing` plus every terminal state are never claimable. `heartbeat` extends a live, owned lease only (an expired lease cannot be resurrected by its old holder — only a fresh `claim_next` may reclaim it). `reclaim_expired_leases` clears expired leases on claimable rows and refuses (rather than silently repairing) if any `committing` row holds a non-null lease — a structural invariant the `committing` transition upholds (`mark_ready_to_commit` clears the lease, `begin_committing` requires it `NULL`).

Startup self-heal (card #360): `reclaim_ingest_runs_on_startup` runs BEFORE the job-queue pools start (`lib.rs` setup ladder), one `Immediate` transaction on one connection handle. Order: (1) find `committing` rows holding a non-null lease — a structural invariant violation, reported and left completely untouched, never auto-repaired; (2) every remaining `committing` row resolves by receipt presence — a matching receipt (hash+revision) finalizes to `receipt.terminal_status`, a receipt that disagrees is left untouched (needs manual investigation), no receipt at all reverts to `ready_to_commit` with the manifest preserved (retryable); (3) ordinary expired-lease clearing on claimable rows. A second reclaim with nothing to do is a pure no-op (`ReclaimSummary::is_noop`).

#### Extraction-profile registry ([ADR 0099](adr/0099-acquisition-mcp-surface-mechanics.md) dec. 6, #383)

Const in-code registry (`storage/kpi_ingest_profiles.rs`): `profile_version` = `{profile_id}@v{N}`, membership-validated at `create_run_if_absent` (no `@vN` parser until a second version exists); `resolve_profile_version(profile_id)` maps the MCP contract's `profileId` to the current version. Each profile carries a per-`statement_type` expected-KPI pack that joins the creation-time stamp's union; every pack key is test-pinned to a seeded `kpi_definitions` row and test-proven resolvable for its statement type.
...
A document referenced by any durable run (any state, including failed/cancelled) joins the report-bytes protection contract above (§ Report Document Model retention, card #359) — the run row itself is separately protected by the `ON DELETE RESTRICT` FK.

**Content-addressed source blobs** (`report_snapshots/{sha256}` under the app data dir, ADR 0099 dec. 5, #384): `start_kpi_ingest` pins the bytes a run reads — the document's current file is read to memory, hashed, written to a per-invocation staged sibling and atomically renamed to its hash name; `mark_source_captured` then freezes that hash on the run. Document recapture overwrites `local_path` in place and NEVER touches a run-referenced blob — a run's bytes resolve by `source_content_hash`, not `local_path` (delivery: `get_kpi_ingest_document`, #385). Blobs are content-shared (natural dedup). Retention: a blob lives while ANY durable run in ANY state references it; **no blob GC exists** — the retention rule is documentation, the collector is future work. Staged-file crash orphans are unreferenced and harmless.

**Blob read path** (`get_kpi_ingest_document`, #385): the stored hash is validated as exactly 64 lowercase hex BEFORE any path is built (malformed → `internal`). Full-buffer hash verification against the frozen `source_content_hash` runs ONCE per `(canonical blob path, hash, size, mtime)` — a process-wide cache; later chunk reads seek (`O(file + delivered bytes)`, never `O(file × chunks)` — documents run up to the 50 MiB fetch cap). The canonical path in the key means one data directory can never authorize another's same-named blob. Accepted boundary: an external replacement that preserves path+size+mtime inside the app-owned, content-addressed, never-rewritten store is outside this check. Availability follows the SOURCE, not the status: a hash-less run (cancelled/failed straight from `discovered`) → `conflict`; any hash-bearing run, terminal included, reads fine; a `discovered` run WITH a hash is a broken invariant (`internal`).

**Context read model** (`get_kpi_ingest_context`, #385): one call serves run status, document metadata (`byteSize` = the pinned blob's size, never the mutable row's), the hash-guarded `derivedPeriod` hint, the catalog (expected keys resolved through the sector-aware resolver + the company's `origin='agent'` minted extras; an expected key that fails to resolve is `internal`, never a silent omission), per-slot plausibility evidence (`observed` slots discovered from `slot_history_points` — the dated, scan-order-preserving source `slot_metric_histories` is a projection of, so validator history and evidence cannot drift — plus `candidate` default slots whose window comes from the validator's `is_flow_key` classifier and the profile: stock → `point_in_time`, interim flow → `cumulative`, other flow → `flow`), `profile_rules` doctrine, and manifest availability. The manifest section serves `latest_validation_attempt` (`ORDER BY revision DESC, attempt DESC`, `failed` included — the run row's `manifest_hash` is NULL after a failed validation) and continuation cursors pin the attempt id (`validation_attempt_by_id`; append-only table, so a pinned id always resolves). Catalog/plausibility cursors are deliberately NOT snapshot-pinned: they are keyset cursors over the live read model — the same contract as `list_pending_kpi_ingests` — so a definition minted mid-pagination that sorts before the cursor appears on the next full read, never as a duplicate; the manifest is the one paginated artifact where mixing two versions would be corruption, and it alone pins. Budget arithmetic: with identities ≤256 B control-character-free (JSON escaping ≤×2), the ≤256-key expected stamp is ≤~135 KiB escaped worst-case, the manifest header is bounded by the stage-tool string caps — so the default call's dynamic shrink always converges above its bounded baseline and the final `response_budget_exceeded` gate is defensive only.

### KPI Staged Observations

`kpi_staged_observations` (migration 0138, [ADR 0098](adr/0098-mcp-native-kpi-acquisition-lifecycle.md) decisions 3/4, epic #352 card #359): run-owned LLM proposals. **Never visible to any fact reader** — a proposal becomes a canonical `financial_fact` only through the commit transaction (`commit_manifest`, #362).
...
- **Revision is shared with the run**: `revision` mirrors `kpi_ingest_runs.manifest_revision`. `stage_observations(run_id, holder, observations)` runs one `Immediate` transaction that atomically bumps the run's counter, zeroes `manifest_hash` (a new snapshot invalidates any prior manifest), flips `status` to `staged`, and inserts a **complete** snapshot under the new revision with store-assigned, contiguous `ordinal`s (0..n-1) — two racing stagers serialize on the run row, so exactly one revision ever lands per staging call. Stale revisions are kept, never deleted (audit trail). Legal only from `status IN ('extracting', 'validation_failed')` — every other status (`InvalidRunStateForStaging`, Conflict) — requires a LIVE lease for `holder` (`RunLeaseNotHeld`, card #360 — `stage_observations` is one of the three agent-facing intents alongside `mark_source_captured`/`begin_extracting`, none of them lease-free), and only once the run has a period identity (`period_id` or a complete descriptor). **"Complete snapshot" is now proven per CALL, not per revision** ([ADR 0102](adr/0102-full-capture-staging-contract-excluded-observations-and-chunked-drafts.md) dec. 6-9, epic #399 S6): a single-call revision (`draft` absent) is still one complete snapshot in one transaction, unchanged; a CHUNKED revision (`draft` present) assembles the complete snapshot across MULTIPLE calls via `kpi_ingest_drafts`/`kpi_ingest_draft_chunks` below, and only `finalize_draft` — sharing `install_observations_on_connection` with the single-call path — performs the SAME bump+insert atomically, once, from the assembled set. Either way, exactly one `Immediate` transaction ever performs the bump — the per-call proof is now per-CHUNK (100 observations/1 MiB) plus a new aggregate bound at finalize (see below), not per-revision.
...
- `mapping_status` (`unmapped` default | `mapped` | `no_definition` | `excluded`, migration 0150 widens the CHECK — see below) and `validation_state` (`none` default | `passed` | `unreviewed` | `flagged`, reusing the 0057 provenance vocabulary) plus `validation_codes_json` — always `none`/`NULL` at staging time; only `apply_validation_outcome` (#361) sets them, in the same transaction as the immutable attempt row (§ KPI Ingest Validation below).
- **`excluded` disposition + `exclusion_reason`** (migration 0150, [ADR 0102](adr/0102-full-capture-staging-contract-excluded-observations-and-chunked-drafts.md) dec. 1, epic #399 S5): `excluded` is a DELIBERATE, reasoned "captured, not mapped" disposition — distinct from an accidental `unmapped`, which stays validated `flagged` (dec. 2). `exclusion_reason` (TEXT, nullable) is REQUIRED exactly when `mapping_status = 'excluded'` and refused otherwise — a rule SQLite's CHECK cannot express across two columns, enforced in `stage_observations` (`InvalidKpiIngestRunValue`). Migration 0150 is the standard SQLite table-rebuild (0138's `mapping_status` CHECK is baked at `CREATE TABLE` time, so widening it needs the 0119/0123 rebuild shape) — every column/FK/index/unique constraint survives (`rebuild_preserves_rows_constraints_indexes`, `storage/tests/migration_safety.rs`).

### KPI Ingest Drafts

`kpi_ingest_drafts` + `kpi_ingest_draft_chunks` (migration 0151, [ADR 0102](adr/0102-full-capture-staging-contract-excluded-observations-and-chunked-drafts.md) decisions 6-11, epic #399 S6): the chunked-transport extension to `stage_kpi_observations` — a report disclosing >100 numbers cannot fit the single-call 100-observation/1 MiB proof, so a draft carries the same complete-snapshot invariant across multiple calls instead. A draft is a **sub-resource of the run, never a new run state** — `STAGEABLE_STATUSES`/the ADR 0098 dec. 6 lifecycle are untouched; a draft only ever exists while the run sits in `extracting`/`validation_failed`.

- **`kpi_ingest_drafts`**: `draft_id` (`kpidft_{32 hex}`, server-issued — never client-supplied, so two agents sharing a credential cannot resurrect each other's draft), `run_id` (`ON DELETE CASCADE`), `lease_epoch`, `expected_observations` (`>= 1`), `status ∈ {active, superseded}`, `created_at`. A **partial unique index** on `(run_id) WHERE status = 'active'` enforces exactly one active draft per run.
- **`lease_epoch` binds a draft to `kpi_ingest_runs.attempt_count` at open time** — no new run column: `attempt_count` already increments ONLY on a genuine claim of an absent/expired lease (same or foreign holder), never on a same-holder renewal (`claim_run_on_connection`) — exactly the epoch a lease takeover bumps. A takeover is detected LAZILY: the first operation (open/append/finalize) that reads a draft whose `lease_epoch` no longer matches the run's current `attempt_count` marks it `superseded` (never eagerly, by the takeover itself — the TTL is not a correctness mechanism, matching the rest of this store's lease philosophy). `open_draft` reuses the SAME conflict check `stage_observations` uses to refuse a single-call snapshot while a draft is open (`KpiIngestActiveDraftExists`): a STALE active draft is superseded and the open proceeds; a LIVE-epoch one refuses.
- **`kpi_ingest_draft_chunks`**: `draft_id` (`ON DELETE CASCADE`), `chunk_index`, `chunk_hash` (SERVER-computed sha256 over `payload_json`, never trusted from the client), `payload_json` (a `Vec<NewStagedObservation>`, the storage-internal round-trip shape — plain `snake_case`, distinct from the MCP wire's `ObservationInput`), `observation_count`, `created_at`. `UNIQUE(draft_id, chunk_index)`. Append (`append_chunk`) re-checks the LIVE lease on EVERY call (not only open/finalize — closing the gap a long capture session would otherwise leave), never bumps `manifest_revision`, and is structurally invisible to validation until finalize (its own table, never merged into `kpi_staged_observations` until then). Replaying the same `(draftId, chunkIndex)` with a MATCHING server-computed hash is an idempotent ack (safe retry over an unreliable transport); a different hash at the same index is a typed `KpiIngestDraftChunkConflict`, never a silent overwrite.
- **Finalize** (`finalize_draft`) reads every chunk ordered by `chunk_index`, refuses a gap or zero chunks (`KpiIngestDraftIncomplete`), sums `observation_count`/`payload_json` byte length and refuses over-cap (`KpiIngestDraftAggregateBudgetExceeded` — see caps below), flattens the chunks in `(chunkIndex, position-within-chunk)` order, then calls **`install_observations_on_connection`** — a connection-level helper extracted from `stage_observations`'s body (same validate/normalize/guarded-UPDATE/insert logic, unchanged) — on the SAME `Immediate` transaction: bump-revision + install-observations (global, store-assigned, contiguous `ordinal`s, exactly like today) + flip-state + `DELETE FROM kpi_ingest_drafts WHERE run_id = ?` (unconditional — cascades to chunks; a guaranteed no-op on the single-call path, which already refused upfront if a live draft existed). Finalize never calls the public `stage_observations` as a black box — it shares the helper on `&Connection` instead (ADR 0102 dec. 9).
- **Aggregate caps at finalize** (ADR 0102 dec. 10, amends ADR 0099 dec. 7 — the per-call 100-observation/1 MiB proof is UNRAISED, it now bounds a CHUNK): `AGGREGATE_OBSERVATIONS_MAX = 1000` (headroom over #398's measured ~426 tagged facts/package) and a **frozen aggregate byte cap = 10 × 719,872 B = 7,198,720 B (≈ 6.87 MiB)** — 10 is `AGGREGATE_OBSERVATIONS_MAX` / the 100-per-chunk cap (at most 10 chunks), 719,872 B is contracts.md tool 5's already-proven per-call escaping bound (703 KiB), reused deliberately conservative rather than deriving a tighter, unproven number. Aggregate `missingReasons` reuses the existing per-call `MISSING_REASONS_MAX` (128) unchanged — finalize's `missingReasons` travels exactly like the single-call form's.
- **Lifecycle**: a single-call snapshot while a draft is open is refused (`KpiIngestActiveDraftExists`) — an explicit abort is required, never a silent orphan. Run cancellation (`cancel_run`), failure (`mark_failed`), and startup lease-reclaim (`reclaim_ingest_runs_on_startup`) all clear any open draft (`ON DELETE CASCADE` makes chunk cleanup structural); startup reclaim additionally clears any draft left on a run that currently holds no lease at all, broader than only the leases that pass just cleared but strictly correct (a draft requires a live lease to be usable). `get_kpi_ingest_status.openDraft{draftId, chunksReceived, expectedObservations}` reads a live-epoch active draft (`None` for no run, no draft, or a stale one) — a restarted agent session resumes instead of guessing. A draft — open or appended-but-unfinalized — is never itself validated or committed; only an installed revision is (the run's `status`/`manifest_revision` never move until finalize).
...
**Diagnostic code registry** — a CLOSED enum (`fundamentals::kpi_manifest::DiagnosticCode`, wildcard-free `as_str`/`parse`/`severity`; `REGISTRY` + a golden-list test keep it stable across refactors). `severity()` (`flagged` | `unreviewed` | `info`, ADR 0102 dec. 2 adds `info`) derives an observation's `validationState`; the three run-level codes always force `outcome = failed`.
...
| `mapping.excluded` | `mapping_status = 'excluded'` — a DELIBERATE, reasoned exclusion ([ADR 0102](adr/0102-full-capture-staging-contract-excluded-observations-and-chunked-drafts.md) dec. 1/2); `detail: { reason }` carries the staged `exclusion_reason` verbatim | info (never flagged, never a run-level diagnostic) |
...
Normalization before every check: `attribution_eff = attribution.unwrap_or("total")` (mirrors `storage::financials::slot_dims`); `measure_window_eff = measure_window` (no default — `NULL` is its own code, `period.window_missing`). **Short-circuit**: `excluded` skips EVERY other per-observation check ([ADR 0102](adr/0102-full-capture-staging-contract-excluded-observations-and-chunked-drafts.md) dec. 2 — an excluded observation is held to none of the fact-quality checks, since it will never become a fact; the ONLY code it carries is `mapping.excluded`) — checked FIRST, before `mapping.unresolved` is even evaluated. `mapping.unresolved` skips every definition-dependent check (`unit.*`, `period.window_kind_mismatch`, `identity.violation`, `plausibility.*`, `duplicate.*`); `value.unparseable` additionally skips `identity.violation`/`plausibility.*`/`duplicate.*`. Mapping: any flagged code ⇒ `flagged`; only unreviewed/info-severity codes ⇒ `unreviewed`; none ⇒ `passed`.

**Expected-KPI snapshot stamp** (`kpi_ingest_runs.expected_kpis_json`, set-once): written by **`create_run_if_absent` in the creating INSERT** ([ADR 0099](adr/0099-acquisition-mcp-surface-mechanics.md) dec. 6). Shape: `{schemaVersion: 1, source: "kpi_relevance+profile_pack", packVersion: "<profile_version>", keys: [...sorted]}` — `keys` is the union of the company's active `primary`-ranked `kpi_relevance` metric keys (`expected_primary_metric_keys`, computed on the creation transaction) and the profile's statement-type pack (§ Extraction-profile registry below); an empty union → `keys: []`, no completeness gate. FROZEN from creation for the run's WHOLE lifetime — the validator consumes the column verbatim, and a definition minted or a relevance row added after creation never enters the denominator ([ADR 0093](adr/0093-agent-write-surface-hardening.md) dec. 4, structurally). The idempotent-hit path returns the existing run without restamping. **Legacy-NULL fallback**: a raw-seeded row whose column is NULL is live-stamped by the first validation via `KpiIngestRunsStore::stamp_expected_kpis` — guarded `UPDATE … WHERE id=? AND status='staged' AND manifest_revision=? AND manifest_hash IS NULL AND expected_kpis_json IS NULL`, shape `{schemaVersion: 1, source: "kpi_relevance", packVersion: null, keys}` (the two provenances stay distinguishable by both `source` and `packVersion`). 0 rows updated is not automatically a refusal — a non-NULL column is returned unchanged; only a wrong status/revision is `InvalidStagingRevision`. A later revision (repair/restage) reuses the SAME frozen snapshot by design: the validator, #362, and #363 all read this column, never live `kpi_relevance`.

**`missing_reasons_json` schema**: a JSON object `{"<metricKey>": "<non-empty reason string>"}`. Malformed, not an object, or any non-string/blank value anywhere → `run.invalid_missing_reasons` (the whole ledger is then treated as empty for completeness purposes, though the code still fires). Keys outside the expected snapshot are ignored — never a gate on their own. **Writer (#386)**: `stage_observations` writes the whole declaration in the SAME staging transaction — the map is required and every revision REPLACES it (`{}` is the explicit clear; there is no destructive default and no other production writer).

**`cost_json` writer (#386, ADR 0099 dec. 8)**: `merge_cost_json_on_connection` — shallow per-key merge of the caller's `ExecutionMeta` over the stored object (omitted fields survive; numerics are totals/snapshots, never accumulated), `schemaVersion` stamped 1, a corrupt stored object replaced fresh (diagnostic, never part of the trust verdict). Runs ONLY inside the `stage_observations` and `commit_manifest` transactions; the commit replay fast-path never writes (the receipt is immutable and replays ignore `execution`).

**Manifest shape** (`fundamentals::kpi_manifest::KpiIngestManifest`, canonical serde: camelCase, fixed field order, `BTree*` collections, no `skip_serializing_if` — every `Option` is an explicit `null`, `Decimal` values `.normalize()`d). `observations[].normalizedValue` specifically: `value.normalize().to_string()` when the staged string parses as `Decimal` (`"1234.500"` → `"1234.5"`, so two runs staging the same value with different trailing-zero formatting hash identically); the raw string verbatim when it does not parse (the same string `value.unparseable` fires against, never silently dropped):
...
{ manifestSchemaVersion: 1, validatorVersion: "1", runId, revision, outcome: "ready"|"failed",
  ...
  observations: [ { observationId, ordinal, rawLabel, metricKey, definitionId|null,
                     normalizedValue|null, currency|null, unitScale|null,
                     attribution, measureWindow|null, scope|null,
                     citation: { page, table, row, quote },
                     validationState, codes: [{ code, detail|null }],
                     excluded, exclusionReason|null } ],  // ordinal order
...
`outcome`/every observation's `validationState`/`counts` are hand-settable Rust fields (the negative-construction tests need that) but only `SealedManifest::seal()` treats a manifest as trustworthy: it independently RE-derives all three from `codes`/`runDiagnostics` and rejects any mismatch (`ManifestSealError`) — a manifest claiming `ready` over a flagged observation, or `passed` over a flagged/abstained code, is structurally unconstructible past `seal()`. `seal()` additionally rejects (`ManifestSealError`): a duplicate `observationId` or a duplicate `ordinal` across `observations`; any `{code, detail}` pair outside the ONE shape that code admits (data-model.md § Rejestr kodów — e.g. `plausibility.implausible` must carry `HistoryMedian`, `duplicate.*` must carry `ObservationIds`, `mapping.excluded` must carry `Reason`, every other code must carry `null`); an `excluded` disposition inconsistent with its `exclusionReason`'s presence OR with whether `codes` actually carries `mapping.excluded` ([ADR 0102](adr/0102-full-capture-staging-contract-excluded-observations-and-chunked-drafts.md) dec. 1 — a hand-built manifest cannot claim the disposition without the evaluator's own code, `ExcludedDispositionInconsistent`); and, when `expectedKpis` is present, a `completeness` inconsistent with `observations` (`present` must be exactly the mapped metric keys among observations that fall inside `expectedKpis.keys`, `missing`/`present` disjoint, their union inside `expectedKpis.keys`). `SealedManifest` is otherwise opaque (private fields): `manifest_hash()` (64 lowercase hex), `manifest_json()`, `observation_verdicts()` (`{observationId, validationState, validationCodesJson}` — exactly what lands in `kpi_staged_observations`), `observation_content()` (a canonical staged-content projection — ordinal, raw label, candidate key, normalized value, currency, effective scale/attribution/window, scope, citation, excluded disposition + reason ([ADR 0102](adr/0102-full-capture-staging-contract-excluded-observations-and-chunked-drafts.md) dec. 1) — re-compared by the atom against the LIVE row, sol r5-2's content-tamper guard), and per-field run-context accessors (`companyId`, `reportDocumentId`, `sourceContentHash`, `scope`, `dataQuality`, `periodId`/`fiscalYear`/`periodType`, `expectedKpisJson`) the atom binds below.

**Apply** (`KpiIngestStagingStore::apply_validation_outcome(run_id, revision, sealed)`) — the SOLE `staged → {validation_failed, ready_to_commit}` path (§ KPI Ingest Runs transition table): one `Immediate` transaction, guards (staged/revision/hash-NULL, `sealed.runId`/`revision` binding, exact observation-set coverage, content-tamper re-comparison) → batch-write every verdict → INSERT the attempt row → connection-level transition per `sealed.outcome`. Any guard failure rolls back everything, including the attempt insert — a refused manifest writes NOTHING. Two more guards close the gap between "structurally consistent" and "consistent WITH THIS RUN": RUN-CONTEXT BINDING re-compares `sealed`'s `companyId`, `reportDocumentId`, `sourceContentHash`, `scope`, `dataQuality`, period (`periodId`/`fiscalYear`/`periodType`), and `expectedKpis` (byte-compared against the run's `expected_kpis_json`, both-NULL OK) against the LIVE run row; COUNT requires `sealed`'s observation count to equal the live staged-row count, defense in depth alongside the coverage check above (which set-collapses and would otherwise mask a duplicate `seal()` somehow missed).

**Resolver** (`storage::kpi_extraction::resolve_kpi_definition`, sector-aware, shared by the structured-extraction pipeline AND the validator): sector-scoped definitions match on **two axes** (#383) — the raw directory `companies.sector` (runtime-created definitions, e.g. `"Gry"`) OR the derived `statement_type` (the seeded statement packs share its vocabulary, § Company Fundamentals), the statement axis guarded to non-`industrial` so the classification default matches nothing the old NULL-sector behavior did not. Precedence: company-scoped `>` canonical `>` **raw-sector match** `>` **non-industrial statement-type match** `>` every remaining global non-sector definition, lexicographic by id within a rank. A sector-scoped definition matching NEITHER axis is EXCLUDED, not merely deprioritized. Pinned-commit eligibility (`record_pinned_fact`) mirrors the same rule via the Rust twin `sector_definition_matches`; a truth-table test drives identical cases through both paths. Deterministic — two calls for the same `(company_id, metric_key)` return the same `definition_id`.

**#362 integration note**: the commit transaction consumes `manifest.observations[].definitionId` and the frozen `expectedKpis`/`completeness` snapshot straight from the sealed manifest bytes (`KpiIngestRunsStore::get_validation_attempt`, `outcome='ready' ORDER BY attempt DESC LIMIT 1`) — it NEVER re-resolves a definition or re-reads live `kpi_relevance`. No attempt row for `(run, revision, hash)` (the pre-0139 compatibility case above) is a typed refusal (`MissingValidationAttempt`), not a silent re-derivation.

### KPI Ingest Commit Receipts

`kpi_ingest_commit_receipts` (migration 0138, ADR 0098 dec. 5, card #359, written by card #362): the immutable per-run commit outcome, append-only — zero `UPDATE` path in the store (the `valuation_runs` idiom).

- **`UNIQUE(run_id)`** — one receipt per run. **Idempotent replay (#363)**: `commit_manifest` classifies the stored receipt BEFORE attempt parsing/sealing — one JOINed receipt+run read; receipt↔run coherence FIRST (run terminal `complete|partial`, run hash/revision == receipt, `run.status == receipt.terminal_status`; incoherent → `CommitReceiptRunMismatch`, Internal — a corrupt pair never hides behind "stale"), THEN the requested tuple (match → the stored row verbatim, `committed_at` included, never re-executing the write primitives — dec. 5: `Created` would answer `Reobserved` on replay; different tuple → `StaleManifestForCommit`, never an overwrite). The receipt fast path deliberately never re-reads the attempt — the stored receipt stays the stable answer even if historical attempt bytes rot. The same classification resolves the **race loser** (a parallel commit won: after its own `BEGIN IMMEDIATE` the loser re-reads on the transaction handle — decisive, every earlier writer has committed/rolled back — and returns the winner's receipt) and the **BUSY path** (`busy_timeout` exhausted at `BEGIN IMMEDIATE` → guard dropped, one recovery read → matching receipt or the explicitly-retryable `CommitContention`, Conflict — never a raw SQLite error). `CommitReceiptAlreadyRecorded` stays unreachable from any production flow (fast path + status guard cut earlier); the `UNIQUE` mapping remains as defense in depth. **The dec.-5 per-(company, period) lock is realized by serialization**: SQLite's single writer plus the `Immediate` transaction spanning the whole commit — strictly stronger than a targeted lock; no separate mechanism exists.
- Written by `record_commit_receipt`, a **connection-level free fn** (`storage::kpi_extraction::record_structured_fact` pattern) that `KpiIngestCommitStore::commit_manifest` (#362) calls under its own externally-owned `&Connection`/transaction — never opens one itself, so it rolls back with the caller's transaction on any failure.
- Carries `manifest_hash`/`manifest_revision`, `terminal_status` (`complete`/`partial` — `partial` iff the manifest's `completeness.missing` is non-empty; `completeness.missing_without_reason` already forces `failed` at validation time, #361, so every `missing` entry here carries an honest reason), `period_id` (`ON DELETE SET NULL`), `accepted_count` (created+reobserved+upgraded — `divergent`/`excluded` never count, no write happened), and `outcomes_json` — a versioned array of `{observationId, revision, ordinal, metricKey, factId, outcome, detail?}`. **`outcomes_schema_version`** is a durable format discriminator, bumped only if the shape changes, so a reader can branch on it forever — **every commit now writes `2`** ([ADR 0102](adr/0102-full-capture-staging-contract-excluded-observations-and-chunked-drafts.md) dec. 3, epic #399 S5: the `excluded` outcome is v2-only); stored `1` receipts are never rewritten and stay readable forever (the MCP reader, `mcp::kpi_ingest_submit::receipt_dto`, accepts both — v1's shape invariant, "every non-divergent outcome carries a `factId`, divergent never does", is never loosened, only extended with a THIRD legal case under v2: `excluded`, no `factId`, carrying its `{label, reason}` detail).
- `outcomes_json` is a **new type**, NOT a reuse of `jobs::record_financial_facts::FactOutcome` — that type carries no `factId`, and a receipt outcome must identify the fact a proposal became (or `NULL` for a rejected one, `divergent`/`excluded` only). `outcome` vocabulary: `created`/`reobserved`/`upgraded`/`divergent`/`excluded` (ADR 0102 dec. 3 adds `excluded`; `no_definition`/`implausible`/`identity_violation` stay in the vocabulary as 0138 schema documentation only — unreachable from `commit_manifest`, which never re-resolves or re-judges plausibility). `excluded`'s `detail` carries `{label, reason}` — sealed from the manifest's `rawLabel`/`exclusionReason` (ADR 0102 dec. 1), never the live staged row (commit reads only the frozen manifest, so a post-validation tamper to the staged row cannot reach an excluded write at all). **Exclusion never manufactures `partial`** (ADR 0102 dec. 4, deliberate deviation from issue #399's own wording — `partial` stays exclusively a function of the `expected` denominator, ADR 0098 dec. 4/5): every excluded observation is ALWAYS ledgered regardless of `terminal_status`; the one interaction is that an excluded observation that WAS an expected key still counts as missing from the denominator and still requires a `missing_reasons` entry. **`commit_kpi_ingest`'s wire response is a BOUNDED summary** ([ADR 0102](adr/0102-full-capture-staging-contract-excluded-observations-and-chunked-drafts.md) dec. 12, epic #399 S6, amends ADR 0099 dec. 1): `{runId, terminalStatus, counts: {acceptedCount, excludedCount}, manifestHash, manifestRevision}` — the full `outcomes` array (up to `AGGREGATE_OBSERVATIONS_MAX` = 1000 rows under a chunked draft) is never inlined; it reads via the paged `get_kpi_ingest_context section:"receipt"` (mirrors the manifest section's mechanics — offset-only cursor, since a run has AT MOST one commit receipt EVER, `UNIQUE(run_id)`, so no attempt-pinning is needed). `validate_kpi_ingest` likewise returns a bounded summary (`{outcome, revision, manifestHash, severityCounts, current}` — observation counts by `validationState`, not the manifest itself); the full manifest still reads via the existing `section:"manifest"`.

Integration gate (closed by #362): `record_commit_receipt` inserts caller-supplied values without reading the run — `commit_manifest` verifies, inside the SAME outer transaction, that `begin_committing` already transitioned the row to `committing` under the caller's exact `manifest_hash`/`manifest_revision` before this insert is ever reached.

**Pinned-definition/statement_basis note (#362)**: every fact `commit_manifest` writes goes through a dedicated `pub(super) record_pinned_fact` primitive (`storage::kpi_extraction`) — it consumes `manifest.observations[].definitionId` verbatim (re-validated for existence/metric-key/sector eligibility on both axes via `sector_definition_matches`, never re-resolved) and sets `statement_basis = obs.scope.unwrap_or(manifest.scope)`, closing a real slot-mapping bug the public `record_structured_fact`/`record_aggregator_fact` paths still carry unchanged: those hardcode `statement_basis = None` (→ `consolidated` via `slot_dims`'s default), so a `standalone` run's facts previously landed in the `consolidated` slot. Provenance is hardcoded in the primitive (`source_tier="agent"`, `extraction_method="mcp_agent"`, `confirmation_state="confirmed"`, `drift_json=None`), mirroring `jobs::record_financial_facts`'s triple.
...
### KPI Ingest Queue Integration (#364)

The durable queue (`jobs/kpi_ingest_queue`, ADR 0098 dec. 8 model A) executes the two SHORT DETERMINISTIC lifecycle steps — validation and commit — on a dedicated `kpi_ingest` lane (constant 1 worker, no settings knob: both steps are short SQLite writes globally serialized by the single writer anyway). Producers: tests today; the acquisition planner (#354) later — the #353 agent path runs validation and commit synchronously inside the MCP tool call and never arms the queue (ADR 0099 dec. 1).

- **Generation-pinned jobs.** A job row is pinned to one GENERATION of work and its id carries the full identity: `kpi_ingest_validate:{run}:rev{N}` / `kpi_ingest_commit:{run}:rev{N}:{manifest_hash}`; the payload duplicates it (plus a redundant `jobId` the handler preflight verifies against the REAL claimed row id — a row whose id and payload disagree never does domain work). A re-stage bumps the revision (new validate generation); invalidate + revalidate freezes a hash for the same revision (identical content legitimately reuses the identical id — same tuple, same identity). Two kinds = two rows = separate retry budgets (max 3 each): the transition table's "the validator's" vs "the commit job's" exhausted retries.
- **Arming (`arm`)**: `pending` → noop (attempts/backoff preserved); `running` → typed error — the **single-armer invariant, RESOLVED at #386**: the synchronous MCP path introduces NO new armer (stage never enqueues; validate/commit run the primitives in the tool call and never arm — ADR 0099 dec. 1), so arming stays the queue chain plus startup reconciliation and a running row of the generation being armed remains unreachable; the sync×queue collision tests prove convergence, and a future concurrent producer (if the #354 planner adds one) revisits this. The error is PROPAGATED (a handler's own row retries; reconciliation refuses to start the lane), never logged-and-ignored. Missing/terminal rows are `reschedule`d — arming is always a fresh intent, never a restart loop; `reschedule` follows the caller's kind/payload/budget (the id IS the identity, so a kind-drifted row heals on re-arm).
- **Terminalization is generation-guarded (#386, E1)**: both callers — the live terminal hook and startup reconcile — write through `mark_failed_for_generation(id, error, IngestGeneration)` (typed `Staged{revision}` / `ReadyToCommit{revision, manifest_hash}` — the ready variant cannot omit its hash), whose WHERE-clause guard makes the select-then-fail pair atomic: a re-stage landing between the caller's selection and the write is a silent no-op (`Ok(false)`) and the run lives on in its NEW generation. Reconcile previously had no revision re-check at all; the guard closes that hole structurally.
- **Supersession invariant**: a handler that proves its generation superseded (run state or tuple no longer matches the payload) arms the CURRENT generation from the live tuple before returning success — a noop hands the baton, never drops it. Validation outcome `failed` is queue SUCCESS (the run returned to the agent's worklist); `CommitContention` retries; a still-matching tuple that errors retries until the row is terminal.
- **Terminal failure is id-authoritative and event-first.** Handlers return bare `Err`; the queue's dispatch marks the row durably `failed`, fires the (0118-deduped) `job_failed` attention event, then calls the `on_terminal_failure` hook, which parses the generation from the job id (never the payload) and marks the run `failed` only if it still sits in that generation's queue-owned state. The run stays queue-owned until the row is durably terminal, so every crash window leaves a `(job running|failed, run queue-owned)` pair that startup reconciliation completes — no terminal failure is ever silent. Failure surface: `TodayAttention` for both kinds (no run UI exists in E1); scope/subject also derive from the job id.
- **Startup order (`lib.rs`)**: (1) run-level reclaim (#360) → (2) explicit `reclaim_stale_running` (the later call inside `spawn_pools` is an idempotent noop) → (3) `reconcile_ingest_jobs`: for each `staged`/`ready_to_commit` run, derive the current generation's (kind, id, payload) from the live tuple and, by row status: `failed` → event FIRST (dedup) then domain `mark_failed`; `pending`/`running` → untouched; missing/`succeeded` → arm. Failed rows of superseded generations are inert bookkeeping — reconciliation never terminalizes a run from a mismatched row. A `ready_to_commit` row with a NULL `manifest_hash` is raw corruption: typed error, no id fabrication. → (4) `spawn_pools`. **Steps 2–3 succeeding is a precondition for the `kpi_ingest` lane** (fail-closed): on failure the lane stays off until the next launch — pending ingest jobs wait, invisible to every worker; other lanes run normally.
- **`progress_json`** (diagnostic, the `cost_json` posture — never part of the trust verdict): written ONLY inside the two atoms, so a stale writer or a rolled-back commit cannot leave a lying snapshot. Shape: `{"schemaVersion":1, "step":"validation_ready"|"validation_failed"|"committed", "revision", "manifestHash", "at", "counts":{…}}` — validation counts `observations`/`flagged`, commit counts `accepted`/`created`/`reobserved`/`upgraded`/`divergent`.
- **`record_source_outcome` deferred to #354** (owner decision 2026-08-13): it is a source-ADAPTER health stamp; the agent-driven path has no `source_adapters` row and no Sources presence, so the DoD guardrail's failure mode cannot occur. The real adapter (the ESPI publication watcher) arrives with #354 and stamps it there. The DoD wording is narrowed to "adapter-backed refresh/ingest path" accordingly.
- **One app process per database** is an enforced invariant (`tauri-plugin-single-instance`, registered first in the builder): a second process would run rival worker lanes and its reclaim would dead-letter jobs genuinely running in the first.
...
- **Restore** is a restart operation surfaced in Settings › Data storage (moved out of Diagnostics 2026-09-08, #451): the chosen backup is staged and applied on app relaunch (no hot in-place swap), because live connections hold the database open. **Verified, never destructive** (#319): staging and apply both run the candidate through `PRAGMA integrity_check` and a `schema_migrations` version-set check (open read-only — verification never mutates the candidate); a candidate that fails is refused and the live database is left untouched — never fatal, `open_pool` continues with the original database and surfaces the reason as a Today attention event. On a verified apply the original is checkpointed and copied aside as an ordinary `backups/pre-restore-<ts>.sqlite3` before the swap (restorable like any other backup, not deleted); a `restore-journal.json` written just before the swap's two file operations (sidecar removal, rename) makes a crash between them resumable on the next start — recovered automatically by copying the pre-restore file back if the swap did not complete, or by simply deleting the journal if it did. The journal is published atomically (unique temp file, `sync`, then rename), so a malformed journal from a crash mid-write never blocks startup when the database is intact — it is dropped instead, falling back to the newest `pre-restore-*.sqlite3` when the database is not intact either. Only a crash-recovery rollback that itself cannot complete is fatal (`StorageError::Io`, setup aborts, as for any unopenable database).
