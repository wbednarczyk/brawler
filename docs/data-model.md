...
- `fetch_status` is `pending | fetched | failed | metadata_only` (the `metadata_only` value is additive over migration 0035 — no new migration). It is the single source of truth for whether bytes exist locally; a failed fetch records `fetch_error`, stays retryable, and never blocks feed-item ingestion. **`fetched` is never silently downgraded** (#455): `mark_failed` is an atomic guarded update (`UPDATE … WHERE NOT (fetch_status = 'fetched' AND local_path IS NOT NULL AND local_path <> '')`, mirroring `mark_metadata_only`'s guard) — a fetch attempt that races a concurrent successful capture declines the downgrade, and the caller re-reads and reports the existing fetched document rather than "no file". Before any fetch, capture/refetch entry points (`capture_report_document`, `fetch_report_document`) check the row's **verified identity** against disk — file present, size and `content_hash` match the stored row — rather than trusting "a file exists at this path": a `fetched` row that still matches needs no refetch; a `failed` row whose file matches is healed back to `fetched` (clearing `fetch_error`, via the same `mark_fetched` write the initial fetch uses — the recovery path for the three-row bug the issue named); a `fetched` row whose file is missing or no longer matches is reset to `pending` (`local_path` cleared) and refetched. The final file is published **atomically**: bytes land at `<local_path>.part` first, then a single rename publishes them at the final path, and the row is updated only after that rename succeeds — a crash mid-write leaves only the `.part` file, never a truncated file at the path the row claims.
