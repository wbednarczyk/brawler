...
- `manifest_hash`/`manifest_revision`, `attempt_count`, `expected_kpis_json` (the versioned completeness-denominator snapshot, stamped set-once by validation — § KPI Ingest Validation), `missing_reasons_json`, `progress_json`, `cost_json` (diagnostic only, never part of the trust verdict), `last_error`.
...
**Expected-KPI snapshot stamp** (`kpi_ingest_runs.expected_kpis_json`, set-once): before validation runs, guarded `UPDATE … WHERE id=? AND status='staged' AND manifest_revision=? AND manifest_hash IS NULL AND expected_kpis_json IS NULL` (`KpiIngestRunsStore::stamp_expected_kpis`). 0 rows updated is not automatically a refusal — if the column is already non-NULL the snapshot was stamped by an earlier attempt (possibly on an earlier revision — the snapshot is per-run, see below) and is returned unchanged; only a wrong status/revision is `InvalidStagingRevision`. Shape: `{schemaVersion: 1, source: "kpi_relevance", packVersion: null, keys: [...sorted]}` — `keys` is the union of the company's active `primary`-ranked `kpi_relevance` metric keys (degenerate today to `FinancialsStore::expected_primary_metric_keys`; `None`/empty → `keys: []`, no completeness gate). Once stamped, FROZEN for the run's WHOLE remaining lifetime, not just the revision that stamped it (ADR 0098 dec. 4: "stamped on the run at start") — the column lives on `kpi_ingest_runs`, one per run, not one per revision; the `status='staged' AND manifest_revision=?` guard only constrains WHEN the write may legally happen (the first validate call of the run), never which revision's snapshot gets used afterward. A later revision (repair/restage) reuses this SAME frozen snapshot by design: the validator, #362, and #363 all read this column, never live `kpi_relevance` — a later relevance change never retroactively changes an already-attempted run's manifest. A future statement-pack profile (E2) moves the stamp to `create_run_if_absent` (`packVersion` stops being `null`).
