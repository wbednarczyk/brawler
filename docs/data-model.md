...
- `parse_status` (`ok` | `unparsed_value` | `unresolved_context` | `unsupported_unit`) + `parse_error` — **decision 9: every supported occurrence gets a row even when normalization fails**, nullable `value_numeric`, typed status, never a silent drop.
- `created_at`/`updated_at`.
...
`report_tagged_fact_roles` — the fact↔presentation-role **relation** (decision 3): a concept participates in several roles (income statement, comprehensive income, cash-flow reconciliation, …), so a scalar column on `report_tagged_facts` could not tell those occurrences apart. `fact_id` → `report_tagged_facts(id) ON DELETE CASCADE`, `role_uri`, `role_kind` (the classified family: `balance` | `income` | `comprehensive_income` | `cash_flow` | `equity_changes` | `other` — an unrecognised role classifies as `other` explicitly, never by guess). `PRIMARY KEY (fact_id, role_uri)` — a pure junction table needs no surrogate id (the `report_document_sections` composite-PK idiom). `esef_package::extract_presentation_roles` reads every `*_pre.xml` in the package and matches on concept **local name** only (the same carve-out decision 1 makes for the existing 22-concept mapping); `esef_package::classify_role` recognises the standard `-210000`/`-220000` (balance), `-320000` (income), `-410000`/`-420000` (comprehensive income), `-520000` (cash flow), `-610000` (equity changes) role-number suffixes plus the vendor Polish role names — the 320000/410000/420000 income-vs-comprehensive-income split is this slice's concrete reading of the ADR's grouped prose, flagged for owner review.
...
- `ReportTaggedFactStore::coverage_counts(company_id)` — the Coverage panel's compact `TaggedFactCoverageCounts { rawStored, projected, dimensional, noteLevel, awaitingName, conflicting }`. `dimensional` is a direct `is_dimensional = 1` count; the other four re-run `project_period` per `(report_document, period_end)` found in the company's Layer 1 rows (including comparative periods, which Layer 1 stores but Layer 2 never writes — decision 7). `projected` here means "the deterministic rule would keep it", **not** "it is live in `financial_facts` right now" — a stricter count is a direct `financial_facts` query. `raw_stored` may exceed the other five buckets' sum by the rare count of unparsed-value rows, which fit none of them.
