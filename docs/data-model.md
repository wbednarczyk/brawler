...
- **Layer 2 — statement packs** (`source = 'sector'`, `seed_statement_pack_kpi_relevance` + migration `0126_seed_statement_pack_kpi_relevance.sql`, #273): joins `companies.statement_type` to the `scope='sector'` definition packs (seeded since `0034`, never read at runtime until now) over a **conservative allow-list** — keys genuinely universal within the type, because an expectation nobody reports inflates the recall denominator without making the gate smarter. **banking**: `net_interest_income`, `net_fee_commission_income`, `total_loans`, `total_deposits` (ratios/capital measures live in the notes, not the statements). **insurance**: `gross_insurance_revenue` only (the IFRS 17 top line; GWP and earned premium are now supplementary). **reit**: `ffo` only. **specialty_finance: nothing** — `0095` maps exchanges and brokers onto the same `statement_type` as debt collectors, so no pack key is universal across the mix. Deterministic `kpirel_sector_<company>_<metric>` ids. Reclassification is **additive**: the new pack is added, the old rows stay.
- **Layer 3 — derived observations** (`source = 'derived'`, `rank = 'secondary'`, `refresh_derived_kpi_relevance`, #274): marks keys the company reports in **≥3 of its last 4 periods with an issuer-tier fact** (a provenance row whose `source_tier` is not `html_aggregator`; no provenance = manual = not issuer evidence). Enrichment for the characteristic-KPI surface and the coverage display. A key that stops being reported is **not** deleted — staleness is a display concern. **These rows never gate**: `expected_primary_metric_keys` filters `source != 'derived'` *structurally*, not by trusting the rank, because the completeness gate compares extraction output against expectations — deriving the expectations from extraction output would let a systematic extraction hole silently erase the expectation that would have caught it.
...
- **Window semantics** (user-facing wording: [wiki/dsl-reference.md](../wiki/dsl-reference.md) § Window semantics). `_ttm`/`ttm()` on an annual period is that period's own figure; on an interim period it is the sum of the four most recent periods in the series and is **unavailable when fewer than four carry the figure** (#62 — a partial window is never presented as a trailing twelve months). `_avg` is a two-point average that *does* degrade to the current value when the prior period lacks the figure (one point-in-time figure is still a figure). `cagr(metric, n)` looks the baseline up by **fiscal-year label** (`fiscal_year − n`, not series position), is unavailable on a missing label or a non-positive endpoint, and takes the fractional root in `f64` — a growth-rate threshold does not need decimal-exactness.
...
- `fact_id` (PK → `financial_facts.id`), `source_tier` (`esef` | `structured_xhtml` | `espi_cover_note` | `pdf` | `html_aggregator` | `ai_text` | `ai` | `manual`), `validation_status` (`passed` | `witness_confirmed` | `unreviewed` | `flagged`; the legacy `none` is **retired from all production writes** — ADR 0077 §4 / G-1 — but readers must keep tolerating it on historical rows written before the retirement), `drift_json` (serialized `DriftReport` label diff when a drift accompanied the fact), `citation` (source concept/label the value was read from).
...
- **Mis-association guard + repair** (card 45fcece): tag-listing ingestion (bankier-company-komunikaty) stamps the tag's company onto every attachment. Before a row is created, an attachment that classifies as a **periodic report** (`doc_kind` `periodic_ssf`/`periodic_jsf` — the only kind that can win a period's canonical slot) is **rejected** when its filename names a *different* tracked issuer and no owner mention (name/alias/ticker over the article title, filename, and URL) is found; rejections are logged, no row. An idempotent startup repair (`repair_misassociated_report_documents`, wired in `lib.rs` setup, mirroring the `reclassify_report_documents` precedent) re-scans `espi_attachment` rows with the **same predicate** (no drift) and deletes the mis-associated ones (FK `CASCADE`/`SET NULL` clean their derived rows). Deliberately conservative — foreign detection uses the filename's full company-name/alias phrase only (never a bare ticker, never the URL hosting path), so counterparty/shareholder mentions and Bankier folder names never delete a legitimate filing (validated against the owner DB: the naive url/ticker rule would have wrongly deleted 36 rows; the scoped rule deletes 0).
  ...
  - **Migration `0107` deleted four legitimate documents — do not repeat it** (content-verified 2026-07-30, epic #229 T3). Its `company_id='company_gpw_cbf' + espi_attachment + url LIKE '%energa%'` predicate was written on the belief that a `Grupy-Energa` filename meant an Energa filing OCR'd onto cyber_Folks. The bytes disagree: all four files are cyber_Folks' own Q3-2024 filing — PDF `/Author: cyber_Folks`, `/Title: cyber_Folks_2024 Q3_SSF`, body "Raport kwartalny Grupy cyber_Folks_". cyber_Folks was left with **no** 2024-Q3 periodic document at all. The files were never deleted — only the rows — so the repair is a re-insert, shipped as the startup self-heal below. The guardrail: `storage::migrations::migration_invariants::no_migration_deletes_report_documents_by_url_pattern` fails any future migration that deletes `report_documents` on a `url LIKE` predicate (0107 grandfathered — migrations are immutable once applied). **SQL cannot read bytes, so a migration is structurally the wrong instrument for an association repair.** T3's corpus-wide content triage of every flagged row (16 canonical slots + 33 mis-association candidates, deduped to 49) found **zero** truly-foreign documents, so T3 ships no repair migration at all. 0107 additionally deletes note-reference-misparsed pdf-tier cash facts (auto-unreviewed, whole multiple of 1000 ≤ 60 000) for re-extraction with the corrected parser (cards 22ac70c / 40281b3); that half stands. Forward, idempotent, self-healing: a clean database matches nothing.
  - **0107 restore self-heal** — `report_documents_restore::restore_migration_0107_deletions`, wired in `lib.rs` setup between the mis-association repair and the container sniff (owner decision 2026-07-30). Re-inserts the four rows from an explicit four-entry table (never a pattern — a heuristic here would repeat 0107's mistake), and **only when the bytes are on this machine**: a row claiming `fetch_status='fetched'` over an absent file is worse than a missing row, which is also why this is Rust and not a migration — SQL can neither read the bytes that prove ownership nor check the file exists. Per row: already present → no-op; company untracked → skip; file absent → skip and retry next start; else insert + `mark_fetched` (real byte length) + `detected_container` sniffed from those bytes. Every field is evidence-backed: the **URLs** are reconstructed from the surviving Vercom/Orlen rows carrying the same reused slug and verified by round-tripping through the `report_documents` id derivation (each derives exactly the filename still on disk — a guard test); **titles** come from the PDF `/Title` for the two statements (`cyber_Folks_2024 Q3_SSF.pdf`, `cyber_Folks 2024 Q3_JSF.pdf`) and from the cover page for the two report decks, whose `/Title` is a stale template naming Q3 **2023**; `doc_kind` is classified by the normal write seam, never hardcoded. Note the slug-distrust above does **not** rescue these rows — `Energa` is untracked, and foreign detection only ranges over tracked issuers; they classify correctly because the slug agrees with the title about the kind.
