...
- `period_fiscal_year`/`period_type` (migration 0138, ADR 0098 dec. 3, card #359) — the run's durable **period descriptor**, a natural key mirroring `financial_periods UNIQUE(company_id, fiscal_year, period_type)`. A run legally has no `financial_periods` row until the commit transaction (#363) creates one, so staging needs a stable period identity before that exists. **All-or-none** (`InvalidKpiIngestRunValue` on a partial pair); `period_type` vocabulary matches `financial_periods.period_type` exactly: `FY`, `H1`, `H2`, `Q1`–`Q4`, `9M`, `M01`–`M12`. When both `period_id` and the descriptor are present, `create_run_if_absent` verifies they name the same period (mismatch → typed error); an active run for the same `(document, company, profile)` triple later requested with a **different** period/descriptor is a typed `RunPeriodConflict` (Conflict), never a silent overwrite.
...
`kpi_staged_observations` (migration 0138, [ADR 0098](adr/0098-mcp-native-kpi-acquisition-lifecycle.md) decisions 3/4, epic #352 card #359): run-owned LLM proposals. **Never visible to any fact reader** — a proposal becomes a canonical `financial_fact` only through the commit transaction (#363).
