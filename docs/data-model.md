...
- **RETIRED (ADR 0095, 2026-08-05):** `pdf` used to also carry the **tier-3b positional** sub-tier ([ADR 0077](adr/0077-trusted-extraction-foundations.md) T-B2): a pdf2htmlEX visual-render XHTML (no `<table>`, no `ix:` tags — e.g. CD PROJEKT interims) that no ESEF tier can read, reconstructed from CSS glyph geometry, distinguished only by `financial_facts.extraction_method='html_positional'`. Its first ground-truth measurement (epic #40/#182, 2026-08-05) found currency-aware precision/recall of 5.6%/2.0% against ESEF's 99.3%/72.4% — the parser (`fundamentals/extraction/html_positional.rs`), its routing arm, and `positional_statement_basis` are REMOVED; migration `0135` deleted every stored `pdf`-tier fact (both this sub-tier and the legacy retired-PDF-arm's `api`-marked rows); the coherence guard (`storage::kpi_extraction::write_fact_provenance_fields`) now refuses `source_tier='pdf'` on any NEW write. `SourceTier::Pdf` stays in the enum as a legacy READ value only. Historical detail (kept for the record): its note-reference stripping was comma-list aware (bug #182) — a statement row's leading footnote reference is sometimes a comma-separated LIST (`"16,30"` — notes 16 *and* 30), shape-identical to a genuine Polish decimal (16.30); `value_columns`/`is_footnote_reference_list` treated it as a note-ref only when followed by a grouped (thousands-separated) value in the same row.
- **Tier↔method coherence** (bug #324): `source_tier` and `extraction_method` are two axes of the same write and must always describe the SAME route — `espi_cover_note` only under `espi_cover_note`, `mcp_agent` only under `agent`, the generic tiered pipeline's `api` under `esef`/`structured_xhtml`/`espi_cover_note`/`html_aggregator` (`html_positional` only ever under `pdf` is RETIRED history — ADR 0095, `pdf` is refused outright for any new write) (`SourceTier::matches_extraction_method`, `fundamentals/extraction/mod.rs`). 7 rows on the maintainer's DB disagreed (`source_tier='esef'`, `extraction_method='html_positional'`): `record_structured_fact`'s tier-precedence UPGRADE (an issuer tier taking over an already-occupied slot) rewrote the provenance row's tier but never synced the fact row's `extraction_method`, which is fixed by syncing both fields in the SAME shared-writer call (`write_fact_provenance_fields`, `storage/kpi_extraction.rs`) alongside every upgrade, and enforced at RUNTIME (a typed `IncoherentFactProvenance`/`RetiredSourceTier` error, not a `debug_assert`) on every provenance write. Migration `0134` repairs the 7 historical rows (`source_tier='esef'` + `extraction_method='html_positional'` → `source_tier='pdf'`) — those rows are then deleted outright by migration `0135` a moment later in version order.
