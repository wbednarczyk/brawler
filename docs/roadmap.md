...
The next product milestone is `v0.53.0`. This is the forward plan (milestone intent only; live epic/task status and IDs are in Radicle/Radboard, see [kanban.md](kanban.md)):
...
- `v0.54.0` — **Attention routing + morning briefing** ([ADR 0068](adr/0068-attention-routing-and-morning-briefing.md)): the `Toast` primitive, user-owned alert rules over typed signals / autopilot events / price conditions, and a cited "what changed + what needs doing" morning briefing on Today.
- `v0.55.0` — **Source reliability & disclosure signals** ([ADR 0069](adr/0069-source-reliability-and-disclosure-signals.md)): the `Fetcher` trait on the SourceAdapter port, ESPI/EBI re-enabled as a reconciliation second witness (closes the Bankier single-point-of-failure), the KNF short-selling registry adapter, and the auditor-opinion/going-concern signal.
- `v0.56.0` — **Ownership structure** ([ADR 0072](adr/0072-ownership-structure.md)): `ownership_stakes` with history (capital % vs votes % separately), extraction from already-stored periodic reports + the major-holdings ESPI signal + aggregator witness, holder-type classification (TFI/OFE/founder/family foundation/state…), derived free float, donut + stake-over-time charts.
- `v0.57.0` — **Company health**: Piotroski F / Altman Z / Beneish M scores as deterministic derived metrics composed into the scorecard (extends [ADR 0046](adr/0046-quality-frameworks-quantitative.md)), insider-sentiment aggregation over existing signals, and a red-flags panel (auditor opinion, going concern, report delays, fund exits, scores). ADR at milestone planning.
- `v0.58.0` — **Analyst recommendations** ([ADR 0073](adr/0073-analyst-recommendations-tracking.md)): attributed third-party ratings/target prices with revision history, consensus-shift signals, "vs target" context beside market data — tracking, never advice. Resolves the former Future Study (see below).
- `v0.59.0` — **Re-invent the notebook panel** (informed by [ux-journeys.md](ux-journeys.md)).

### Cross-cutting UX epic: Research workspace shell (mode-based, thesis-centric)

> **Status: the first wave shipped in `v0.48.0`** — left-sidebar IA spine + pinned companies, the Today/Pulse Triage home (new default landing), the sectioned Company workspace with dockview as an opt-in Advanced layout, and Focus reader/writer modes ([ADR 0054](adr/0054-mode-based-thesis-centric-shell.md)). Per-company conviction status and the Compare KPI table remain shell placeholders pending their dependency milestones (quality `v0.50.0` / valuation `v0.61.0` / thesis `v0.64.0` / cross-company `v0.60.0`); the feed-triage keyboard state is a deferred follow-on (`SemanticSearch` retired — [ADR 0080](adr/0080-retire-embedding-model.md)). The "foundation lands right after `v0.48.0`" sequencing note below is superseded by this — the shell *was* `v0.48.0`.
