...
- `v0.51.0` — **Judgment capture** ([ADR 0071](adr/0071-judgment-capture.md)): the minimal decision journal (recorded buy/pass/keep-watching + rationale + evidence links — the early slice of ADR 0043's journal, pulled forward because judgment history cannot be backfilled) plus pre-report expectations with post-report expectation-vs-actual review.
- `v0.52.0` — **Import/export v2 + offsite backup**: unified data bundle and per-feature coverage, including the financial facts + KPI definitions export/import deferred from `v0.37.0` and the `v0.51.0` judgment entities, plus rotating backups to a user-chosen folder ([ADR 0032](adr/0032-search-and-backup-boundaries.md) update).
...
- `v0.64.0` — **Investment thesis workbench**: persisted, provenance-stamped theses (verdict as decision support, scenario forecasts, variant/inversion/disclosed-gaps, valuation↔thesis link with orphan check) extending the already-accumulating `v0.51.0` decision journal with outcome review and thesis links. Design in [ADR 0043](adr/0043-investment-thesis-and-decision-journal.md).
...
1. **NS1 — MCP surface + research chat** (after `v0.64.0` — the chat needs the thesis/valuation domain to exist). The main north star.
2. **NS2 — Investor calibration loop**: decision journal + pre-report expectations + outcomes + price history composed into a "where am I right / where am I wrong" review. Data accumulates from `v0.51.0` (judgment) and `v0.53.0` (price history).
...
**Promoted 2026-07-03 to the destination arc as NS1 — see [North Star: Talk To Your Research](#north-star-talk-to-your-research-mcp).** The design posture above stands; the ADR is written when NS1 is taken up (after `v0.64.0`).
...
- trade journal (note: a decision journal, distinct in scope, lands early at `v0.51.0` — see [ADR 0071](adr/0071-judgment-capture.md) / [ADR 0043](adr/0043-investment-thesis-and-decision-journal.md))
