...
The next product milestone is `v0.47.0`. This is the forward plan (milestone intent only; live epic/task status and IDs are in Radicle/Radboard, see [kanban.md](kanban.md)):
...
- `v0.47.0` — **Report-over-report diff**: diff consecutive periodic reports section by section with a cited AI delta summary. **First retrieval consumer of the `v0.45.0` embedding model** — semantic section alignment across reports (embeddings propose, the AI delta summary disposes).
- `v0.48.0` — **Feed triage mode and command palette**: keyboard feed triage and a global command palette over search, navigation, and actions — including **`SemanticSearch`** over the embedding model (find-by-meaning ranking), the model's primary post-`v0.45.0` home now that clustering is dropped.
