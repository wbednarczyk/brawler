...
The next product milestone is `v0.50.0`. This is the forward plan (milestone intent only; live epic/task status and IDs are in Radicle/Radboard, see [kanban.md](kanban.md)):
...
- **Cross-cutting — Trusted extraction foundations** ([ADR 0077](adr/0077-trusted-extraction-foundations.md), ships as `v0.51.0`, continues the [ADR 0061](adr/0061-deterministic-fundamentals-data-gathering.md) epic; subsequent milestones renumbered +1 at closure — Radicle `milestone:` labels follow at the next planning pass): the owner's post-v0.50 trust verdict turned into measured reliability — document taxonomy (`doc_kind` + canonical report per period), a fundamentals **coverage map** (what has data, what is missing, why), a **history sweep** chained from backfill (add company → Backfill → data appears without per-document clicking), a free-tier-only vision/OCR tier-4 fallback chosen by a numeric ground-truth spike (Gemini vs Mistral OCR; descoped if neither clears the bar), one validation regime for AI and deterministic facts (`validation_status='none'` retired), AI call budgets, and a recall/precision ratchet on real labeled reports. Execution plan: [docs/plans/trusted-extraction-foundations.md](plans/trusted-extraction-foundations.md). **Status: delivered — closing `v0.51.0`** (F0–F6; the vision spike chose free-tier **Mistral OCR** and added a positional `html_positional` tier; shipped the coverage map, backfill-chained history sweep, per-company OCR profile, per-sweep AI budget, and review queue). Delivered detail lives in [CHANGELOG.md](../CHANGELOG.md); remove this line at release.
- `v0.52.0` — **Judgment capture** ([ADR 0071](adr/0071-judgment-capture.md)): the minimal decision journal (recorded buy/pass/keep-watching + rationale + evidence links — the early slice of ADR 0043's journal, pulled forward because judgment history cannot be backfilled) plus pre-report expectations with post-report expectation-vs-actual review.
- `v0.53.0` — **Import/export v2 + offsite backup**: unified data bundle and per-feature coverage, including the financial facts + KPI definitions export/import deferred from `v0.37.0` and the `v0.52.0` judgment entities, plus rotating backups to a user-chosen folder ([ADR 0032](adr/0032-search-and-backup-boundaries.md) update).
- `v0.54.0` — **Market data foundation** ([ADR 0067](adr/0067-market-data-foundation.md)): Stooq EOD quotes with full history, company sector classification (registry-sourced + manual override), market cap and level-0 market ratios (P/E, P/BV, EV/EBITDA, dividend yield, FCF yield, 52-week stats, own-history percentile) via the derived-metrics engine. Unblocks the valuation arc, price alerts, and the future backtest/calibration north stars.
- `v0.55.0` — **Attention routing + morning briefing** ([ADR 0068](adr/0068-attention-routing-and-morning-briefing.md)): the `Toast` primitive, user-owned alert rules over typed signals / autopilot events / price conditions, and a cited "what changed + what needs doing" morning briefing on Today.
- `v0.56.0` — **Source reliability & disclosure signals** ([ADR 0069](adr/0069-source-reliability-and-disclosure-signals.md)): the `Fetcher` trait on the SourceAdapter port, ESPI/EBI re-enabled as a reconciliation second witness (closes the Bankier single-point-of-failure), the KNF short-selling registry adapter, and the auditor-opinion/going-concern signal.
- `v0.57.0` — **Ownership structure** ([ADR 0072](adr/0072-ownership-structure.md)): `ownership_stakes` with history (capital % vs votes % separately), extraction from already-stored periodic reports + the major-holdings ESPI signal + aggregator witness, holder-type classification (TFI/OFE/founder/family foundation/state…), derived free float, donut + stake-over-time charts.
- `v0.58.0` — **Company health**: Piotroski F / Altman Z / Beneish M scores as deterministic derived metrics composed into the scorecard (extends [ADR 0046](adr/0046-quality-frameworks-quantitative.md)), insider-sentiment aggregation over existing signals, and a red-flags panel (auditor opinion, going concern, report delays, fund exits, scores). ADR at milestone planning.
- `v0.59.0` — **Analyst recommendations** ([ADR 0073](adr/0073-analyst-recommendations-tracking.md)): attributed third-party ratings/target prices with revision history, consensus-shift signals, "vs target" context beside market data — tracking, never advice. Resolves the former Future Study (see below).
- `v0.60.0` — **Re-invent the notebook panel** (informed by [ux-journeys.md](ux-journeys.md)).

### Cross-cutting UX epic: Research workspace shell (mode-based, thesis-centric)

> **Status: the first wave shipped in `v0.48.0`** — left-sidebar IA spine + pinned companies, the Today/Pulse Triage home (new default landing), the sectioned Company workspace with dockview as an opt-in Advanced layout, and Focus reader/writer modes ([ADR 0054](adr/0054-mode-based-thesis-centric-shell.md)). Per-company conviction status and the Compare KPI table remain shell placeholders pending their dependency milestones (quality `v0.50.0` / valuation `v0.62.0` / thesis `v0.65.0` / cross-company `v0.61.0`); the feed-triage keyboard state + `SemanticSearch` are a deferred follow-on. The "foundation lands right after `v0.48.0`" sequencing note below is superseded by this — the shell *was* `v0.48.0`.
...
1. **NS1 — MCP surface + research chat** (after `v0.65.0` — the chat needs the thesis/valuation domain to exist). The main north star.
2. **NS2 — Investor calibration loop**: decision journal + pre-report expectations + outcomes + price history composed into a "where am I right / where am I wrong" review. Data accumulates from `v0.52.0` (judgment) and `v0.54.0` (price history).
...
The owner study (2026-07-02) is **resolved 2026-07-03** per its own decision criteria: promoted to milestone `v0.59.0` with design, entity shape, source-policy approach, and the tracking-never-advice presentation recorded in [ADR 0073](adr/0073-analyst-recommendations-tracking.md).
...
**Promoted 2026-07-03 to the destination arc as NS1 — see [North Star: Talk To Your Research](#north-star-talk-to-your-research-mcp).** The design posture above stands; the ADR is written when NS1 is taken up (after `v0.65.0`).
