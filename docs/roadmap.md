...
The next milestone is `v0.41.0`. This is the forward plan (milestone intent only; live epic/task status and IDs are in Radicle/Radboard, see [kanban.md](kanban.md)):

- `v0.41.0` — **Report document ingestion & history backfill**: persist ESPI/EBI report attachments and user-supplied report URLs as stored report documents (full file stored for periodic reports only; other filings kept as metadata + URL), an explicit on-track action that backfills ~3 years of periodic reports and filings for a company (calendar stays with the existing forward-looking calendar adapters), and **derived calendar events from dividend/general-meeting signals** — extracting the future date from the report body deterministic-first with an opt-in AI fallback, always confirm-before-create. All target the active Bankier official-report path; report ingestion and backfill were deferred from `v0.34.0`, and event derivation from `v0.40.0`. Design in [ADR 0036](adr/0036-report-document-storage-and-backfill.md).
