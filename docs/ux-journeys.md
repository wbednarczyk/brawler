...
- **Steps:** Library nav → Report Season (or `Otwórz ekran: Report Season`) → per-company pre-report card (open questions, unresolved claims, last KPIs, evidence) → add expectations (stance + optional metric expectations) → mark as prepared.
...
- **Steps:** Library nav → Events (or `Otwórz ekran: Events`; F4b S4 — Events joined the Library nav, ADR 0054 amendment) (week calendar — what's coming) → `Otwórz ekran: Watchlists` (overview: lists with member counts, the selected list's members) → Library nav → Research (one click, or Ctrl+4; F4c S3 — Research joined the Library nav, #94) (review queue + gaps) → Spółka (deepening, via the review queue row's own `Company` destination action — owner decision 2026-08-26, ADR 0107; label per ADR 0104 dec. 3, F4c) → plan the week.
