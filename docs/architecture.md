...
The AI surface is split into two layers ([ADR 0035](adr/0035-two-layer-ai-and-local-interpretative-layer.md)). The **generative** layer (above) handles summarize/extract/assess and is provider-neutral and remote by default. The **interpretative** layer (`src-tauri/src/interpretation`) is on-device semantic lookup exposed as task-level capability contracts — since [ADR 0080](adr/0080-retire-embedding-model.md), `Classifier` only (live consumer: ESPI signal classification). Feature code binds to a capability, never to a model: implementations are selected through a registry, with a deterministic **static** baseline (rules) as the shipped default.
