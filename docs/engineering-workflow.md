...
- [ ] New UI workflow/behavior has a Vitest component/workflow test; a new/changed primitive follows [ui-authoring § Adding a new primitive](ui-authoring.md#adding-a-new-primitive) (gallery + a11y contract).
...
- [ ] **A new IPC command** adds a step to the **dual-execution mock-fidelity corpus** — unless it is documented headless-only / MCP-only (no mock half): [Testing § Mock-runtime fidelity](testing.md#mock-runtime-fidelity--the-dual-execution-contract).
...
- [ ] Every flagged defect has its **class** closed in this change per [CLAUDE.md § Guardrail harvest](../CLAUDE.md) (ritual: the `guardrail-harvest` skill); a bug not fixed now → a tracked GitHub issue.
