...
- [ ] **Mutants — one owner-triggered sweep per epic closure** (ADR 0049): agent reminds; owner runs `gh workflow run mutants.yml` (never agent-initiated, never on WSL, no cron). Expand `-f` to the epic's new transform modules; findings = cards, never blockers. · **`make bench`** when a hot kernel changed — bench-ratchet vs `bench-baseline.json` (informational, machine-dependent, never a hard gate).
