...
### §A — Always
- [ ] Implemented to spec: read the canonical doc(s) for the area (the [Required Reading](../CLAUDE.md) map) — don't infer architecture/field/command names from code alone. ADR added/confirmed if durable architecture or policy changed — shipped `Accepted` (dated to the owner's decision) once the owner approved the plan; `Proposed` is only for decisions still awaiting approval, and epic closure never flips a status.
