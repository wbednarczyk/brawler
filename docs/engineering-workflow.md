...
- [ ] New UI workflow/behavior has a Vitest component/workflow test; a new/changed primitive follows [ui-authoring § Adding a new primitive](ui-authoring.md#adding-a-new-primitive). Test-hygiene lints (`.only`, reasonless `.skip`, no-assert tests, attribute-vs-paint): [ui-authoring § Enforcement](ui-authoring.md#enforcement).
...
- [ ] **A new IPC command** adds a step to the **dual-execution mock-fidelity corpus** — or is declared in `src/test/scenarios/headless-only.json`; enforced by `fidelity-membership.test.ts` + the Rust reachability guard: [Testing § Mock-runtime fidelity](testing.md#mock-runtime-fidelity--the-dual-execution-contract).
