...
- `make check-local` before handover (the full `make check` runs only as the PR's required checks — see What runs where); `make build` to validate frontend production output; `make frontend-preview` for a quick browser layout check from Windows (no Tauri APIs).
- `make smoke-keyring`: opt-in live smoke test needing OS keychain state.
...
Brawler uses Nix from the first scaffold: `flake.nix` is canonical, `nix develop` the explicit entrypoint (optional `direnv`). Nix provides toolchains, not a command hiding place — build/test commands stay runnable inside `nix develop` (sole exception: `check-visual` runs docker, #448). Secrets stay outside the Nix store (never in `flake.nix`/`flake.lock`/`.envrc`); commit `flake.lock`. Flake provides Rust + fmt/lint, Node.js/npm, Linux Tauri prerequisites, SQLite dev libs, `pkg-config`, plus `devShells.windows-cross` (`packaging` skill). CI runs the same `make` targets inside `nix develop`.
...
**The loop for every behavior change:** 1) **write/extend the test first**, at the cheapest layer that proves the behavior (map below) — a feature isn't "done" until a test **reddens when it breaks** ([Testing § Strategy](testing.md)); 2) **iterate against a targeted, fast subset** (seconds; see "Targeted run" below, or `make check-local`); 3) the **full `make check` runs only as the PR's required checks** ([ADR 0096](adr/0096-quality-gate-architecture-under-continuous-release.md)) — the local pre-handover floor is `make check-local`.
...
- [ ] Feature-branch commit/push freely (owner 2026-09-08); see git-boundaries above.
...
- [ ] [ui-authoring § Pre-write self-check](ui-authoring.md) done (primitive-first, no raw controls / inline styles).
- [ ] New panel/screen or redesign: [ui-authoring § Mockup-first](ui-authoring.md) satisfied (approved mockup in `docs/mockups/`; in every UI subagent brief).
- [ ] Strings per [ui-authoring § i18n](ui-authoring.md) (both locale maps; translation guard green).
- [ ] New UI workflow/behavior has a Vitest component/workflow test; a new/changed primitive follows [ui-authoring § Adding a new primitive](ui-authoring.md#adding-a-new-primitive). Hygiene lints: [ui-authoring § Enforcement](ui-authoring.md#enforcement).
- [ ] **You rendered the changed screen and looked at it** — don't defer the visual check to the user. **Look at measured/sticky table geometry under the DEFAULT smoke scenario too, not only the feature's own overlay** (harvest 2026-09-07). "No GUI in WSL" is not a reason: the browser harness renders any screen headlessly in Chromium ([Testing → Browser UI regression smoke](testing.md#browser-ui-regression-smoke-playwright)) — drive a throwaway Playwright spec, `await page.screenshot(...)`, read the PNG; add any command it calls to `src/test/browserSmokeRuntime.ts`.
...
- [ ] **"Newest/latest" selection orders by the domain date, never `created_at`** (backfill makes them diverge); ships a test where `created_at` order ≠ domain-date order. Rationale: [data-model.md](data-model.md#model-principles); gate: `recency` source-tree guard.
- [ ] Non-trivial CPU/inference/IO work runs **off the UI thread** (`async fn` + `spawn_blocking`) and reads the persisted derived index rather than recomputing the corpus per call. G9 pins sync cmds.
- [ ] **A new adapter-backed refresh/ingest path calls `record_source_outcome`** + a test that `last_success_at` gets set — else Sources shows "never refreshed". Gate: `OUTCOME_RECORDERS` pin + `source_outcome_coverage`. A path with no `source_adapters` row (agent-driven KPI ingest, #364) is exempt.
...
- [ ] **A new IPC command** adds a step to the **dual-execution mock-fidelity corpus** — or is declared in `src/test/scenarios/headless-only.json`: [Testing § Mock-runtime fidelity](testing.md#mock-runtime-fidelity--the-dual-execution-contract).
- [ ] **Any `#[ts(export)]` type change — including an additive `CommandErrorCode` variant — regenerates the TS bindings in the same change** (`make types`): `check-local` excludes the ts-rs drift guard, so only CI reddens on it; the `ts-export-reminder` hook reminds you.
- [ ] **Write transactions are `IMMEDIATE`** (gate `no_write_transaction_is_deferred`, G8 pins writers) — DEFERRED risks `SQLITE_BUSY_SNAPSHOT`. **Multi-step writes ship a fault-injection test** (poison step k, no partial state): [Testing](testing.md#failure-path-tests-fault-injection).
...
- [ ] Every flagged defect has its **class** closed in this change per [CLAUDE.md § Guardrail harvest](../CLAUDE.md) (ritual: the `guardrail-harvest` skill); a bug not fixed now → a tracked GitHub issue. Retro escaped-defect tables and advisory-workflow carding are gated.
...
### Visual baseline (ADR 0076 D7, pinned renderer #448)

Committed baselines under `tests/browser/visual/`: each panel × S/M/L widths on `chromium-visual` (dark) + one M pass on `chromium-visual-light`; only these two projects run `tests/browser/visual/**`.

- **Compare:** only inside the pinned renderer — `make check-visual` (docker required; official Playwright image tagged at the locked `@playwright/test` version). Zero tolerance (byte-identical). Required CI check: **`Visual baselines (pinned renderer)`**. Determinism: animations off, clock fixed at `SAMPLE_NOW`, `document.fonts.ready` per shot, Chromium raster flags (`playwright.config.ts`).
- **Deliberate update:** `make visual-update SCREEN=<name>|ALL=1 REASON="…"` — same docker wrapper, no PNG deletion first; commit the PNGs naming **which screens changed and why**. A Playwright bump moves the pinned tag; regenerate baselines in the same PR.
- **Host runs** (`make ui-smoke`, `ux-contact-sheet`, the 4 CI `browser` shards) execute the specs (layout/console gates hold) but never compare pixels — that only happens in the pinned renderer.
