...
- [ ] **A new IPC command** adds a step to the **dual-execution mock-fidelity corpus** — or is declared in `src/test/scenarios/headless-only.json`; gate: fidelity-membership + reachability guard: [Testing § Mock-runtime fidelity](testing.md#mock-runtime-fidelity--the-dual-execution-contract).
...
- [ ] Every flagged defect has its **class** closed in this change per [CLAUDE.md § Guardrail harvest](../CLAUDE.md) (ritual: the `guardrail-harvest` skill); a bug not fixed now → a tracked GitHub issue. Retro escaped-defect tables are validated in `check-docs-gates`; advisory workflows card findings.
