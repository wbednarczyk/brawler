...
- [ ] **Write transactions are `IMMEDIATE`** — `unchecked_transaction`/bare `.transaction()` are banned (source-scan gate `no_write_transaction_is_deferred`); a DEFERRED read→write upgrade under WAL returns `SQLITE_BUSY_SNAPSHOT` past `busy_timeout`. G8 pins writers.
