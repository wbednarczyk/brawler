...
- [ ] **Mutants — standing cadence: one sweep per epic closure, owner-triggered (2026-07-29, supersedes the 2026-07-16 on-request-only posture; closes #90)** (ADR 0049): at each epic closure the agent *reminds* and the owner runs `gh workflow run mutants.yml` (CI runners; NEVER `make mutants` on WSL — OOM); when the epic touched transform logic, expand the `-f` globs to its new modules first. No scheduled/cron sweeps (CI Posture: no scheduled workflows — mutants on unchanged code find nothing). Triage findings into cards, never blockers; still never agent-initiated. · **`make bench`** when a hot kernel changed — bench-ratchet vs `bench-baseline.json` (informational, machine-dependent, never a hard gate).
