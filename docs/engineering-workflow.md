...
- [ ] **A new adapter-backed refresh/ingest path calls `record_source_outcome`** + a test that `last_success_at` gets set — else Sources shows "never refreshed". A path with no `source_adapters` row (the agent-driven KPI ingest, #364) is exempt — the failure mode cannot occur; the stamp lands with the real adapter (#354).
...
- [ ] **Any `#[ts(export)]` type change — including an additive `CommandErrorCode` variant — regenerates the TS bindings in the same change** (`make types`): `check-local` deliberately excludes the ts-rs drift guard, so only CI reddens on it.
