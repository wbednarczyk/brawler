...
## Release-Owner License Workflow

M17 embeds public verification material in the app and keeps private signing material outside the repository and build outputs. Operational details live in [License Operations](license-operations.md).

Rules:

- Production private license signing keys must not be committed to the repository, stored in app databases, logs, exported settings, Nix files, `.envrc`, or GitHub Actions.
- Author and friend-test license tokens are generated by the release owner with private keys stored outside the app repository.
- The repository may contain token-format documentation and deterministic test signing material only when clearly marked as test-only.
- Packaged friend-test artifacts must be smoke-tested with missing, invalid, and valid license states before distribution.
- Hosted activation, billing, subscription refresh, revocation, or cloud entitlement checks require a future ADR.

External token-generation shape:

1. Build a JSON claims payload with `license_id`, `holder`, `channel`, `edition`, `features`, `issued_at`, `expires_at`, `app_version_range`, and `key_id`.
2. Base64url-encode the JSON payload without padding.
3. Build the signed message as `BRAWLER-LIC-1.<payload>`.
4. Sign that exact message with the release-owner Ed25519 private key matching the embedded public `key_id`.
5. Base64url-encode the signature without padding.
6. Distribute the token as `BRAWLER-LIC-1.<payload>.<signature>`.
7. Keep the private key, signing command history, generated token inventory, and recipient notes outside the app repository.
...
Generated token outputs default to `private/licenses/`, which is gitignored.
