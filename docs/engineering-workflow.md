...
- [ ] **Spec-conformance audit against the epic's ADR(s), decision by decision.** For every ADR decision, verify a **live-path invocation exists** (`repoctx callers` from the real job/command/UI entry, not only unit tests) and record a verdict (conforms / partial / deviates / not built). Unit-green pure modules with no live wiring are the recurring failure this catches — the ADR 0061 epic shipped a validated PDF tier, comparative cross-check, and drift persistence that **no live path invoked** (harvested 2026-07-02; the older instance of this class was the `v0.49` autopilot enable-toggle). "A capability is not done until a user can reach it" applies to every ADR decision, not just commands.
- [ ] `make check-epic` — the full mandatory gate + the coverage ratchet, **all hard-fail** ([ADR 0062](adr/0062-mandatory-test-gate-and-test-driven-loop.md); the old `-`-prefixed run-and-report steps are gone, and `gate-integrity` fails the gate if any return). knip + the full Playwright suite are already part of the per-commit `make check`, so closure is not the first time they run. Triage every failure (fix or file a tracked issue). · Retrospective written (both domains, still-open items honest). · `wiki/` created/updated for every user-facing change. · Version bump via the release workflow — **only on explicit user sign-off**.
- [ ] **`make mutants`** at closure cadence (ADR 0049): mutation-tests the deterministic transform cores (DSL eval, migrations, feed dedup/matching, source normalization) to prove the property/golden tests *kill* defects, not just execute them. Slow + periodic (never in `make check`, and kept separate from `check-epic` — a 30 min–2 h hard-fail step is a footgun to bundle into the closure command); when a new dedup/normalization/matching transform lands, extend the `-f` scope in the `mutants` target. · **`make bench`** when a hot kernel changed — the bench-ratchet flags relative regressions against `bench-baseline.json` (informational, machine-dependent, never a hard gate).

### §K — Honest handover report — always
- [ ] **A "gate green" claim requires the gate's own exit code as evidence** — for a backgrounded `make check`, grep the echoed `EXIT=`/`${PIPESTATUS[0]}` line from the saved output; a wrapper/task-notification exit code is **never** evidence (it reflects the last shell command, not `make`). A failed step also **aborts the steps after it** — a partially-green log proves nothing about the suites that never ran (two S6 gate runs were mis-reported green exactly this way, with the browser suite never having started).
