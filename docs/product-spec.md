...
Some GPW issuers (CD PROJEKT among them) publish their **interim** statements only as a visual XHTML render (no machine-readable table or iXBRL tags); the deterministic positional parser that once reconstructed those figures from the page layout is retired ([ADR 0095](adr/0095-retire-html-positional-tier.md)) — such interims now arrive, if at all, from the BiznesRadar-primary pull or an MCP agent's manual read, never a bespoke layout parser.
