...
The Coverage panel gives the investor a per-company **map of what data exists for each reporting period** — one row per fiscal period showing whether a canonical report was found, how many facts were extracted and how many of those are validated versus flagged, and how many proposals still await review. Its purpose is that **gaps are always visible, never silent**: a period with no report, a report that was never processed, or a period the app skipped because an AI budget ran out each appears explicitly rather than simply going missing, so the user can trust that an empty cell means "nothing here" and not "quietly dropped". Clicking a period opens that period's report documents to act on the gap. Confirming an AI-extracted value **validates it against the same accounting-identity checks as every other fact** ([ADR 0077](adr/0077-trusted-extraction-foundations.md) §4): a confirmed total that breaks the balance-sheet identity is surfaced as flagged rather than trusted silently — one validation regime for all sources, deterministic or AI.

The panel's footer carries the two **history actions** that close those gaps in bulk. **Backfill history** fetches the last few years of reports and filings, then automatically extracts the periods it just fetched — the single action behind the acceptance journey "add a company → Backfill → data appears" (the backend chains a **history sweep** onto a completed backfill). **Extract missing periods** runs that sweep on its own, for the case where the documents are already stored and only the extraction is missing (no re-download). A history sweep enqueues extraction for every canonical periodic report whose period still lacks accepted facts; it extracts what the deterministic pipeline can for free, and falls back to the AI vision tier **under a per-sweep AI budget** ([ADR 0077](adr/0077-trusted-extraction-foundations.md) §6) — the budget (default 30 calls per sweep, configurable in Settings → AI, 0 = unlimited) is fixed when the sweep starts, and once it runs out the remaining periods are marked **"skipped — budget"** on the map rather than silently dropped or run at unbounded AI cost over a whole backfill. Both actions require the company to be opted into automation: a company with automation **off** shows the actions disabled with an explicit "automation off" status rather than silently doing nothing. Backfill **depth is configurable** in Settings → Sources (1–10 years, default 3), and when a backfill hits its page cap before reaching that depth the panel reports the **truncation** explicitly ("older filings may be missing") — never a silent gap. While a sweep drains, the status line shows the live extraction counter ("Extracting… {done}/{total}").

**Tier-4 vision fallback** ([ADR 0077](adr/0077-trusted-extraction-foundations.md) §4). When the deterministic pipeline cannot read a PDF (its layout drifted, or nothing structured is present) and the caller allows it — a manual "Extract data" click, an autopilot detection run, the manual "Extract KPIs" job, or a history sweep run **within its per-sweep AI budget** ([ADR 0077](adr/0077-trusted-extraction-foundations.md) §6) — the app falls back to reading the report through **Mistral OCR**. Crucially, the AI **never reads the numbers**: the first time a company reaches this tier the model only *proposes the table layout* (which row label means which metric, the reporting scale, which column is the current period), and every value it produces lands as a **proposal for the user to confirm** — nothing is written as a fact. Confirming any of those first proposals is what **confirms the layout** (until then every OCR run keeps landing proposals); once confirmed, every later document for the company is parsed **deterministically** through it, and the resulting values pass the **same accounting-identity checks as every other source** before they are stored (`source_tier` = AI, with the OCR label kept as the citation); a value that fails validation becomes a proposal rather than a silent fact. The tier is **PDF-only** and degrades honestly when it cannot help — a structured/ESEF document, a company with no vision provider configured, or a layout the model couldn't map each shows an explicit reason instead of a silent empty result.

Some GPW issuers (CD PROJEKT among them) publish their **interim** statements only as a visual XHTML render (no machine-readable table or iXBRL tags); those now extract **deterministically** through a positional parser that reconstructs the figures from the page layout — no AI, no vision budget, the current-period (year-to-date) column read automatically ([ADR 0077](adr/0077-trusted-extraction-foundations.md) T-B2).

Those unconfirmed values collect in the per-company **Review queue** ([ADR 0077](adr/0077-trusted-extraction-foundations.md) §5, T5.3b), reachable from the Coverage map's "to review" cell or the panel palette. It lists every pending proposal grouped by reporting period, each tagged with where it came from — an unconfirmed OCR profile (`OCR bootstrap`), a deterministic parse the accounting-identity check flagged (`OCR · flagged`), or an older text-AI proposal (`AI`) — beside the source snippet and document. **Confirm** validates the value and commits it as a fact (confirming the first OCR proposal also confirms the company's extraction profile, so later documents read deterministically); a flagged validation outcome is surfaced as a caution and never blocks. **Reject** discards the proposal. An empty queue means the deterministic pipeline is writing facts directly with nothing left to review.
