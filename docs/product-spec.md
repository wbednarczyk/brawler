...
Daily note work happens in the company's Spółka `notatnik` tool; every cross-screen entry point (an Inbox feed item, research evidence, global search) lands its draft there (F4c S2, ADR 0108 amendment — resolved 2026-09-02: the cross-company Notebooks screen this section's earlier assumption anticipated saw no real use, 7 notes across 6 companies in 3 months, and retired).
...
**Activity — what the app is doing** (F3d, [ADR 0109](adr/0109-activity-center-occurrence-ledger.md), #133; v0.81.0). One place, opened from the top toolbar, tells the investor what background work runs **now** and what finished **recently** (the last 7 days), as tasks in plain language — reading a report, fetching a company's history, refreshing a source, reading a shareholder table — grouped per company, each with progress where the task has real counters and with a single action that lands on the item itself (the document, the company's coverage, the Sources screen). A task's outcome is stated honestly: finished, failed with its raw error, partially done, queued (waiting or retrying), stalled or interrupted after a crash — never a perpetual "in progress". The toolbar control signals work in progress only; it never counts failures, so the Today stream stays the one place a failure is announced and this panel is where its state is read. Cancelling or re-running tasks is not part of this surface (retry lives where the task's result lives, e.g. a period's "Try again" in Coverage).

When a rule fires it records an **attention event** tied to the exact evidence that triggered it — the signal item, the autopilot run card, or the quote context — so every alert traces back to a fact. Events are **deduplicated** (re-ingesting the same filing never re-fires) and each rule is throttled to at most once per day. Fired events surface two ways ([ADR 0097](adr/0097-toasts-are-action-feedback-only.md)): the **Today attention stream** grouped by company where events are marked seen or dismissed, and an unobtrusive **sidebar badge on Today** counting unseen non-routine events (it clears on a visit — *seen* means the event was on screen the last time Today was open). **No attention event raises a toast** — toasts are reserved for feedback on the investor's own actions; one polite screen-reader announcement coalesces count increases. Attention routing is **in-app only** in v1 (the boundary is shaped for later OS-notification adapters) and stays strictly factual — an alert states what happened with a link, and never phrases it as advice.
...
Cross-company wall folding (the `×K spółek` aggregates of [ADR 0087](adr/0087-today-attention-home-v2.md) amendment 2026-07-23) was a surface behavior of the v1 Today stream and retired with it (F2 Dziś v2, epic #410): attention events now render as individual rows inside their day's section, and volume is controlled by day sections, the per-day S-tier cap, and day collapse instead of cause-keyed aggregates. The severity typing and aging-demotion rules above are read-model semantics and are unchanged.

## Morning Briefing

The morning briefing answers "what changed in my companies and what needs doing" — a deterministically composed list of **new signals since the last briefing, autopilot runs, claims due, upcoming report dates, and fired alerts**, ordered by when each thing happened.

**Since F2 (Dziś v2) it is no longer surfaced on the Today screen** ([ADR 0068](adr/0068-attention-routing-and-morning-briefing.md) amendment 2026-08-20): Today's delta header carries the "what changed since your last visit" job. Briefing composition stays in the backend and is consumed over MCP (`generate_morning_briefing` / `get_latest_morning_briefing` — BYOA posture, [ADR 0084](adr/0084-retire-in-app-ai-layer.md)).
...
The panel's footer carries the two **history actions** that close those gaps in bulk. **Fetch older reports** fetches the last few years of reports and filings, then automatically extracts the periods it just fetched — the single action behind the acceptance journey "add a company → Fetch older reports → data appears" (the backend chains a **history sweep** onto a completed backfill). **Read the ones not read yet** runs that sweep on its own, for the case where the documents are already stored and only the extraction is missing (no re-download). A history sweep enqueues extraction for every canonical periodic report whose period still lacks facts, running the deterministic tiers only — no AI, no per-call budget. Both actions require the company to be opted into automation: a company with automation **off** shows the actions disabled with an explicit "automation off" status rather than silently doing nothing. Backfill **depth is configurable** in Settings → Sources (1–10 years, default 3), and when a backfill hits its page cap before reaching that depth the panel reports the **truncation** explicitly ("older filings may be missing") — never a silent gap. While a sweep drains, the status line shows the live extraction counter ("Extracting… {done}/{total}").

**Automatic, deterministic extraction; PDFs are for humans, not fact-scraping** ([ADR 0086](adr/0086-aggregator-primary-fundamentals.md), `v0.59.0`; positional tier retired [ADR 0095](adr/0095-retire-html-positional-tier.md)). Core KPIs arrive on their own: the **BiznesRadar aggregator is the primary source for periods no tagged issuer filing covers**, pulled daily per tracked company (every period column its statement pages carry). **Where a tagged filing exists, the issuer package is the breadth source, not a corroborator** ([ADR 0100](adr/0100-two-layer-tagged-fact-capture-and-ifrs-vocabulary.md) amends ADR 0086, epic #398): full capture reads every tagged fact an **ESEF/iXBRL** annual report discloses — around 400 per report against the aggregator's ~20–30 — so for those periods the filing supplies the figures and the aggregator corroborates them. The precedence ladder is unchanged (`esef` already outranks `html_aggregator`); only the stated posture moves. The **ESPI cover-note "wybrane dane finansowe" (WDF)** table lifted from the ingested komunikat remains the interim corroborator (the `structured_xhtml` tier value survives only on historical rows — no live writer produces it). The **PDF fact-extraction arm is retired**, and so is the **positional (visual-render) parser** that used to also read visual-only interims — no machine scrapes financial values out of statement PDFs or bare pdf2htmlEX renders anymore; some GPW issuers (CD PROJEKT among them) publish interims only as such a visual XHTML render, and those now arrive, if at all, only from the BiznesRadar-primary pull or an MCP agent's manual read, never a bespoke layout parser. Stored files stay on disk for the investor to read, and PDF text still serves only document period derivation and insider/ownership parsing. Precedence is per slot (`manual` > `esef` > `espi_cover_note` > `html_aggregator`): a strictly higher tier upgrades a lower tier's slot, the aggregator only ever overwrites its own, and **manual values are never touched** by any automatic path. Where an issuer tier holds a value and the aggregator disagrees, the disagreement is recorded informationally and never overwrites the issuer figure. When no tier can place a document, the app **says so** — the period is flagged with a typed reason, never guessed and never silently absent. Coverage numbers therefore describe deterministic coverage only.

*Planned — [ADR 0098](adr/0098-mcp-native-kpi-acquisition-lifecycle.md) (epics #352–#356):* acquisition becomes **agent-first** — an external agent (BYOA over MCP) reads each issuer report through a durable, resumable ingest run (staging → deterministic validation → atomic commit with provenance and citations); BiznesRadar's role moves from daily primary to witness/complement per slot on completion of that workflow. Until then the ladder above is the live behavior.
...
Global search (delivered in `v0.38.0`) lets the user find anything stored locally from one search box in the top toolbar — companies, watchlists, feed items, notes, and company events — with ranked results grouped by type and a snippet, and navigation to the matching item. Search is local-only and covers the user's own content.

Automatic local backups (delivered in `v0.38.0`) keep recent copies of the local data so it can be recovered. The app takes a safety copy before any data-structure upgrade and keeps a rotating set of recent backups; if a safety copy cannot be written, the upgrade is stopped. Backups and restore live in Settings › Data storage (a product feature, never behind Developer mode — owner 2026-09-08, #451); restore asks for explicit confirmation and is applied when the app restarts. A chosen backup is verified before it is ever applied — a corrupt or unrecognized file is refused with the current data left exactly as it was, reported the next time the app opens (#319). Backups stay on the machine and never contain stored API keys. A broader full app-data bundle and hosted data services remain later design discussions.
