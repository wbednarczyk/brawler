...
| **notable** | Stream + a **transient** toast at most | Fired price alert (`price_enters_range`, `price_week52_low`); autopilot-completion event (`autopilot_run_completed`); an autopilot **run** that ended `failed` or `partial`; signal categories `dividend`, `own_shares`, `general_meeting`, `significant_contract`, `guidance_change`, `major_holdings_change`, `short_position_change`, `fund_exit`, `recommendation_change`, `report_delay`, `score_deterioration`; and any unknown/unclassified signal category (never silently downgraded to routine) |
