...
### v0.61.0: Cross-company comparison + comparative valuation L1 (epics b7a54ba + fabd80f; ADR 0089)

Intent: turn per-company fundamentals into relative analysis — the real Compare mode (side-by-side KPI tables + multi-series trends with period alignment, unit/currency normalization, every value evidence-linked) plus level-1 comparative valuation (peer-multiples fair value, sector percentiles, a deterministic confidence grade, football-field readout), GPW-honest about thin peer sets.

Delivered: A0 real-data pre-validation spike on the owner DB (annual-dominant corpus, thin-is-the-norm sector histogram, 100% PLN, hand-labeled alignment ground truth reproduced 3/3 exact; found the EPS `currency='shares'` mislabel, card 47a21f1); NBP FX substrate (migration 0115 `fx_rates`, Table-A adapter with ≤90-day chunking + 404-tolerant trading-day gaps, `fx_daily_pull` on the market-data lane, flow=period-average / stock=last-mid≤end conversion, typed `MissingRate`); `get_kpi_comparison` read model (aligned axis, PLN conversion + `fxBasis`, server-side QoQ/YoY with the documented `delta_undefined` rule, typed per-cell gaps; `currency_unknown` correctly scoped to monetary/per-share kinds); the **Porównaj** mode restored under Dashboard — owner-amended mid-milestone to **Profil-as-default** (all KPIs × companies at one period, Różnica column for pairs, period selector) with Trend (one KPI × periods, 4-series `MultiLineChart` cap) as the toggle and **no submit button** (reactive compute); the Fundamentals "Pozycje × okresy" table (N=1 case, QoQ/YoY inline); sector percentiles (mid-rank inclusive method, thin N<4, typed `no_sector`/`insufficient_peers` absences); the `valuation` slice (peer P25/median/P75 × driver per multiple with a real net-debt EV bridge, migration 0116 `valuation_runs` with signature-gated appends + domain-date ordering, confidence grade A–D from four inspectable components) and the Compare valuation section (new `RangeBarChart` primitive, percentile chips with honest N, grade breakdown, thin-state honesty); 4 new MCP tools (3 read + 1 act with `WritesDisabled` negative path; frozen count 100 consistent at both sites); stale v0.61-MCP references cleaned and a **real v0.60 gap fixed en route — QualityPanel never rendered MCP-written qualitative verdicts** (capability was user-unreachable; fixed RED-first with a standing guard). Owner dogfooding along the way fixed the Diagnostics reconciliation chip overlap (scoped grid + bounding-box guardrail spec + ui-authoring track rule, card 228762e) and raw-English KPI names (`localizedKpiLabel` rule + pl-locale tests). Closure: wiki `cross-company-comparison.md`, J6 journey updated + `J6-compare` E2E with budget (3 interactions measured), for-the-author refresh, retro `docs/retros/v0.61-compare-valuation.md`.

Verification: 12 subagent slices (2 lower-tier implementers max, orchestrator quality-gating each with independent re-runs — catching the B1 second frozen-count site and auditing B4's gate freshness by mtime), RED-first with pasted evidence throughout; two full `make check` runs green (epic A and B, both `MAKE_CHECK_EXIT=0` quoted from logs), `make check-epic` + coverage ratchet green at closure, Windows cross-build green; live-verified twice on the owner's real Windows app (Profil/Trend/valuation-thin/reconciliation screenshots read); spec-conformance audit vs ADR 0089: 7× conforms, 1 partial (`ensure_fx_backfilled` uncalled in production — daily job covers backfill; card ad27483). Known-honest residuals: cards 47a21f1 (EPS currency relabel + ISO guardrail), 7c3e3cd (flaky v055 live-spec locator), ad27483, 2423f27 (MCP count single source); valuation DTO currency field deferred to the first non-PLN adapter.

### v0.60.0: Today re-invention + MCP surface v2 (epics 4f6ec2c + dc04eef; ADR 0087 + ADR 0088; ADR 0081 pilot)

Intent: (1) rebuild the Today attention home to journey J1 — grouped/deduped/severity-ranked stream, typed three-level severity mapped in one backend place, toast-as-pointer policy, the briefing translation seam cut on the autopilot token pattern; (2) make the MCP port a UI-parity surface — registry-driven tools over the typed command layer with capability tiers and provenance-mandatory writes.

Delivered: the full ADR 0081 loop (approved experience contract + storyboard + mockup; two live checkpoints on the owner's real DB — the first caught a P1 stale-urgent wall, fixed same-milestone with severity aging (72h) + systemic-cause cross-company aggregates + group Dismiss-all); briefing seam with typed payloads + dedup + class-guardrail scan; 96 MCP tools (41 read incl. facts-with-provenance / 55 act behind `mcpWritesEnabled` default-OFF with typed `writes_disabled`/`provenance_required` refusals; permanent denylist; classification gate = bijection with invoke_handler); `set_qualitative_verdicts` + unclassified-filings triage pair (`classified_by='agent'`, migration 0113); wiki+`brawler-mcp` skill with a generated catalog + drift gate; MCP dogfooding ritual (institutionalized in testing.md) run live — harvested 3 skill defects + card 670946e; three owner dogfooding rounds delivering evidence-specific row copy (fire-time `evidence_title` snapshots, migration 0114; report-number fallback for the registry's empty titles, card e5e9f9e), filename-as-metadata rendering with document links, per-company chip units, in-app severity legend, alert-origin indicator, and the attention Archive view; hard gates: anti-filename assertion, orphaned-evidence/pruned-feed overlays, live-checkpoint specificity probe, and the standing "UI dogfooding finding ⇒ overlay" rule. J1 budget: 6/15 interactions. Roadmap arc shifted −1 (comparison v0.61 … import/export v0.68). Known-honest residuals: 27 legacy orphaned attention events render category-only (source data pruned pre-0114); GPW registry listing titles empty (card e5e9f9e); live-suite network-spec timing flake (card filed).

### v0.58.0: Analyst recommendations + espi-wdf spike + retention disable (epic b4afbf9, ADR 0073; spike bdda6cf, ADR 0061 amendment)

Intent: track sell-side recommendations strictly as attributed third-party opinions with an append-only revision history (which cannot be backfilled — the earlier ingestion starts, the richer it gets), surfaced as a typed signal and a "vs target" context beside market data; plus a ground-truth-first verdict on the ESPI "Wybrane dane finansowe" cover table as a deterministic facts source.

Delivered: migration 0100 (`analyst_recommendations` append-only entity with natural-key dedupe, `recommendation_change` signal category seed, adapter catalog rows); `storage/analyst_recommendations` (direction derivation vs the same firm's prior entry — initiate/upgrade/downgrade/reiterate via a rating-rank map, target-direction fallback; per-new-entry feed item + signal + `evaluate_signal_rules` + `record_source_outcome`); `biznesradar_recommendations` adapter (live-probed `/rekomendacje-spolki/<slug>`, 301 ticker→slug like ownership, robots-clean; verbatim rating spans, Polish datetimes, Autor→analyst+firm split, broker-PDF absolutization; landing→clean empty, malformed→Parse; insta golden on a faithful CDR sample; free-page depth honesty: ~3–5 newest entries, full history is BR Plus — recorded in source-strategy with the new **live-probe rule** after the v0.53 research proved stale on both URL path and depth); `get_analyst_recommendations` + full IPC chain (mock runtime, fidelity-corpus step both replayers, byte-pinned populated wire fixture); opt-in cockpit panel per owner-approved storyboard (verbatim chips in fixed slots, toned direction sub-labels, %-vs-close, attribution-inseparable-from-number, honesty footer) + "vs target" readout in Price context with a focus seam; signal frontend (category union, tone, label = seed display_name, Alerts preset, en+pl). **First real run**: 93 recommendations / 41 of 49 companies ingested from live BiznesRadar into a copy of the owner DB (93 entries = 93 signals = 93 feed items, `last_success_at` set), via a durable env-gated live smoke. **Espi-wdf spike (ADOPTED)**: 15-doc/347-fact hand-labeled ground truth (pass-1 agent transcription, pass-2 orchestrator value-by-value verification incl. EUR-rate cross-checks and a ×1000 COGNOR share-count correction); structural parser measured **recall+precision 347/347, 0 false values**, 33 ambiguous digit-runs all resolved by the form's own PLN↔EUR rate → `SourceTier::EspiCoverNote` planned below `StructuredXhtml` (ADR 0061 tier 2a amendment; implementation card `76a4636` — ingest-time extraction mandatory). **Feed auto-retention disabled** (owner decision mid-session after the 30-day auto-prune deleted ~3,900 items incl. 448/451 WDF carrier bodies): timer removed with a red-first fake-timer regression test, manual "Clean up feed now" in Settings, truthful cleanup section; future mechanism card `cc674d4`. Today-view revamp study filed from dogfooding evidence (card `abd456e`). Wiki: new `analyst-recommendations.md` + index entry.

Verification: TDD contracts with pasted red evidence across 4 subagent slices (2 Opus implementers, Fable orchestrating + independently re-running every suite; concurrent builds serialized via a shared flock); per-phase `make check-fast`, full `make check` green (EXIT=0 evidence), `make check-epic` at closure; spec-conformance audit of ADR 0073 decision-by-decision (D1–D4 conform with live-path evidence, D5 deliberately not built); owner dogfooding approved the live panel; a latent v0.56 non-idempotence in `canonical_holder_key` (Unicode decomposing uppercase, 'ǰ') surfaced by its own proptest mid-gate and was fixed with the counterexample pinned.

Guardrails harvested: live-DB reads must go through copy+checkpoint — `?immutable=1` skips the WAL (testing.md + realdata README); no timer/background path deletes user data (product-spec canonical + DoD §C line + regression test); adapter-count assertions derived from `REGISTRY` instead of hand-counted constants (schema/source_registry tests); adapter contracts written only against freshly live-probed source facts (source-strategy).

### v0.57.0: Company health (epic 318d025, ADR 0083 as amended 2026-07-17/18)

Intent: the app raises "something smells here" per company on its own — published health formulas (Piotroski F + Altman Z″ EM; Beneish deferred) over confirmed facts, a parsed insider substrate feeding skin-in-the-game + sentiment, and an alertable red-flags panel.

Delivered: `fundamentals/health` deterministic score computer (strict completeness with typed per-component breakdowns; `not_applicable` via `statement_type`; scalars injected into scorecard criteria — scoped ADR 0046 amendment; F5 = non-current liabilities per T3 evidence) + extraction of 4 balance-sheet concepts (migration 0089) + health-facts re-extraction backfill; **T3 real-data gate** vs BiznesRadar (published EM = 3.25 + our Z″, confirmed to ±0.12; financial issuers mapped via migrations 0095/0098 — KRU owner decision); insider substrate: corrected `insider_transaction` classifier seeds (0/22→22/22 real-title corpus test), MAR art. 19 cover-note parser (GT 22 filings/30 tx: person 96.7%/100%, direction 95.8%/100%, inline figures 100% precision) + attachment-PDF tier (migration 0094, fill-NULLs-only merge, conflicts→diagnostics), `management_holdings` section parser (GT 15 docs/67 rows: person 95.8%, role 91.3%; corpus junk-rate 0 after plausibility gate; VRC glyph→PDF-sibling) + `stamp_founder_insiders` join (person OR indirect-via vehicle; badge from corroboration read, dictionary labels preserved); insider sentiment view (90d/12m count-based net, min-2-tx, undetermined bucket); red-flags panel (owner-approved mockup; migration 0092: `red_flag_acks` + 3 signal categories + internal `brawler-red-flags` adapter; detections: report_delay 3d grace, fund_exit basis-vanish + ESPI <5% crossing with active-window dedup, score_deterioration F≥2/band-downgrade; auditor+short_spike composed at read; KNF raise pattern → existing alert rules); ownership OCR residuals (card 5171372: tier-4 vision over `ownership_ocr_proposals` migration 0093, confirm-before-apply, PDF-sibling targeting for xhtml residuals, bulk scope table_unparsable+glyph); **live-found fixes**: automatic report-history backfill catch-up (33/50 companies had zero periodic coverage — startup + post-refresh, ADR 0077 trigger-parity amendment), alert hygiene (wall-clock `fired_at` + 14-day freshness gate at the ingest seam + repair migrations 0096/0097; toast cap 3 + „+N więcej" clear of the sidebar), toast/health-label localization (shared KPI-label mechanism, no raw backend tokens), CDR ratification (unit-scale precedence fix — bare `mln zł` never overrides a thousands declaration — + guarded repair migration 0099; 0 divergences across all companies after). Two big live passes on the owner's real Windows app (Z″ headlines live, 27 real flags, 220+ docs backfilled error-free). Wiki: quality-frameworks health section, ownership Insiders+OCR sections, new red-flags page.

Intent: make ownership a first-class, automatically gathered, time-aware entity — `ownership_stakes` snapshots with history (capital % and votes % separately), three ingestion streams (stored periodic reports, ESPI major-holdings notifications, aggregator pages), holder-type classification (dictionary → AI-with-confirm), derived free float, and an Ownership section in Basic info.

Delivered: append-only `ownership_stakes` + seeded `ownership_holder_dictionary` (migration 0082, deterministic ids, upsert-in-place per basis; classification-preserving COALESCE upsert); deterministic report extraction on the autopilot lane (migration 0083 residual queue: section_missing | table_unparsable | glyph_encoded; on-new-report + startup catch-up + backfill triggers; T1 spike gate: recall 90.2% / precision 100% on a 14-document hand-labeled ground truth) writing directly (owner decision) with the AI/OCR residual always confirm-gated (v0.57 card 5171372); `major_holdings_change` seeded signal category + conservative deterministic art. 69 stake updates (never guessed; `ownership_espi_unparsed` + diagnostics); holder classification: canonical key + heuristic markers + routable `ownership_holder_classification` AI proposals (migration 0084, confirm/reject, manual re-type always wins) and holder-identity merging (migration 0086 aliases, `HolderIdentityMap`, parenthetical-strip; treasury markers excluded); **mid-milestone pivot on real-data evidence (ADR 0072 amended, owner-approved)**: BiznesRadar "Akcjonariat" promoted from compare-only witness to the automatic breadth source — writes full-picture `aggregator` bases (single as_of from "Data aktualizacji", same-basis reconcile in one tx, table anchored on "Główni akcjonariusze", th-row/percentage-name/>100% row guards, Σ>102% implausibility gate + diagnostic), reports+ESPI demoted to depth/freshness, witness direction reversed (disclosed-only reference read), registry role witness→primary (migrations 0087 rename + 0088 garbage reset); disclosure-basis `current_state` (newest full picture wins, ESPI/manual overlay, baseline = {report_document, aggregator}); free float derived (+ per-basis `free_float_history`); UI: Ownership section in `CompanyBasicInfoPanel` (approved storyboard) with new `DonutChart` + `MultiLineChart` primitives (dataviz-validated palette, unknown/neutral kinds, SVG tooltip), honest source labels (BiznesRadar/ESPI/manual), classification CTA + proposal confirm/reject + re-type with undo; `get_ownership_overview` + classification commands with full mock-fidelity chain; J3 gains a zero-interaction ownership assertion; wiki page `ownership.md`.

Verification: TDD contracts with pasted red evidence across 5 subagent slices + pivot rounds (≤2 concurrent, disjoint files); per-phase `make check-fast`, `make check-epic` green pre-pivot and full gate re-run at closure; real-data validation before parse design (hand-labeled ground truth, orchestrator-owned; 2 labeling errors caught by parser-vs-label cross-check); four dogfooding rounds on the owner's real Windows app — r1 merged trajectories chart + inline legend, r2 unknown-slice + donut tooltip, r3 free-float series, r4 a live-caught aggregator-parser defect (th-"razem" summary rows ingested as holders with share counts as percentages on 49/49 companies; the sub-5% fund-statement table ingested wholesale) fixed same-day with red evidence reproducing the exact defect, repair migration, a real trimmed DBC sample, and a durable live spec asserting all rendered percentages ≤ 100 (post-fix live DB: 172 aggregator rows / 49 companies / 0 garbage).

Guardrails harvested: visual baselines — a small deliberate change below maxDiffPixelRatio needs `rm` before regeneration (engineering-workflow.md); ground-truth labeling from full section dumps with whole-document negative scans, orchestrator-owned labels (testing.md); classification-preserving upsert (COALESCE) pinned by test after review caught the daily-refresh wipe; aggregator plausibility gates (row + basis) so the automaton can never write garbage stakes.

Owner decisions recorded: ownership lives as a Basic info section (not a new panel); deterministic parser writes directly, AI/OCR residual always confirms; BiznesRadar over Bankier as the aggregator (permissive robots, host independence); the breadth pivot itself (aggregator primary, reports depth) executed pre-release in this milestone; sub-5% "Pozostali akcjonariusze" fund-statement depth deliberately not ingested (roadmap Not-In-V1 candidate).

### v0.55.0: Source reliability & disclosure signals (epic ad9d24f, ADR 0069)

Intent: make the feed harder to fool and harder to starve — a real behavioral contract on the SourceAdapter port (`Fetcher`), the official GPW ESPI/EBI channel back as a reconciliation second witness closing the Bankier single-point-of-failure, the KNF short-selling register as a disclosure source with a typed signal + dashboard panel, and the auditor-opinion/going-concern red-flag signal.

Delivered: `Fetcher` trait **at the refresh level** (ADR 0069 amended 2026-07-15, owner-approved — no unified item type; `RefreshContext`/`RefreshOutcome`, `joins_full_refresh`); all seven active adapters migrated strangler-style, `RefreshBehavior` collapsed to `Fetcher | Disabled`, sweep membership pinned by test, twelvedata residue removed. ESPI witness (`role: witness` on the descriptor, visibility optional, migration 0081): pair ledger `source_reconciliation_results` (matched | bankier_only | espi_only; exact report-number match then company+date fallback; boundary-date excluded from bankier_only), **system attention events** (nullable `rule_id` + `trigger_type`, partial-unique dedup) routing espi_only through toast/Today/briefing, no-Inbox-leak tripwire test, Diagnostics ledger (`list_source_reconciliation`), Sources "Świadek" chip. KNF adapter `knf-short-selling` (stable public JSON endpoint — probe verdict on card 6204cd0; migration 0080): `short_positions` mirror + append-only `short_position_events`, one feed item + `short_position_change` signal per change firing v0.54 alert rules, empty-register guard (a transient fault must not read as a mass exit; same guard added for an empty witness listing), cockpit `shortPositions` panel (palette-only, approved mockup) with `list_short_positions` incl. `registerUpdatedAt`. `auditor_opinion` category (migration 0079, PL phrasings incl. "odmowa wyrażenia opinii", "kontynuacja działalności") with danger badge, alert preset, AI-fallback untouched. Wiki page `source-reliability-and-disclosure-signals.md`.

Verification: TDD contracts with pasted red evidence in all 5 subagent slices (≤2 concurrent, disjoint files); per-phase `make check-fast`, `make check-epic` green at closure (real `EXIT=0`); spec-conformance audit of ADR 0069 decision-by-decision with live-path callers (4/4 conform); real-data validation of the auditor rules (523 titles, 0 false positives; classifier is title-only); live-verified end-to-end on the owner's real Windows app — KNF 25 entries/12 matched + idempotent re-poll, witness first measured completeness run (15 items / 5 matched / 0 espi_only), panel opened from the palette on a real company with real Marshall Wace data and the register timestamp.

Guardrails harvested: a new refresh/ingest path must call `record_source_outcome` + test (`last_success_at`) — the witness ran fine while Sources showed "never refreshed", caught **only** by live-verify (DoD §C line); negative-zero formatting normalized in `formatFixedDecimal` with tests; bankier_only boundary-date exclusion + empty-snapshot guards (both sources); live-spec lessons (panel-open evidence is the dockview tab, not text presence; always look at the PNG); ADR 0063 byte budget enforced by trimming, not raising.

Owner decisions recorded: Fetcher at the refresh level (variant A) with a marker to re-evaluate a shared disclosure item shape at the third disclosure source; espi_only surfacing through attention routing, not diagnostics-only; a dedicated dashboard panel for the KNF register (palette); card ee81afe (free degraded quote fallback) detached from v0.55.

### v0.54.0: Attention routing + morning briefing (epic 5bc40a8, ADR 0068)

Intent: give the app an attention layer — it tells the user what deserves a look instead of making them scan feeds: user-owned alert rules over typed signals / autopilot completions / price conditions, persistent in-app toasts with click-through to evidence, a Today attention list, and a cited "what changed + what needs doing" morning briefing. In-app only; the boundary is shaped for later OS adapters.

Delivered: `alert_rules` + `attention_events` (migration 0077; content-derived rule ids, UNIQUE evidence dedup, per-rule daily throttle) with inline evaluation at the three ingestion hooks (signal classification, autopilot `finalize_notify`, post-daily-pull price check — no new worker lane); the `Toast` persistent variant (explicit dismiss, click-through action, MAX_STACK-exempt, `role="alert"`) on the existing ADR 0076 transient primitive; 8 attention commands + 2 briefing commands (DTOs, mock runtime, fidelity corpus journeys); the **Alerts Library screen** (own sidebar destination + Ctrl+9/⌘K entry — owner IA decision mid-milestone, moved out of Settings; visual redesign to the approved mockup: three cards, preset chips with lucide icons, two-step creation, live plain-language preview sentence, fixed trailing slots); Today: 5th "attention" stream category with per-company grouping, seen/dismiss, evidence click-through + session-deduped persistent toasts; **morning briefing**: deterministic composer (new signals / autopilot runs / claims due / upcoming reports / fired alerts, domain-date ordered, strict `since` boundary, per-day idempotent) persisted to `morning_briefings`/`morning_briefing_items` (migration 0078, digest-style citation keys), optional AI narrative via the new routable `MorningBriefing` capability with citation-integrity validation (uncited narrative rejected, never stored) and a no-provider structured-list fallback, Today card with generate-on-demand + once-daily auto trigger; toast adoption sweep (manual source refresh, import apply, digest/brief start) with a documented toast-vs-inline boundary rule; J1 journey gains the briefing read step; wiki page `attention-and-briefing.md`.

Verification: TDD per slice with red evidence in every subagent contract (7 slices, ≤2 concurrent); inter-phase gates on `make check-fast` (new owner rule), full gate + Playwright matrix green at epic end (525 browser tests incl. the new narrow-window Alerts assertion); live-verified repeatedly on the owner's real Windows app via `make live-cycle` — real briefing composed from real WZA/report items, since-boundary "no news" on regenerate, rule creation on the real watchlist, undo-toast delete; a **live-caught regression** (COUNT-based rule ids collided after delete → `UNIQUE constraint failed`) fixed same-day with content-derived ids + typed `DuplicateAlertRule` (backend + mock parity + UI message, 3 regression tests).

Owner decisions recorded: alerts are a Library view, not a Settings section; `docs/plans/` + `docs/mockups/` are gitignored working artifacts; inter-phase gate = check-fast, browser matrix once at epic end, and no manual full gate when the pre-push hook will run it; mocks are never completion evidence — every feature checked live before "done".

Retrospective digest: the mockup-first rule existed but wasn't in the UI subagent contracts — the owner caught an ugly screen; harvested as a DoD §B line ("approved mockup in docs/mockups/ first; goes in every UI subagent brief"). WCAG light-palette contrast failed twice for the same reason (`--primary` small text on tinted surfaces — toast action, then trigger chips); harvested as a ui-authoring styling rule. Piped make runs masked two red exits (`| tail` swallowed a failed cross-build and a failed gate; one stale exe nearly passed for new code) — harvested into memory: `set -o pipefail`, verify the bundle hash, never filter background gate output at capture time. Global overlays (toasts) polluted every visual baseline; the visual harness now hides the toast viewport per shot.

Owner decisions still-open: none for this epic; polish items (duplicate-message copy, palette entry) were closed in-milestone.

### v0.53.0: Market data foundation (epic e189379, ADR 0067/0082) + Dashboard redesign (epic c793ca1) + UX quality loop v2 pilot (bd1a6af, ADR 0081)

Intent: give every tracked company an automatic price picture beside its fundamentals — daily EOD quotes with full-history backfill, registry-sourced sectors, market cap and level-0 valuation ratios via the derived-metrics engine — as an automaton (no manual import), local, decision-support only. Plus the dashboard redesign (one company-scoped Dashboard with follow presets) and the UX-quality-loop v2 pilot, already on the same branch.

Delivered: `daily_quotes` + sector columns (migrations 0071–0076); automatic full-history backfill on company add + a self-healing post-session daily pull (Yahoo v8, GPW `.WA`, PLN); registry-sourced sector classification with manual override (type-to-filter suggestions, case-fold dedup of the GPW/NewConnect taxonomies); 12 canonical level-0 `kpi_definitions` (close, 52w high/low, valuation percentile, market cap, P/E, P/BV, EV/EBITDA, dividend/FCF yield, distance-from-52w) computed over the latest close × confirmed facts with a new `coalesce()` DSL fallback chain ("compute from whichever inputs exist"); the `get_price_context` read model + `PriceContextSection` (close/change, 52w range, market cap, ratios, **candlestick chart** with a round-number nice-scale) leading the reordered Fundamentals panel; a new read-only **Basic info** panel (`get_company_basic_info`) with edit behind one toggle. Source pivot (ADR 0082): Stooq's automated CSV gained an anti-bot PoW gate and BiznesRadar's history path is robots-disallowed → **Yahoo primary**; **Twelve Data was implemented then removed the same day** when a live smoke proved GPW `time_series` is paid-plan-only (free tier = metadata only) — the adapter, descriptor, credential form, and seeded row were cut (migration 0076); a free degraded fallback is card `ee81afe` (v0.55). Also shipped: the Dashboard unification (Companies→Library, follow presets) and the Q0–Q9 UX-quality-loop pilot (interaction contracts, visual contact sheets, live UX checkpoints, escaped-defect taxonomy).

Verification: `make check` + `make check-epic` green (closure gate + coverage ratchet); spec-conformance audit of ADR 0067/0082 decision-by-decision with live-path callers (daily pull ← scheduler, backfill ← create_company, price context/basic info/sector commands ← registered + UI, all conform; TD "removed" recorded as a corrected decision); live-verified throughout on the owner's real Windows app via `make live-cycle` — refresh-without-lock, candlestick scale + full-width, Basic info edit toggle, sector propagation (751/771 registry entries, 50/50 tracked companies), ratio spot-checks (ABE C/Z 16.46; LPP market cap vs BiznesRadar). Guardrails harvested: `BEGIN IMMEDIATE` write txns (clippy `disallowed-methods`), percentile ≥20-session floor, presets→type-to-filter above ~12, every credential enterable in Settings (Rust `CREDENTIAL_PROVIDER_IDS` ↔ fixture ↔ frontend contract), fan-out integrator fixes shared inventory gates, automaton-no-manual-import.

Owner decisions recorded: Yahoo primary / Twelve Data secondary → the secondary removed after the paywall finding (no free fallback shipped; v0.55 owns the replacement); the app must be an automaton — no manual import ever; ratios compute from whichever inputs exist before going empty; the WSL disk problem solved by moving the distro to a dedicated partition (sparse-vhdx I/O penalty retired); **the mandatory gate split by git phase** — `make check-fast` per commit, full `make check` at pre-push-to-master ([ADR 0062](../adr/0062-mandatory-test-gate-and-test-driven-loop.md) amendment 2026-07-15), after per-commit full runs proved too costly.

Retrospective digest: source-probe premises must be validated with a real pull, not a metadata endpoint (Twelve Data's `/stocks` looked fine; `time_series` was paywalled — caught only by the live smoke, now a documented "smoke before you commit to a source" rule); a backend capability with no Settings entry point is invisible (the un-enterable TD key — harvested as the credential↔Settings contract); self-heal must key on history depth, not just latest-bar freshness (the 5-bar blind spot); cross-language mock fidelity drifts silently on the populated path unless a corpus journey exercises it (basic-info shares ordering — now pinned both sides).


### v0.52.0: Judgment capture (epic 3242a93, ADR 0071) + read-only MCP MVP (epic 829a2ea, ADR 0078) + embedding-model retirement (5215946, ADR 0080)

Intent: capture the owner's judgment before it can be backfilled — an append-only decision journal (buy/pass/keep-watching/sell-note + Markdown rationale + evidence links) and pre-report expectations that freeze when period facts confirm, reviewed as a factual expected-vs-actual mirror (never grades); plus NS1 slice 1 — a read-only MCP server so external AI assistants can read the research; plus exercising ADR 0035's reversibility — retire the never-winning e5-small embedding model and the dead subsystems around it. Scope grown at planning (2026-07-11) in lieu of renumbering: 9 backlog/deferred cards pulled in.

Delivered: decision_entries with DB-trigger immutability + supersede chains and report_expectations with freeze-in-transaction (migrations 0067/0068); 7 commands on the new `CommandError` envelope (ADR 0070 first delivery) with fidelity coverage; journal panel (per-company + all-companies) with evidence picker, expectations in the report-season card with expected-vs-actual review + resolution notes; journeys J4 re-baselined (10→13, recorded-expectations value) and J6 implementable at 11/15. MCP: hand-rolled JSON-RPC 2.0/Streamable-HTTP subset on tiny_http, 127.0.0.1-only, bearer token in the keychain (shown once, constant-time compare), 4 read-only tools (dossier, research search, claims due, quality assessment) with a frozen tools/list snapshot contract, Settings section with token lifecycle + client snippets, dumb-pipe stdio adapter shipped beside the portable exe. Retirement: −57 crates / exe −3.3 MB / ~−4300 LOC — embedding model + vector scaffold + write-only story_key path out (migrations 0069/0070, forward + tolerant reads), rule classifier and claim tracking untouched (claim-extraction pipeline parked, not removed). Cards: Inbox-0 live bug (c80dabe), template-framework relocalization (baba638), AI-locale prompt sweep (fd38d5a), cockpit axe debt (02e991e), compact pane headers (b97e349), raw quality value formatting (619c8d9), TodayScreen + SettingsScreen flakes (a91d260, b6b866f), orphaned KpiExtraction (db9a292), generic add-panel types (106f8a7).

Verification: TDD wave model (7 waves, ≤3 delegated slices, red evidence per contract); check-epic green with the coverage ratchet raised (77.5 frontend / 83.3 rust); spec-conformance audit of ADRs 0071/0078/0080 decision-by-decision with live-path callers — all conform; migrations verified against seeded old-state data with embeddings + story_key present, and live against the owner's real DB; MCP proven with curl, the stdio pipe, a nixpkgs Claude CLI, and finally the owner's own Claude session over WSL mirrored networking reading a live CDR dossier; two owner dogfooding rounds on the live Windows app — every finding fixed in-milestone (DateField primitive, P1 evidence-type dispatch arm + structural test, generic add-panel palette, decluttered palette + styled search); live suite green on the final build.

Owner decisions recorded: no renumbering — grow scope instead; import/export v2 + offsite backup moved to v0.67 (schema stability first); claim-extraction parked not removed; all dogfooding findings fixed in-milestone; closure artifacts (retro, guardrails, audits) must be presented inline in chat before commit.

Retrospective digest: removal ADRs should name symbols not files (entity_resolution half-alive near-miss); owner decisions in Radicle comments need same-day roadmap edits; hermeticity debt (env fallbacks, port clamps, proxy-await) surfaced under parallel load — all three closed with rules; disk-guard now watches /mnt/c after a host-C: outage; the P1 escaped because two exhaustive lists drifted — harvested as "one allowed-values list, one dispatcher, one structural test".
