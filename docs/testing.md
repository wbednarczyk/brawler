...
- **Browser storage is reset per test too** (harvest 2026-08-03). jsdom keeps ONE `localStorage` for a whole test *file*, so a component that persists state — the cockpit writes its live dockview geometry to `cockpit.layout.v2` on every layout change and restores it on mount — hands that state to the next test in the file. Whether the write lands before the test ends depends on machine timing, which is how `CockpitScreen.test.tsx`'s pin/unpin test failed twice in CI while passing on every local run. `src/test/setup.ts` now clears `localStorage`/`sessionStorage` in a global `beforeEach`. **A test must never rely on state left by an earlier test**; seed what it needs in its own body.
