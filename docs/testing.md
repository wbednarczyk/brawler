...
Proves the runtime OS credential backend can persist the MCP auth-token target. The ignored Rust test `live_keyring_persists_mcp_auth_token_secret` writes a temp secret to the real OS store, reads it back, clears it, and restores any pre-existing key. **Run it on the OS whose persistence you're validating** — a WSL run validates the WSL/Linux keyring, not the packaged Windows app's Windows Credential Manager. If the packaged Windows app can't persist credentials while WSL passes, add a Windows-runtime keyring validation path before closure.
