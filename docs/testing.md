...
- **Timeout budgets are set centrally and must not converge**: Testing Library's `asyncUtilTimeout` (5s, `src/test/setup.ts`) sits under vitest's `testTimeout` (15s, `vitest.config.ts`). Equal budgets turn a slow wait into a bare "timed out" instead of the assertion message naming what differed.
