...
- **English-in-PL detector** (G12, dogfooding #8 class — "Official report"
  shipped in English inside the PL UI; the retro F4c fix was instance-only and
  the class returned) — the same `translationCompleteness.test.ts` scans every
  `plText.ts` entry for a value byte-identical to its EN key (catches entries
  reached only through a dynamic key, e.g. `text(formatEvidenceType(x))`, which
  the static literal scan above can't see). A new identical entry fails unless
  listed in `identicalEntries.json` with a reason — for a genuine
  brand/acronym/proper noun or established domain loanword; a plainly-English
  label is a real defect, fix the PL text instead of allowlisting it. The
  allowlist is a ratchet: it also fails if it lists a key that is no longer
  identical.
