...
A landmark (`<section>`/`<aside>` **with an accessible name**, `role="region"`, `main`, `nav`) is a page-level navigation target, and axe requires each one's role + name to be **unique**. A panel hosted by the Spółka workshop tool bar is not the screen root, so a landmark inside a panel body would collide with the screen's own landmark and with any other panel reusing the same shared component (issue #142).
...
- **A scrollable, Tab-reachable card is an `<article aria-label>`**, never `role="group"` + `tabIndex=0` (axe's `scrollable-region-focusable` wants it focusable, and `focus-order-semantics` then wants a scrollable-region role) and never a named `<section>` (that is a landmark — see above). `article` passes both rules without adding a landmark (the five Spółka core cards; guarded by the `tests/browser/spolka-keyboard.spec.ts` axe pass, F3c).
...
- **Several same-level destinations in one bar = an APG toolbar** (`role="toolbar"`, roving tabindex through `useRovingToolbar` in `src/shared/focus/`): one Tab stop that follows the last-focused entry while the bar has focus, and moves to the pressed entry when the selection changes while the bar is unfocused (a tool opened by `H`/`L` or the palette makes Tab-from-the-tool land on its own entry); `←`/`→` wrap, `Home`/`End`, the `Button`'s own `Enter`/`Space`. Never N Tab stops of plain buttons. The Spółka workshop bar is the reference (`WorkshopBar`, `SpolkaScreen.tsx`).
- **A keyboard-driven screen change moves focus by an explicit intent**, never by guessing from `document.activeElement`: ToolHost records `focusIntent` (`heading` on open — authoritative, no yield to whatever is focused inside the frame; `entry` on close; `overview`; `company` only for the adjacent-company shortcuts; `none` for a pointer/palette company open) with every commit and exactly one owner consumes each intent. **Focus never lands on `<body>`**: `focusScreenHeadingIfBody()` (`src/shared/focus/`) is the fallback after a palette command and after a `none` intent — the active screen's first heading gets `tabindex=-1` and focus (a non-input, so Chromium paints the ring only for keyboard users). JSX `autoFocus` is banned app-wide (`src/ui/noAutoFocus.test.ts`) — initial focus is always an explicit ref/intent.
...
- **Typography (ADR 0104 dec. 2, F0.5; amended owner dogfooding 2026-08-27):** both faces ship as committed variable-woff2 subsets in `src/assets/fonts/` + `src/styles/fonts.css` — never load a font from a CDN or add a font npm dependency. Sans is the root `font-family` (Schibsted Grotesk, `var(--font-ui)`); mono is **always `var(--font-mono)`** (JetBrains Mono) — no hand-rolled mono stacks. **Figures, dates, and percentages render in the UI face with `font-variant-numeric: lining-nums`, never mono** — JetBrains Mono spaces punctuation ("15 , 2 mld PLN"), which reads as broken. `.num-tabular` sets this explicitly (`font-family: var(--font-ui); font-variant-numeric: lining-nums`), so any element carrying it is immune to an inherited mono ancestor. Mono stays reserved for identifiers, keys, period ids, eyebrows, and filenames.
...
- **Never alpha-fade small/muted text toward transparent** (`color: color-mix(in srgb, var(--muted) N%, transparent)`): `--muted` sits near the 4.5:1 floor by design, so any fade lands below WCAG AA on light — and under a gradient/tinted ancestor axe cannot compute the background, hiding the violation until the background flattens (F0.5 harvest: `.nav-group-label` at 3.77:1). Quiet text = `var(--muted)`, full stop; quieter than that is a design smell. (Fading `--text` with a wide margin, decorative icons, and the input placeholder are outside this rule.)
- **Small text colored `var(--primary)` on a tinted/raised surface fails WCAG AA in BOTH light palettes** (~4.2–4.4:1 vs the required 4.5; the axe gate reddens it) — accent-colored small text (chips, toast actions, bolded preview words) ships with a `:root[data-theme="light"] … { color: var(--primary-strong); }` override; dark themes keep `--primary` (there the lighter tint has the higher contrast). Recurred twice on 2026-07-15 (`.ui-toast-action`, `.alerts-trigger-chip-active`) and a third time 2026-08-27 (`.spolka-provenance-ticket`, owner dogfooding wave 2) — a candidate for an automated stylelint check if it recurs again.
...
mark the panel's leading `SectionHeader`/`PanelHeader` with `paneLead`; the shared `.spolka-layout
...
The cross-cutting infrastructure (U7) is in place: `.spolka-layout` and `.workspace` are named `pane` **size** containers (`container: pane / size`), so a panel's CSS reacts to the *pane's* size, not the window's. To implement a row:
...
3. **`setPaneSize`/`resetPaneSize`** (`tests/browser/helpers/harness.ts`) force a pane's inline size directly (the sanctioned approach) so the query fires regardless of the real tool-host cell. Return the pane via `page.locator(".spolka-layout")` (or `.workspace` for a standalone screen) from `open`.
...
- **English-in-PL detector** (G12, dogfooding #8 class — "Official report" shipped in English inside the PL UI; the retro F4c fix was instance-only and the class returned) — the same `translationCompleteness.test.ts` scans every `plText.ts` entry for a value byte-identical to its EN key (catches entries reached only through a dynamic key, e.g. `text(formatEvidenceType(x))`, which the static literal scan above can't see). A new identical entry fails unless listed in `identicalEntries.json` with a reason — for a genuine brand/acronym/proper noun or established domain loanword; a plainly-English label is a real defect, fix the PL text instead of allowlisting it. The allowlist is a ratchet: it also fails if it lists a key that is no longer identical.
...
- **Test hygiene lints** (G10, owner-approved hard gate 2026-09-07) — `eslint.config.js` bans a committed `.only()` (`it.only`/`test.only`/`describe.only`/`test.describe.only`) across `src/**/*.test.{ts,tsx}`, `src/test/**`, and `tests/**/*.ts` (added to `npm run lint`'s glob), and requires a documented reason on every `.skip()`/`.fixme()`/`.todo()` — a `// skip-ok: <reason>` comment for Vitest (no reason slot in `it.skip(name, fn)`), the string last argument for Playwright's `test.skip(condition, "reason")`. `src/test/testHygiene.test.ts` additionally scans every `it()`/`test()` in `src/**/*.test.{ts,tsx}` for an `expect*`/`assert*` call, exempting a title ending "renders"/"does not throw" or a preceding `// no-assert-ok: <reason>` comment; new offenders redden, `src/test/testHygiene.baseline.json` only shrinks (coverage-baseline pattern).
- **Paint, not attribute** (G11, dogfooding #11 — `data-document-highlighted` was set for 4s with no CSS rule ever painting it, and the browser spec that asserted the attribute stayed green) — `eslint.config.js` bans `toHaveAttribute("data-*-highlighted|-selected|-active|-marked", …)` in `tests/**/*.ts` (Playwright only; jsdom `src/**/*.test.*` computes no colors, so the equivalent Vitest attribute assertions stay legitimate per ADR 0045). Use `tests/browser/helpers/paint.ts`'s `expectVisiblyMarked`/`expectOpaqueSticky`/`expectInsideScroller` instead, which assert the rendered computed style (background/outline/box-shadow, sticky opacity/z-index, an action's position inside its scroller's `clientWidth`) rather than a DOM attribute or position.
