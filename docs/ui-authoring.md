...
- **Typography (ADR 0104 dec. 2, F0.5):** both faces ship as committed variable-woff2 subsets in `src/assets/fonts/` + `src/styles/fonts.css` — never load a font from a CDN or add a font npm dependency. Sans is the root `font-family` (Schibsted Grotesk); mono is **always `var(--font-mono)`** (JetBrains Mono) — no hand-rolled mono stacks. `.num-tabular` is digit *alignment* only; putting a figure in the mono face is a per-screen migration decision, not automatic.
