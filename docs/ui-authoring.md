...
Primitive-first authoring is policy ([ADR 0037](adr/0037-ui-component-framework-and-authoring-contract.md)), not a style preference. Enforcement runs in the `check:frontend` gate (`typecheck → lint → test → build`) and is deliberately **non-restrictive** — it catches regressions without blocking legitimate natives or legitimately-English strings:
