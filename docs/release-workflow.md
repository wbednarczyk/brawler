...
The human/agent role shrinks to: **the right `release:*` label on the PR** — the agent creating the PR sets the proposed label matching the change scope, the owner confirms or overrides at merge (merging stays owner-only) — plus **`make sync-rad`** when convenient. Everything else is automatic.
