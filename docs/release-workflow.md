...
- **`CHANGELOG.md` stays the canon, written by the bot.** After a successful tag, the release job generates the `previous-tag..tag` entry with git-cliff and commits it **together with the version stamp** (`chore(release): vX.Y.Z [skip ci]`, the one narrow `github-actions` ruleset bypass; `[skip ci]` prevents a loop; each push attempt regenerates the commit on a fresh `origin/master`, so merge races cannot conflict). The same text is the GitHub Release body. The stamp/changelog commit for X naturally lands one commit after tag X — cosmetic, by design.
