...
**Amendment 2026-09-07 (owner):** `AGENTS.md` stays a ≤ 1 KB stub but now also carries the Codex-peer context pointer (astra/sol under the `astra-peer` user skill read it first: CLAUDE.md → engineering-workflow → `investing-domain`); the session hook and CLAUDE.md § Required Reading name the `investing-domain` skill as mandatory for any product/UI/analysis/planning work. Budgets unchanged (hook 1.7 KB / 2.5 KB, CLAUDE.md 18.9 KB / 20 KB). Same change (context-hygiene D1/D2/D6/D7, epic #470): rules that had drifted into 2–3 separately maintained versions (primitive-first, mockup-first, i18n/gallery, test-that-reddens, real-data validation, ADR 0049 obligations, resource discipline, coverage floors, guardrail harvest, spec-first) are now one full statement in their home doc + pointers elsewhere; the `make check`-before-PR vs never-locally contradiction and the fidelity-corpus headless/MCP exemption are fixed at the rule site; retired sections inside contracts/data-model/IA are one pointer line each (anti-archaeology). Remaining D3–D5 (task router, per-task budgets, generated reference) are the epic.
