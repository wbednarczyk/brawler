...
2. **Per-capability disposition** (input read → fate):
   ...
   | `event_date` / `signal_classification` (AI fallbacks, default OFF) | filing title+body | **removed**; deterministic rule classifier + `signal_dates` stay; residual unknowns land in an explicit unclassified bucket (future MCP triage tool, `v0.61.0`) |
   | `qualitative_assessment` | report excerpts + transcripts + typed | **removed**; criteria verdicts are manual now, agent-written with provenance via MCP write-tools later (`v0.61.0`) |
