...
3. ~~**Transcripts stay**~~ — superseded by [ADR 0111](0111-retire-video-transcription.md) (2026-09-07, #463): video transcription is retired too; the app has no in-app AI. Original text: `VideoTranscriptProvider` trait + the Gemini implementation. Transcription is data acquisition (speech→text), not interpretation; it is the only Gemini/API-key use left. The trait remains the pluggability boundary for future engines (e.g. local Whisper).
