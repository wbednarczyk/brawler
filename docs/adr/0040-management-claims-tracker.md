...
Status: Accepted — **amended 2026-07-20 by [ADR 0084](0084-retire-in-app-ai-layer.md)**: AI claim extraction from report documents/transcripts is removed in `v0.59.0`; claims are created through the manual path (`create_management_claim`), with agent-proposed claims returning via MCP write-tools (`v0.61.0`). Claim entity, due-period derivation, verdicts, and KPI-backed verification are unchanged. **Amended 2026-09-10 by [ADR 0111](0111-retire-video-transcription.md)**: the `transcript_segment`/`transcript` source-evidence types are retired with video transcription.
