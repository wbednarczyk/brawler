...
In v0.36.0 the resolver is **user-triggered** (a "fetch report from IR page" action). Wiring it to fire automatically on report-event detection is the v0.47.0 autonomous report pipeline (task `05ebf07`/epic `9a607da`), behind the per-company trust ladder. v0.36.0 delivers the durable building block; v0.47.0 delivers the automation.
...
- The IR URL field and resolver are exactly the building blocks the autonomous pipeline (v0.47.0) needs, drawn before automation so the trust ladder composes them rather than retrofitting.
...
- **Manual-assist only** (open the IR page, user clicks the report): viable and lower-risk, but more clicks and no path to the v0.47.0 automation; kept as the graceful-degradation fallback rather than the primary mechanism.
...
Event-driven automation over this building block is deferred to v0.47.0 (epic `9a607da`).
