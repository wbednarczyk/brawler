...
**Amended (2026-08-19, issue #403):** `value_kind` was originally forced to `monetary` too, mistyping every proposed count metric at birth (proven live at epic #399 §G — a disclosed client COUNT would have asserted a false currency). `propose_kpi_definition` now takes an optional `valueKind ∈ {monetary, count}`, default `monetary` (additive, backward-compatible); any other value, including the wider `kpi_definitions.value_kind` vocabulary's `ratio`/`percentage`, is refused typed `invalid_input`.
