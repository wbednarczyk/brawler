...
6. **Claim-extraction pipeline: parked, untouched** (owner decision 2026-07-11). The AI report→claim-proposal pipeline (commands/job/storage/migration 0046) remains in the codebase awaiting a future UI slice; revisit at the NS1 conversational slice. Touching it in stream R is a tripwire. **Reversed 2026-07-20 by [ADR 0084](0084-retire-in-app-ai-layer.md):** the parked pipeline is removed in `v0.59.0` — the revisit happened and the answer is MCP write-tools (`v0.61.0`), not an in-app UI slice. The manual claims path stays live.
