...
1. **`docs-drift` gate** — a hard-fail `make check` step verifying, at minimum:
   ...
   - **MCP catalog + manifest coherence** (ADR 0088 dec. 5; registry-driven since #387): the generated tool-catalog span in `wiki/mcp-agent-guide.md` and `.claude/skills/brawler-mcp/SKILL.md` is compared byte-for-byte to a block regenerated from the `tools/list` snapshot. The read/act split comes from the **registry-truth capability manifest** (`entries()` → a committed insta snapshot carrying each tool's `tier` and acquisition membership), **never a tool-name regex** — the wire `tools/list` omits `tier`, so the manifest is the only registry-truth artifact JS can read. The gate also asserts the manifest, the full snapshot, and the acquisition-scoped snapshot stay mutually coherent (order + deep-equal projection), and that each doc's hand-authored acquisition-workflow inventory covers exactly the nine acquisition tools. Registry drift reddens the Rust golden (`capability_manifest_is_the_frozen_contract`) until `cargo insta accept`.
