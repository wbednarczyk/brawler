...
**Amended (2026-08-19, #404):** the processor is a fixed pool of 3 threads (owner decision), not one — a single processor thread let one stalled request body, or one synchronous heavy act, head-of-line-block every other `tools/call` behind it (issue #404 H3). All 3 threads share one `mpsc::Receiver` behind a mutex held only across `recv()`; concurrency safety across threads is delegated entirely to storage-level guards (leases, revision pins, IMMEDIATE transactions) — nothing in `mcp/server.rs` itself. The stalled-processor shutdown posture (above) now parks per-THREAD, not per-server: `STALLED_PROCESSOR_CAP` is recalculated from 4 to `4 × 3 = 12` to preserve the original 4-rotation tolerance (one rotation can now park up to 3 threads, one per stalled connection held open across the pool, instead of 1).
