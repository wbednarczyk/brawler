...
- **Auto-committing sample/mock output.** An auto-commit path must treat a **non-real** analysis provider (the `test_sample` provider) as "no provider" and degrade rather than confirm its placeholder output as facts — `autopilot` extraction does this and a guard test pins it (`a3643d7`). Any future auto-commit path (quality auto-eval, pipeline stages) must repeat this guard. Settings already exclude `test_sample` from the selectable providers; the guard is defense-in-depth for a value that bypassed validation (corrupt/imported settings).
- **Detection fires on a stale report.** Detection must rank "newest report per type" by the report's **domain date**, not `created_at` — after a history backfill, old reports carry a newer `created_at` (tracked: `d60305c`; rule in [data-model.md](../data-model.md#model-principles)).
- **A failed extraction is swallowed as a silent success.** `run_kpi_extraction_job` returns `Ok` even when it marks the job `failed` (e.g. a transient provider `UNAVAILABLE` 503), so the extract stage must inspect the job's **status**, not just count proposals — otherwise a run records `proposed: 0` and finalizes `succeeded`, permanently blocking re-detection with zero facts. The stage now fails on a `failed` job (guard test), and `create_run_if_absent` **self-heals**: a prior run that `failed` without producing any fact is deleted so the next refresh-driven detection sweep re-runs it (a `succeeded`/`partial` run, or a `failed` run that already committed facts, still dedups). See [ADR 0059](0059-worker-pools-and-queue-fairness.md) (found in the same real-DB validation).
