...
- **aggregation suffixes** already present in seeded formulas — `_ttm`, `_avg` — and the synthetic intermediates seeded formulas reference (`nopat`, `invested_capital`, `net_debt`, `ebitda`, `capital_employed`). Their resolution is defined explicitly: where a clean reported/derived definition exists it is a seeded `kpi_definitions` row; the small remainder are built-in resolvers documented alongside the engine.
