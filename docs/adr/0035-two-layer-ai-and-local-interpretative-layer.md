...
6. **Eval execution.** The similarity eval runs against the **real cached model** (downloaded once into the local/CI model cache), not committed precomputed vectors. To keep default `make check` offline and fast, the model-backed eval is an **opt-in/periodic** suite (credentialed/heavy tier, alongside coverage/mutants — not the deterministic per-commit gate, which now includes `knip`/Playwright per [ADR 0062](0062-mandatory-test-gate-and-test-driven-loop.md)) and **skips gracefully** when the model cache is absent — it never triggers a download from the per-change gate. See [engineering-workflow.md](../engineering-workflow.md).
