...
**Escape hatch `BRAWLER_GATE_ALREADY_GREEN=1`** skips the pre-push full check on a master push. It exists solely for `make release`, which runs `make check` itself immediately before pushing; without the hatch the identical tree would be gated twice back-to-back. It is a caller *vouching it already ran the gate*, not a way to weaken it — a plain `git push` (no env var) always runs the full gate. `make release` sets it for its own pushes.
