...
**Amended (2026-09-12, #511): the family registry is evidenced and bounded.** The first v2 measurement (ADR 0112) showed the two-family table rejecting valid filings: three annual packages projected nothing and six issuers lost their whole income statement because their roles were unknown to the classifier — `310000` (profit or loss by function, named in the classifier's own comment but absent from its table), the `315000`/`415000`/`425000` presentations, English vendor names (`BalanceSheet`, `ComprehensiveIncome`, `CashFlow`, `ChangesInEquity`), the `Rnn_RZiS|OCI|Bilans|KW|CF` abbreviations, and Polish names in upper case, with numbered suffixes, or with a `Skonsolidowane`/`Jednostkowe` prefix (plus a sixth name, `SprawozdanieZZyskowLubStratIInnychCalkowitychDochodow`). Every entry in the registry below was read from a filing's own `roleType/definition`, with one exception: `510000` (statement of cash flows, direct method) is the IFRS taxonomy's own sibling of the evidenced `520000` and is admitted on that authority. `classify_role` applies three bounded rules to the URI's **terminal segment** (the text after the last `/`) and nothing else:

| Rule | Input normalization | Registry (terminal segment → kind) |
| --- | --- | --- |
| 1. IFRS role number | none; the segment must END with `role-` + exactly six ASCII digits | `210000`, `220000` → balance · `310000`, `315000`, `320000` → income · `410000`, `415000`, `420000`, `425000` → comprehensive_income · `510000`, `520000` → cash_flow · `610000` → equity_changes |
| 2. Vendor name | ASCII lower case → strip trailing ASCII digits → strip one leading `skonsolidowane` or `jednostkowe`; then **equality** | `sprawozdaniezsytuacjifinansowej`, `balancesheet` → balance · `wynikfinansowy`, `incomestatement` → income · `sprawozdaniezcalkowitychdochodow`, `sprawozdaniezzyskowlubstratiinnychcalkowitychdochodow`, `comprehensiveincome` → comprehensive_income · `sprawozdaniezprzeplywowpienieznych`, `cashflow` → cash_flow · `sprawozdaniezezmianwkapitalewlasnym`, `changesinequity` → equity_changes |
| 3. Abbreviation | ASCII lower case; the whole segment must be `r` + two digits + `_` + token | `rzis` → income · `oci` → comprehensive_income · `bilans` → balance · `kw` → equity_changes · `cf` → cash_flow |

Never a substring over the URI (a `NotesToBalanceSheet` or `CashFlowDisclosures` role stays `other`), never a digit-block inference (`8xxxxx` notes stay `other`), and a segment satisfying two rules is `other`. The no-linkbase fallback boundary is unchanged and explicit: `has_presentation_linkbase` means "at least one concept→role pair parsed" (`structured_extraction::compute_layer1_generation`), so a linkbase whose roles all classify `other` projects nothing rather than falling back to dimensionlessness. `TAGGED_FACT_EXTRACTOR_VERSION` is 3 so already-captured packages re-classify on their next run. A new family enters this list only with the same evidence (a role definition read from a filing); the measurement's zero-output and missing-by-concept reports are the detector.
...
- **Statement basis is derived per instance** from the package entry path (TXT ships standalone and consolidated filings in one package under Polish-named folders) — a standalone filing is never silently stamped consolidated, and one document projects ONE basis (amendment 2026-09-14 below).
...
**Amended (2026-09-14, #509): currency is a property of the value kind, and one refused fact never costs a filing.** The first v2 measurement (ADR 0112) found one annual package aborting the whole run on `invalid financials value for currency: shares`: the projection copied the Layer 1 unit measure (`shares` for `xbrli:shares`) into `currency` for a `count` concept, the #93 store guard refused, and the commit loop's error propagated before any outcome was recorded. Two rules now hold. (1) The projection sets `currency` from the crosswalk `value_kind`: the unit's numerator measure for `monetary` (EPS included), `None` for every other kind — the slot key stays `(basis, metric_key)` and aliases sharing a key agree on `value_kind` (asserted over the whole crosswalk). The store guard stays the guard (no shape check at the projection; a refused monetary fact is never retried with `currency: None`). (2) A store refusal typed `InvalidFinancialsValue` on a fact-local field (`currency`, `value_numeric`, `attribution`, `data_quality`) holds back that fact and records it in the outcome detail as `rejectedFacts`; the set records `flagged` / `validation_failed`, the other facts emit, an all-rejected set records no facts and `emitted = false`; a refusal on a run-context field (`period_type`) or any other storage error in a fact commit still fails the run (facts committed earlier in the loop stay — per-call transactions; the outcome-row write itself is best-effort as before). `EXTRACTION_PIPELINE_VERSION` is 4 so the couldn't-extract verdicts stamped by the earlier build (this class and the #511 role families) retry once.


**Amended (2026-09-14, #508): one statement basis per document, stored under its true label.** A package carrying a consolidated and a standalone instance mixed both bases in the identity-check fact set (last write wins per metric), so a valid filing was flagged; and the structured writer stored every fact as `consolidated`, so a standalone-only concept — or every fact of a standalone-only filing — carried a false label the UI shows. The owner chose truthful storage (2026-09-14). Rules: (1) an instance's basis comes from its entry path — a standalone token (`jednostkow`, `separate`, `standalone`, `unconsolidated`) → standalone, a consolidated token (`skonsolidowan`, `consolidated`) → consolidated, both → ambiguous (excluded, counted), neither → consolidated (the ESEF mandate's stated default). (2) The document's primary basis is selected ONCE, over all of its usable (parseable value, dimensionless), crosswalked primary-statement facts regardless of period: consolidated if any comes from a consolidated instance, else standalone — so a consolidated instance with no usable primary-statement fact never suppresses usable standalone statements, which are then stored under their true label; the current and the comparative projections both use it, and the other basis stays Layer 1 evidence, counted as `otherBasis` in the coverage counts. (3) The writer stores the selected basis (`StructuredFactInput.statement_basis`); the prior cross-check and the history-plausibility gate read only facts of that basis and abstain rather than substitute the other. Readers still prefer consolidated through the one canonical preference (final › reported › consolidated › attribution), so consolidated wins among facts of equal data quality and variant; a final standalone fact can still outrank a preliminary consolidated one. `EXTRACTION_PIPELINE_VERSION` is 5.
