...
## Amendment (2026-09-02, #410 F4b/F4c) — Library nav spine

The sidebar spine this ADR owns gained three Library destinations that were palette-only: **Events** and **Report Season** (F4b S4, #447) and **Research** (F4c S3, #94 — Ctrl+4, freed by the retired Notebooks screen). Order: Companies, Watchlists, Alerts, Events, Report Season, Research, Sources (Transcripts sat between Research and Sources until its retirement, [ADR 0111](0111-retire-video-transcription.md)). The **Notebooks-global and Journal-global** routes are retired ([ADR 0108](0108-retire-docking-engine.md) amendment). Earlier docs cited "ADR 0104 dec. 1 amendment" for the F4b nav change — that decision is the accent budget; the nav spine is this ADR's, and those citations now point here. Decision 5's rationale stands: a nav entry is earned by a non-blank screen with a job (Research has both), never by a mode with nothing in it.
