...
- **Stored periodic reports** (already ingested): extraction of the mandatory "shareholders ≥5% of votes" section via the layered extraction pipeline (deterministic parse first, AI fallback with confirmation). Depth stream: real-data evidence (9/50 companies — 35/50 have no fetched documents) disqualified it as the breadth source.
