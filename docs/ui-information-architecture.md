...
**Reachability** ([ADR 0107](adr/0107-company-view-paradigm.md), F3a). `ResearchScreen` has no persistent top-level nav button; it is reached three ways: the command palette's `Open screen: Research` entry (a standalone route, no company scope), the Spółka workshop's **`research`** tool (scoped to the open company), and — inside a frozen named Cockpit view that already carries it — the **`research`** global panel (read-only structure; no new preset/panel can be added to it, F3a S3 consent 1).
