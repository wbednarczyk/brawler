...
- **🔬 Company workspace ([ADR 0107](adr/0107-company-view-paradigm.md), F3a #429)** — selecting a company opens the engine-free **Spółka** screen: glance bar (identity + signals/claims/shorts/events counters with typed drill targets), co-visible core (annual KPI table with provenance tickets, company feed, log-axis daily candles, report coverage, recommendations), bottom workshop bar opening one of 15 typed tools in place of the core (F3a S1–S2, #429). Single-company settings: the automation (autopilot) mode is edited only in Companies → Manage settings ([ADR 0056](adr/0056-per-company-settings-surface.md) amendment 2026-09-04 — the Fundamentals fold retired); the IR reports URL and the sector override live in the Basic info panel's edit mode (hosted by the Spółka `akcjonariat` tool — workshop `Akcjonariat`, palette `Otwórz narzędzie: Akcjonariat`); **cross-company settings management** is a separate **master-detail surface** ("Manage settings" in the Companies screen — company multi-select + grouped settings applied to the selection, with watchlist-scope selection), the scalable home for *all* per-company settings ([ADR 0056](adr/0056-per-company-settings-surface.md); v1 ships autopilot, pinned/watchlists next).
- **Compare (Porównaj)** — removed 2026-08-10 (#351, [ADR 0089](adr/0089-cross-company-comparison-and-valuation-l1.md) amendment): the cross-company screen saw no real use; the single-company periods × deltas table stays in the Spółka `Fundamenty` workshop tool (Fundamentals panel) and the comparison/valuation read models stay agent-reachable over MCP.
...
- **Activity control (F3d, [ADR 0109](adr/0109-activity-center-occurrence-ledger.md), #133, v0.81.0):** an icon button next to the source health indicator (`Otwórz aktywność`) signals **work in progress only** — a spinner + count while tasks execute (`N w toku`), a quiet count while tasks wait (`N w kolejce`), otherwise a quiet icon whose title names the last finished time; never a failure count (ADR 0097 amendment). It opens the **Activity panel** (a `Modal`, the palette's primitive): sections **W toku** (active + queued + stalled) and **Ostatnio** (last 7 days, 40 newest after the per-task collapse), rows grouped per company (non-company rows under "Źródła i system"), one row per **task** — an automatic report reading per document, a history sweep / re-extraction batch as a parent with progress, a source refresh per adapter, an extraction per document, a KPI ingest per run — each an `ExpandableRow` (family · raw subject · status · times · progress; the detail holds the raw error, attempt count, members) with **exactly one `open` action** (the dictionary verb — the row is a navigation, not a noun destination; #410 closure audit 2026-09-08) (`Otwórz w dokumentach` / `Otwórz spółkę` / `Otwórz źródła` / `Otwórz Dziś` / `Otwórz transkrypcje`) landing on the item itself (a document target opens the company's Documents tool with that row selected and scrolled into view — ADR 0107 deep-link target contract) through the Spółka `Tool` union, then closing the panel. Empty = a quiet state ("Nic nie działa w tle" + the last finished task), no CTA; a read failure keeps the last-known-good rows under a typed error strip with a retry control. While any modal dialog is open, global app shortcuts are suppressed (the dialog keeps `Escape`). The panel is a **status ledger**, not a notification surface: failures stay announced once on their ADR 0091 surface. Palette entry `Otwórz aktywność`; no dedicated shortcut. Journey-independent utility (ux-journeys).
...
Dense list rows should share state behavior before sharing body layout. The reusable row shell owns selected, unread, disabled, hover, and focus treatment. Row body structure stays screen-owned until a specific row family proves reusable enough to migrate. This avoids hiding domain differences between feed evidence, companies, sources, events, shortcuts, and settings forms behind one generic component.
...
Opening a company from the Companies library lands the **Spółka workshop** ([ADR 0107](adr/0107-company-view-paradigm.md), F3a; the docking engine and its frozen legacy dashboards were retired — [ADR 0108](adr/0108-retire-docking-engine.md)): a co-visible core (KPI table, feed, price chart, coverage, recommendations) plus a workshop bar opening one of 15 typed tools in place of the core. **Keyboard model (F3c, #197, contract `docs/plans/frontend-v2-f3c.md`):** the bar is an APG toolbar — one Tab stop (roving tabindex; the tab stop follows the last-focused entry, initially the pressed one), `←`/`→` (wrap) and `Home`/`End` move, `Enter`/`Space` open; `Ctrl+.` (`app.focusWorkshop`, rebindable) jumps to the bar from anywhere on Spółka and is a no-op while a dialog is open; `H`/`L` (`company.previousTab`/`company.nextTab`) cycle Overview → tools → Overview; `Shift+J`/`Shift+K` move to the adjacent company **with the tool closed** (ADR 0107 dec. 6). Focus is an explicit intent per transition: open → the tool `h2`; `Escape`/✕ → the closed tool's bar entry (`feedItem` maps to Feed); Overview/summary ticker → the Overview entry; `Shift+J`/`Shift+K` → the company picker (a mouse/palette company open moves nothing — a programmatically focused `<select>` always paints `:focus-visible`). `Escape` inside a tool returns to Overview through the dirty seam (a draft opens stay/discard; only native pickers — `select`, date inputs — keep Escape for themselves; `SearchField` consumes Escape only while it has a query). The panels below live in `src/screens/Spolka/panels/`, hosted by the workshop's typed tools. Panel keys: Basic info → `basicInfo`, Feed → `companyFeed`, Notebook → `companyNotebook`, Coverage → `coverage` (which now also hosts the flagged-periods review), Decision journal → `decisionJournal`, Short selling (KNF) → `shortPositions`, Warning signals → `redFlags`, Fundamentals/Claims/Quality/Report documents/diff as their own panels. Transcripts remains a future-milestone placeholder; company metadata lives in the Companies library (see there), not a panel.
...
Header shows: qualified ticker, display name, exchange, watchlist memberships as read-only context, last feed update, feed/unread/saved counts, and quick actions (refresh company, add note).
...
Note work happens in the per-company Spółka **`notatnik`** tool (above); cross-company note review has no dedicated screen (retired 2026-09-02, F4c S2, ADR 0108 amendment — 7 notes across 6 companies in 3 months, no real cross-company review use). Every deep link that used to land on a global Notebooks screen (Inbox "Create note draft", research evidence, global search) now opens the per-company tool via a typed intent, highlighting or prefilling as appropriate. Note detail shows origin links as compact, read-only actions: a feed-item origin's external source link, a URL-backed origin's source link. Agent reads across companies stay on MCP.
...
- Database: connection-pool tuning (`maxConnections`, `busyTimeoutMs`, `acquireTimeoutMs`) with safe-range clamping, a reset-to-defaults control, and a clear "applied on next launch" note (see [ADR 0032](adr/0032-search-and-backup-boundaries.md)); backups and restore sit in this section too (moved from Diagnostics 2026-09-08, #451)
