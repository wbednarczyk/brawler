...
Opening a company from the Companies library lands the **curated cockpit dashboard** ([ADR 0057](adr/0057-composable-views-and-curated-dashboard.md)): a seeded `cockpit_layout` scoped to the company, opening with a calm default panel set (Basic info, Fundamentals, Coverage, Feed, Claims, Quality, Report documents, Warning signals, Notebook) that stays composable (add/remove/move panels, then `Save dashboard` to persist per company). Panel keys: Basic info → `basicInfo`, Feed → `companyFeed`, Notebook → `companyNotebook`, Coverage → `coverage`, Review queue → `review`, Decision journal → `decisionJournal` (palette-only, not a default), Short selling (KNF) → `shortPositions` (palette-only, not a default), Warning signals → `redFlags` (a default), Fundamentals/Claims/Quality/Report documents/diff as their own panels. Transcripts remains a future-milestone placeholder; company metadata lives in the Companies library (see there), not a panel.
