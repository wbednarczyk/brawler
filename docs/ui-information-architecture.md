...
- **command palette (⌘K):** a **global** accelerator (v0.50 U6) — **Ctrl/⌘+K opens it from any screen**, complementing (not replacing) the visible spine. It lists app-level commands (navigation and global actions from the rebindable shortcut registry, jumps to saved views, **`Open company: TICKER` for every tracked company**, and **`Open screen: X`** entries for the global surfaces with no top-level nav item — Research, Events, Report Season, Notebooks, Decision journal); while the Spółka screen is active it also contributes contextual `Open <tool>` entries for its workshop tools. Every command carries `{actionKey, verb}` (ADR 0104 dec. 3) and a copy gate (`src/app/paletteCopy.test.ts`) enforces verb-first labels, both locales. Named-view **creation is gone** (F3a S3, consent 1 — the freeform cockpit is frozen): the Cockpit's own local palette, behind its "Commands" toolbar button, carries **only navigation** (`Open view: NAME` entries) — no preset/layout/panel-type/`Open panel:` commands remain — and shares no vocabulary obligation with the global palette. Both `app.commandPalette` (⌘K) and `app.focusSearch` (Ctrl+F) are rebindable in Settings → Keyboard shortcuts.
- **left sidebar — the IA spine ([ADR 0107](adr/0107-company-view-paradigm.md) amendment, F3a S3):** grouped into **Modes** (Dziś, Inbox, **Spółka** — Section `Spolka` — opens the last-viewed company, else the first pinned, else the first tracked company; never a blank screen), followed by **„Widoki"** — the user's saved named views (composable dockview layouts, ADR 0057) plus every legacy per-company dashboard (`dashboard:*` rows) as **„Dawny dashboard · TICKER"**, both **read-only structure** (no "+ New view", no `Dodaj panel`/`Zastosuj preset` — the freeform cockpit is frozen until the #414 engine decision; domain edits inside named views/the cockpit stay fully writable) — a **pinned/favorite companies** group (each with a glanceable conviction status — a neutral placeholder until per-company conviction lands), a **Library** group (Companies, Watchlists, Alerts, Transcripts, Sources), and a **Utilities** group (Settings, Diagnostics — developer-gated). This is the load-bearing navigation; a blank/freeform workspace is never the entry point. Exactly one sidebar row carries `aria-current="page"` at any time (a pinned+selected company's own row wins over the Spółka mode item). The **Decision journal (all companies)** is a standalone screen route (`Journal`), reachable via `Open screen: Decision journal` — not a panel inserted into the frozen cockpit.
...
- **🔬 Company workspace ([ADR 0107](adr/0107-company-view-paradigm.md), F3a #429)** — selecting a company opens the engine-free **Spółka** screen: glance bar (identity + signals/claims/shorts/events counters with typed drill targets), co-visible core (annual KPI table with provenance tickets, company feed, log-axis daily candles, report coverage, recommendations), bottom workshop bar opening one of 15 typed tools in place of the core (F3a S1–S2, #429). The frozen cockpit (dockview's free arrangement) stays reachable only via a named view or a "Dawny dashboard · TICKER" Widoki row (F3a S3) — no generic "open the cockpit for any company" bridge. Single-company settings (autopilot mode, IR reports URL) live in the Basic info panel's edit mode (hosted by the Spółka `akcjonariat` tool); **cross-company settings management** is a separate **master-detail surface** ("Manage settings" in the Companies screen — company multi-select + grouped settings applied to the selection, with watchlist-scope selection), the scalable home for *all* per-company settings ([ADR 0056](adr/0056-per-company-settings-surface.md); v1 ships autopilot, pinned/watchlists next).
- **Compare (Porównaj)** — removed 2026-08-10 (#351, [ADR 0089](adr/0089-cross-company-comparison-and-valuation-l1.md) amendment): the cross-company screen saw no real use; the single-company periods × deltas table stays in the company Dashboard (Fundamentals panel) and the comparison/valuation read models stay agent-reachable over MCP.
...
The shell is mode-based ([ADR 0054](adr/0054-mode-based-thesis-centric-shell.md), Accepted): a **left-sidebar IA spine + pinned companies**, a **Today/Pulse Triage home**, a **sectioned Company workspace** and **Focus** modes, with a **glanceable per-company conviction status** (a composite rolled up from a fixed check set, decomposed into a few named factors, plus a watchlist rollup). The organizing unit is the **company + its thesis/conviction state**; the feed is *input* that moves that state. A cited UX research pass (terminals + retail-research apps + IDE/PKM) grounds this — see [ADR 0054](adr/0054-mode-based-thesis-centric-shell.md) Evidence. Per-panel content/behavior for the company deep-dive is specified in [Company Cockpit Dashboard Panels](#company-cockpit-dashboard-panels); this subsection is the cockpit **engine** mechanics only.

Since F3a S1 ([ADR 0107](adr/0107-company-view-paradigm.md)) the main company surface (`Spółka`) runs without a docking engine; the engine below serves only the named views and the four legacy `dashboard:*` layouts.

**Layout structure is frozen (F3a S3, ADR 0107 decision 5):** every named view and legacy dashboard renders a strip — "Layout frozen until the engine decision" / "Edit inside panels still saves." — and view creation, panel add/close/drag, preset application, and layout save are all removed; the local palette is navigation-only (`Open view: <name>`). Domain editing inside an already-open panel (facts, claims, notes, journal, quality) stays fully writable — only the dock's own arrangement is locked, and the linked selection workflow (feed → inspector → claims/diff) keeps working. The four legacy per-company dashboards (`dashboard:cbf/acp/abe/cdr`) stay reachable read-only as "Legacy dashboard · TICKER" rows in the sidebar Widoki group, fixed to their company (no company/preset selector there — the row already chose it). A saved layout whose geometry predates the running dockview version (or fails to parse) shows "Saved layout comes from another version — default layout restored" instead of silently discarding it. `#414` decides the engine's stay/replace/remove fate; the freeze exists only until then.

**dockview is the opt-in "advanced layout" engine** inside the Company-workspace mode (re-scoped from the app-wide grid; [ADR 0053](adr/0053-dockview-layout-pilot.md) amended by [ADR 0054](adr/0054-mode-based-thesis-centric-shell.md)). It is built behind the `src/screens/Cockpit/` adapter; framework comparison and the gradual-migration plan are in ADR 0053. Its behavior (carried forward into the advanced-layout mode):
...
- **Named saved layouts** (decision 3A) let the user switch task-shaped workspaces; persisted in SQLite (`cockpit_layouts`), not `localStorage` ([data-model.md](data-model.md), [contracts.md](contracts.md)). A saved layout naming a panel key that no longer exists degrades cleanly on restore — unknown pane kinds are stripped rather than left as ghost panes (`savedLayoutGeometry` tests).
...
## Company Panels (Spółka Workshop + Frozen Cockpit)

Opening a company from the Companies library lands the **Spółka workshop** ([ADR 0107](adr/0107-company-view-paradigm.md), F3a): a co-visible core (KPI table, feed, price chart, coverage, recommendations) plus a workshop bar opening one of 15 typed tools in place of the core. The SAME panel components below are shared verbatim by Spółka's tools and by a **frozen** legacy per-company Cockpit dashboard (`dashboard:*`, reachable only as a read-only-structure "Dawny dashboard · TICKER" Widoki row — no add/remove/move panel, no `Save dashboard`; domain edits inside the panels stay fully writable). Panel keys: Basic info → `basicInfo`, Feed → `companyFeed`, Notebook → `companyNotebook`, Coverage → `coverage` (which now also hosts the flagged-periods review), Decision journal → `decisionJournal`, Short selling (KNF) → `shortPositions`, Warning signals → `redFlags`, Fundamentals/Claims/Quality/Report documents/diff as their own panels. Transcripts remains a future-milestone placeholder; company metadata lives in the Companies library (see there), not a panel.
...
**Add-panel surface — retired** (card 106f8a7, then removed by F3a S3 consent 1, [ADR 0107](adr/0107-company-view-paradigm.md) decision 5): the cockpit no longer has an `Add panel` button or `Open panel:` palette entries — every structural command that adds/creates/rearranges the panel layout is gone from the frozen dockview (surviving structural affordances, if any, are the frozen-view invariant test's job to enumerate — see `docs/plans/frontend-v2-f3a.md`). Retargeting an existing named view to another company still works via its header **View company** selector and the palette's `Switch view company:` entries. Reaching one of these panels for a company today means the Spółka workshop's typed tool bar, not an add-panel flow.
...
**Reachability** ([ADR 0107](adr/0107-company-view-paradigm.md), F3a). `ResearchScreen` has no persistent top-level nav button; it is reached three ways: the command palette's `Open screen: Research` entry (a standalone route, no company scope), the Spółka workshop's **`research`** tool (hosts the same global screen — it is not company-scoped; the company picker inside it selects the scope), and — inside a frozen named Cockpit view that already carries it — the **`research`** global panel (read-only structure; no new preset/panel can be added to it, F3a S3 consent 1).
