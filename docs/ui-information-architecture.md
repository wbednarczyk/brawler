...
- **left sidebar — the IA spine (implemented):** grouped into **Modes** (Today/Pulse · Companies/Company-workspace · Compare — see below), followed by the user's **saved named views** (composable dockview layouts, ADR 0057) as their own nav entries and a **"+ New view"** creator, a **pinned/favorite companies** group (each with a glanceable conviction status — a neutral placeholder until per-company conviction lands), a **Library** group (Inbox, Watchlists, Alerts, Transcripts, Sources), and a **Utilities** group (Settings, Diagnostics — developer-gated). This is the load-bearing navigation; a blank/freeform workspace is never the entry point. A single **"Research"** entry in Modes opens the cockpit as a **never-blank** door — it resumes the last-viewed layout or seeds a first company (ADR 0057 decision 5, **amended 2026-07-13** — owner discoverability decision, card `47d5fbb`).
...
- **🏠 Today / Pulse** (default / home) — the morning-review surface (journey J1): a **grouped, severity-ranked "what changed" stream** (a deterministic dedup → group → rank pipeline; urgent rows lead, then notable, then routine; one Review action per row, `j`/`k` navigation over rows and expanded group members) beside a **narrow counters column** (Pilne / autopilot / do weryfikacji / nadchodzące) that doubles as a filter, above it a compact **morning-briefing strip** and a dismissible **config-state banner**. The **quiet state is the goal** — when nothing needs attention the stream shows one calm empty state. A compact **Aktywne | Archiwum** switch in the header toggles a **second, read-only view** — an **Archive** of dismissed attention events (dismiss is an acknowledgement, never a delete). See [ADR 0087](adr/0087-today-attention-home-v2.md) (grouping + typed severity + quiet toasts), built on [ADR 0076](adr/0076-ui-design-system-and-density-contracts.md) (task U-Rb).
...
- **⚖️ Compare (Porównaj)** — cross-company KPI tables + comparative valuation L1 (`v0.61.0`, [ADR 0089](adr/0089-cross-company-comparison-and-valuation-l1.md)). **Live in the sidebar** as its own Modes item directly under Dashboard (entry restored in v0.61 now that the comparison read model + FX substrate give the mode content; approved storyboard: `docs/mockups/v061-compare-storyboard.html`; UI copy says "zestaw spółek", never "kohort"). The screen selects a **zestaw spółek** (multi-select from tracked companies + a watchlist quick-pick + a "peers in sector" helper), a canonical KPI, and a granularity (annual/quarterly), then renders a side-by-side evidence-linked table (per-cell ⧉ → the fact's company Fundamentals/report surface; EUR→PLN chip with FX-basis tooltip on non-PLN companies; typed per-cell flags for gaps — never a silent absence or an unconverted comparable) with per-section typed+translated error strips (retry per section). Only the single-company periods×deltas table lives inside the company Dashboard (Fundamentals panel) — all multi-company comparison lives here.
- **📋 Journal** — the decision journal (shipped `v0.52.0`, [ADR 0071](adr/0071-judgment-capture.md)).
...
The grouped Today attention list + persistent-toast wiring (T4) live on Today/Pulse (see the Today section above); this screen owns rule authoring and the raw fired-event list.

## Company Cockpit Dashboard Panels

Opening a company from the Companies library lands the **curated cockpit dashboard** ([ADR 0057](adr/0057-composable-views-and-curated-dashboard.md)): a seeded `cockpit_layout` scoped to the company, opening with a calm default panel set (Basic info, Fundamentals, Coverage, Feed, Claims, Quality, Report documents, Warning signals, Notebook) that stays composable (add/remove/move panels, then `Save dashboard` to persist per company). Panel keys: Basic info → `basicInfo`, Feed → `companyFeed`, Notebook → `companyNotebook`, Coverage → `coverage` (which now also hosts the flagged-periods review), Decision journal → `decisionJournal` (palette-only, not a default), Short selling (KNF) → `shortPositions` (palette-only, not a default), Warning signals → `redFlags` (a default), Fundamentals/Claims/Quality/Report documents/diff as their own panels. Transcripts remains a future-milestone placeholder; company metadata lives in the Companies library (see there), not a panel.
...
**Review queue panel — REMOVED (`v0.59.0`).** The pending-KPI-proposal pane of [ADR 0077](adr/0077-trusted-extraction-foundations.md) §4/§5 went with the in-app AI layer ([ADR 0084](adr/0084-retire-in-app-ai-layer.md)): its proposals came from OCR bootstrap and text-AI extraction, both retired. Deterministic extraction either emits a validated fact or flags the period, so human review now happens in the **Coverage panel's "Flagged periods" section** (above), and the `review` panel key no longer exists. A saved cockpit layout still naming it degrades cleanly — unknown pane kinds are stripped on restore rather than left as ghost panes.
...
**Dashboard redesign (2026-07-13, epic `c793ca1`, amends [ADR 0057](adr/0057-composable-views-and-curated-dashboard.md) decision 5).** The **standalone Research screen is retired as a nav destination**. The same `ResearchScreen` component now lives **only** as the **`research` cockpit panel**, surfaced through the Dashboard's **"Evidence / Research" preset** (which follows the view company). Callers that used to open the screen — `openResearchEvidence` (research case) and brief/digest global-search results — redirect into the Dashboard on the `evidence` preset (`openAdvancedLayout(companyId, "evidence")`). The `Research` `activeSection`/`Section` value and its standalone render are kept for programmatic/deep-link navigation, consistent with the sibling panel-hosted sections (Notebooks/Events/ReportSeason) — there is simply no nav entry that targets it.
