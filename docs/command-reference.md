...
The full `make` target catalog (extracted from [engineering-workflow.md](engineering-workflow.md), which keeps the workflow rules — what runs where, the TDD loop, the Definition of Done). Loaded on demand; the Makefile itself is the source of truth for recipes.
