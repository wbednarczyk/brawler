...
8. **`commit_kpi_ingest`** *(shipped, #386; outcome `excluded` + `excludedCount`/`excluded` ledger since #399 S5, ADR 0102 dec. 3/4)* — `{ runId, manifestHash, revision, execution?: ExecutionMeta }` → the receipt, faithful to the durable shape: `{ runId, terminalStatus: "complete"|"partial", periodId|null, acceptedCount, outcomesSchemaVersion, outcomes: [{ observationId, revision, ordinal, metricKey, factId|null, outcome, detail? }], excludedCount, excluded: [{ label, reason }], manifestHash, manifestRevision, committedAt }`. Outcome vocabulary: `created|reobserved|upgraded|divergent|excluded`. `detail` is omitted for `created|reobserved|upgraded`; `divergent` carries `{ existingFactId }`; `excluded` carries `{ label, reason }` and its `factId` is always `null` — commit writes no fact for an observation sealed `excluded` in the VALIDATED manifest (a status/reason tampered into the live staged row AFTER validation cannot reach commit at all — commit reads only the frozen manifest). `outcomesSchemaVersion` is `2` for every new commit (`excluded` is a v2-only outcome member); stored `1` receipts stay readable forever, never rewritten. `excludedCount`/`excluded` are populated regardless of `terminalStatus` — exclusion never manufactures `partial` (that stays exclusively a function of the `expected` denominator, ADR 0098 dec. 4/5); excluding an EXPECTED key still leaves it counted missing and still requires a `missingReasons` entry. Idempotent replay returns the stored receipt verbatim; a stale tuple → `conflict`.
