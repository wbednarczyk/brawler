...
*LIVE — [ADR 0098](adr/0098-mcp-native-kpi-acquisition-lifecycle.md), [ADR 0099](adr/0099-acquisition-mcp-surface-mechanics.md), epic #353. **Shapes frozen here.** The implementation (#382–#386) is written to these shapes; changing them is a deliberate contract change. All ten tools are live: 1, 2, 8, 9 since #384; 3–4 since #385; 5, 7–8 since #386; 6 since #399 S4 ([ADR 0101](adr/0101-agent-proposed-kpi-definitions-and-full-catalog-visibility.md), the allowlist's first widening since #386).* A compact run-based MCP workflow of **ten tools** behind the acquisition-scoped credential (ADR 0099 dec. 2–3, widened by ADR 0101 dec. 1): the nine original workflow tools (`get_kpi_ingest_document` for portable document-byte delivery among them) plus `propose_kpi_definition` (agent-side catalog minting, company-scoped `origin=agent`, hard-edged anti-duplicate guard). `record_financial_facts` remains a low-level repair tool, no longer the normal agent path. Validation and commit execute synchronously inside the tool call (ADR 0099 dec. 1). The mechanics (credential, gate, lease, blobs, profiles, budgets, execution metadata) are canonical in ADR 0099; this section freezes the wire.

Conventions: camelCase, `deny_unknown_fields`, internal ids only. String limits are **UTF-8 byte** limits; control characters (U+0000–U+001F) are rejected in every text field (`invalid_input`) — this bounds JSON-escaping expansion at ×2 and guarantees every schema-valid request stays under the 1 MiB transport cap (a schema-INVALID oversized body is refused by the transport with 413 before JSON parsing). Failures use the `CommandError` envelope with seven additive codes (see [Error codes](#error-codes)): `run_lease_expired`, `run_taken_over`, `response_budget_exceeded`, `synonym_redirect`, `draft_chunk_conflict`, `draft_incomplete`, `draft_superseded`.
...
3. **`get_kpi_ingest_context`** *(shipped, #385; catalog widened + `notRequested`, ADR 0101 dec. 7/8; `receipt` section, ADR 0102 dec. 12, #399)* — `{ runId, section?: "catalog"|"plausibility"|"manifest"|"receipt", cursor?, limit? }`. The default call (no `section`) returns everything within caps: `{ run: RunStatus, document: { url, title|null, contentType|null, byteSize|null, localPath|null }, derivedPeriod: { fiscalYear, periodType, periodEnd }|null, catalog: [CatalogEntry], notRequested: ["metricKey"], plausibility: [{ metricKey, slot: { definitionId, scope, attribution, measureWindow }, slotOrigin: "observed"|"candidate", median|null, nonZeroCount, abstentionReason|null, recentPoints: [{ fiscalYear, periodType, value }] }], profileRules: ["…"], manifestAvailable: bool, truncated: { "catalog": "cursor"?, "plausibility": "cursor"?, "manifest": true? } }`. `catalog` = **every canonical KPI definition** (`company_id IS NULL`) plus the company's `origin=agent` minted extras — user-origin company rows stay excluded, and a SECTOR-scoped shared row is offered only to companies of that statement type (a foreign-sector key would never resolve at staging — §G harvest, #399 S8). Two-tier `CatalogEntry`: `{ definitionId, metricKey, label, unit|null, statementGroup, valueKind, origin }` (FULL) for the run's expected keys and the company's agent-minted extras; `{ metricKey, label }` (COMPACT, every other field absent) for the rest of the canon — enough to check "does this already exist" before proposing, server-side propose validation is the real duplicate guard. Sort order is `(tier, metricKey, definitionId)`: every FULL entry precedes every COMPACT one, so the truncated first page always carries what the run needs before the wider canon. With ~400 canonical definitions the default call's `catalog` is **always truncated** (`truncated.catalog` carries a cursor) — page the `catalog` section to the end of its cursor before proposing a new definition (ADR 0101 dec. 7 doctrine). `notRequested` lists the metric keys present in the returned `catalog` page that are NOT members of `run.expectedKpis.keys`: their absence from `plausibility` is doctrine, never "no history exists" (ADR 0101 dec. 8) — `plausibility` stays computed only from the expected set (cost unchanged). `plausibility` is validator-equivalent evidence (exact slot identity, whole-history median, non-zero count, abstention reason, bounded recent points) — `observed` entries are slots the fact store realizes, `candidate` entries are the recommended default slot a staged observation lands in (dated addition, #385); `median` is `null` exactly under `thin_history`; `document.byteSize` is the size of the run's PINNED BLOB, never the mutable document row's; `derivedPeriod` is a hash-guarded hint served only when the cached derivation describes the run's frozen bytes; `localPath` is a local-client convenience, never the delivery contract. **Overflow recovery**: a section exceeding its cap returns truncated with a cursor in `truncated`; the remainder is fetched via section calls — never a dead end. **Section pages** (dated completion, #385): a section call returns `{ runId, section, catalog?|plausibility?|manifest?, nextCursor|null }` carrying only the requested section; an empty terminal page is legal. Cursors are opaque; the special `{}` start-of-section cursor (emitted when the default call had to shrink a section to zero) restarts catalog/plausibility from the beginning and is refused for manifest. The manifest (repair context; it can exceed the budget by itself) is NEVER in the default call — only `section: "manifest"`, paginated over observations (manifest header + `runDiagnostics`/`completeness` on the first page; continuation cursors pin the validation attempt they started from, so a newer attempt never splices into a paginated read; `section: "manifest"` with no attempt yet → `conflict`). **`section: "receipt"`** (ADR 0102 dec. 12, epic #399 S6) is the paged full commit receipt — `commit_kpi_ingest`'s own response is a bounded summary (tool 8 below), so the complete `outcomes` ledger (up to 1000 rows under a chunked draft, one `{label, reason}` per `excluded` outcome) reads here instead: page 1 carries the receipt header (`runId`, `terminalStatus`, `periodId`, `acceptedCount`, `outcomesSchemaVersion`, `manifestHash`, `manifestRevision`, `committedAt`) plus the first `outcomes` page; continuation pages carry `outcomes` only. Unlike the manifest section, the cursor is a bare offset — a run has AT MOST one commit receipt EVER (`UNIQUE(run_id)`), so no attempt needs pinning. No receipt yet → `conflict`. Pure read, no side effects.
...
5. **`stage_kpi_observations`** *(shipped, #386; `excluded` disposition since #399 S5; chunked drafts since #399 S6, [ADR 0102](adr/0102-full-capture-staging-contract-excluded-observations-and-chunked-drafts.md) dec. 1/2, 6-11, 14)* — `{ runId, observations: [ObservationInput] (1..100), missingReasons: { "<metricKey>": "<reason>" }, execution?: ExecutionMeta, draft? }` → `{ runId, revision, observationCount, status: "staged" }` (single-call/finalize) or `{ runId, status: "draft_open"|"draft_appended", draftId, expectedObservations?, chunkIndex?, chunksReceived? }` (open/append) — every field beyond `runId`/`status` is absent unless the branch populates it. **`observations` is the COMPLETE revision snapshot** for a single-call request (storage replaces the whole set per revision — a repair must resend the retained observations). **`missingReasons` is required** on a single-call request or a finalizing draft call; `{}` is the explicit "no declared omissions"/clear — there is no destructive default; it is written in the SAME transaction that installs the revision. `ObservationInput` mirrors the staging boundary: `rawLabel`/`rawValue` non-blank ≤256 B; `metricKeyCandidate` ≤256 B; `normalizedValue` decimal string ≤64 B; `rawCurrency`/`rawUnitScale` unnormalized source strings ≤128 B; `currency` = 3 uppercase ASCII letters; `unitScale` ∈ `ones|thousands|millions`; `measureWindow` ∈ `flow|point_in_time|trailing|cumulative|duration`; `attribution` ∈ `total|owners_of_parent|nci`; `scope` ∈ `standalone|consolidated`; `mappingStatus` ∈ `unmapped|mapped|no_definition|excluded` (default `unmapped`); `exclusionReason` ≤512 B, **required exactly when `mappingStatus="excluded"`**, refused (typed `invalid_input`) on any other disposition; `citation: { page? (≥1), table? (≤128 B), row? (≤128 B), quote? (≤1024 B) }`; `missingReasons` ≤128 entries, key ≤128 B, reason ≤512 B. `excluded` is a deliberate, reasoned "captured, not mapped" disposition — distinct from an accidental `unmapped`, which stays validated as `Flagged`; an excluded observation seals `mapping.excluded` (Info severity, never `Flagged`, never a run-level diagnostic) and is held to none of the other fact-quality checks. Transport arithmetic per CALL (string payloads at every documented maximum; excludes JSON field/structure overhead): 100 max-bounded observations ≈ 269 KiB raw + ≤83 KiB reasons ≈ 352 KiB, ×2 escaping bound ≈ 703 KiB — every schema-valid request reaches tool validation under the 1 MiB cap. Raising the 100-per-call cap is an additive change.

   **Chunked drafts** (dec. 6-11, 14 — a >100-observation report physically fits by spreading the SAME complete-snapshot proof across multiple calls, never by raising the per-call cap): `draft` is an **explicit wire union** — absent selects the single-call shape above, byte-for-byte unchanged (dec. 14; `missingReasons` stays required exactly then). Three legal `draft` sub-forms, runtime-classified (a malformed combination is a typed `invalid_input`, not a schema-level branch):
   - `{ open: true, expectedObservations }` — mints a **server-issued** `draftId` (never client-supplied) bound to the run's current lease epoch; exactly ONE active draft per run — a second open, or a single-call attempt while one is open, is a typed `conflict` (explicit abort required, never a silent orphan). The run STAYS in `extracting`/`validation_failed` — a draft is a sub-resource, never a new run state.
   - `{ draftId, chunkIndex }` + `observations` (1..100, same per-call caps as above) — appends one chunk. Re-checks the LIVE lease on EVERY call (not only open/finalize); never bumps `revision`; structurally invisible to validation until finalize. `missingReasons` MUST be absent (typed `invalid_input` otherwise — reasons belong to a complete revision). Replaying the same `(draftId, chunkIndex)` with matching content is an idempotent ack; the same index with different content → typed `draft_chunk_conflict`.
   - `{ draftId, final: true }` + `missingReasons` (required, same caps as the single-call form) — finalizes: checks chunk contiguity (a gap or zero chunks → typed `draft_incomplete`), the aggregate caps below, re-checks the live lease, then bump-revision + install + flip-state + delete-the-draft in ONE transaction — global `ordinal`s assigned by `(chunkIndex, position-within-chunk)`, identical to today's single-call numbering. Response shape matches the single-call success shape exactly (`status: "staged"`).

   A lease takeover invalidates any open draft (`draft_superseded` on the next operation against it) — open a fresh draft instead. Cancelling or failing the run, and startup lease-reclaim, all clear any open draft. `get_kpi_ingest_status.openDraft: { draftId, chunksReceived, expectedObservations } | null` lets a restarted agent session resume instead of guessing. **Aggregate caps at finalize** (amends ADR 0099 dec. 7 — the per-call 100/1 MiB bound is UNRAISED, it now bounds a CHUNK): `AGGREGATE_OBSERVATIONS_MAX = 1000` observations and a **frozen aggregate byte cap of 7,198,720 B (≈ 6.87 MiB)** — 10 chunks (`AGGREGATE_OBSERVATIONS_MAX` / 100-per-chunk) × the 703 KiB (719,872 B) per-call escaping bound above, reused deliberately conservative rather than a freshly derived tighter number; aggregate `missingReasons` reuses the existing ≤128-entry cap unchanged.
6. **`propose_kpi_definition`** *(shipped, #399 S4, [ADR 0101](adr/0101-agent-proposed-kpi-definitions-and-full-catalog-visibility.md); `valueKind` since #403)* — `{ runId, metricKey, label, unit?, statementGroup, description?, valueKind? }` (`metricKey` = `^[a-z][a-z0-9_]*$`, the same shape `create_kpi_definition` enforces; `statementGroup` ∈ `income|balance|cash_flow|per_share|other`; `description`, when present, is stored as the new row's `formula` free text — never evaluated, `computation` is always `reported`; `valueKind` ∈ `monetary|count`, default `monetary` — a deliberately narrower vocabulary than `kpi_definitions.value_kind`'s full set (§ KPI Definition): a full-capture agent stages disclosed numbers, never derived ratios, so `ratio`/`percentage`/`physical`/`duration` are refused typed `invalid_input`; `unit` is allowed with `count` — the canon already carries non-null units on count rows, e.g. `shares`, `properties`). `scope`/`companyId`/`sector`/`origin` are never caller-suppliable: the storage layer forces `scope="company"`, `companyId` = the run's own company, `sector=null`, `origin="agent"` (dec. 6) — a proposal is always an extra, never a completeness-denominator entry. Result on success: `{ definition: KpiDefinition, created: bool }` — `created: false` on an exact-key reuse (dec. 3), `true` on a fresh mint. Guard order (dec. 4, no fuzzy matching): (1) an exact `metricKey` match for this company returns the company's existing definition with no alias check at all; (2) a curated `kpi_aliases` (ADR 0100 dec. 12) hit refuses with typed `synonym_redirect` — message names the canonical key and its `definitionId`, no separate structured payload (`CommandError` stays `{code, message}`); an alias source is deprecated even when its zero-fact canonical row still exists, so the redirect outranks canon reuse; (3) an exact match in the shared canon (`companyId` null) returns the CANONICAL definition — never a company-scoped shadow whose duplicate `metricKey` would fragment the catalog; (4) only then is a fresh row created. Requires the caller's live lease exactly like `stage_kpi_observations` (dec. 11): `run_lease_expired`/`run_taken_over` on a lapsed/foreign lease. Doctrine (dec. 7): page `get_kpi_ingest_context`'s `catalog` to the end of its cursor before proposing — the server guard catches exact keys and curated synonyms, but only the paged catalog tells the agent which existing key a differently-worded line should map to.
7. **`validate_kpi_ingest`** *(shipped, #386; response bounded since #399 S6, ADR 0102 dec. 12, amends ADR 0099 dec. 1)* — `{ runId, revision }` (**generation-pinned** — the revision from the stage response; the synchronous path pins its generation exactly like the queue). Result: `{ outcome: "ready"|"failed"|"superseded", revision, manifestHash|null, severityCounts: { passed, unreviewed, flagged }|null, current: { status, revision, manifestHash }|null }` — **no inline `manifest`**; `severityCounts` (observation counts by `validationState`) is the bounded substitute, `null` only on `superseded` (no manifest was produced by this call). The full manifest ([Data Model § validation](data-model.md#kpi-ingest-validation) is the manifest shape; a `failed` manifest IS the typed repair report) reads via `get_kpi_ingest_context section:"manifest"`. A moved generation/state → `superseded` with the current tuple. A broken state, empty staging, or storage failure → `CommandError`.
8. **`commit_kpi_ingest`** *(shipped, #386; outcome `excluded` since #399 S5, ADR 0102 dec. 3/4; response bounded since #399 S6, ADR 0102 dec. 12, amends ADR 0099 dec. 1)* — `{ runId, manifestHash, revision, execution?: ExecutionMeta }` → a BOUNDED summary: `{ runId, terminalStatus: "complete"|"partial", counts: { acceptedCount, excludedCount }, manifestHash, manifestRevision }` — **no inline `outcomes` ledger** (a 1000-row ledger under `AGGREGATE_OBSERVATIONS_MAX` cannot ride an unpaged response); `excludedCount` is populated regardless of `terminalStatus` — exclusion never manufactures `partial` (that stays exclusively a function of the `expected` denominator, ADR 0098 dec. 4/5); excluding an EXPECTED key still leaves it counted missing and still requires a `missingReasons` entry. The full receipt — `periodId`, `outcomesSchemaVersion`, `outcomes: [{ observationId, revision, ordinal, metricKey, factId|null, outcome, detail? }]` (outcome vocabulary `created|reobserved|upgraded|divergent|excluded`; `detail` omitted for `created|reobserved|upgraded`; `divergent` carries `{ existingFactId }`; `excluded` carries `{ label, reason }`, `factId` always `null` — commit writes no fact for an observation sealed `excluded` in the VALIDATED manifest, a post-validation tamper to the live staged row cannot reach commit at all, which reads only the frozen manifest), `committedAt` — reads via `get_kpi_ingest_context section:"receipt"`. The excluded ledger is the `outcomes` rows with `outcome === "excluded"` (their `detail.{label, reason}`) — there is no separate top-level ledger field. `outcomesSchemaVersion` is `2` for every new commit (`excluded` is v2-only); stored `1` receipts stay readable forever, never rewritten. Idempotent replay returns the stored receipt's summary verbatim; a stale tuple → `conflict`.
9. **`get_kpi_ingest_status`** *(shipped, #384)* — `{ runId }` → `RunStatus`. Pure read.
10. **`cancel_kpi_ingest`** *(shipped, #384)* — `{ runId }` → `RunStatus` (`cancelled`). From `committing` or a terminal state → `conflict` (ADR 0098 dec. 6).

Rules:

- **Scope allowlist = exactly these ten tools** (ADR 0099 dec. 3, widened by [ADR 0101](adr/0101-agent-proposed-kpi-definitions-and-full-catalog-visibility.md) dec. 1); the acquisition credential discovers and captures nothing — capture and URL→`documentId` resolution belong to the UI/Full scope/#354; "process all pending KPI ingests" presumes pending runs exist. Full scope is a superset.
- **Budgets** (runtime, typed overflow — never silent truncation): stage ≤100 observations PER CALL with the byte limits above (chunked-draft aggregate: ≤1000 observations, ≤7,198,720 B, ≤128 missingReasons entries — see tool 5); context pages: catalog ≤64 definitions, plausibility ≤64 slots with ≤8 recent points each, manifest section ≤50 observations, receipt section ≤50 outcomes; context OUTPUT strings are byte-bounded too (label ≤256 B, unit/statementGroup ≤64 B, profileRule ≤512 B, abstentionReason ≤256 B; overlong stored values are per-field truncated with an `…` marker) so every response is ≤256 KiB by construction; document chunk ≤262144 B; `list_pending` limit ≤50; the scoped `tools/list` carries its own frozen snapshot since #386 (`tools_list_schema_acquisition` + the ≤16 KiB byte gate as regression — measured 12,236 B as of #403's `valueKind` field). Unsatisfiable requests (e.g. out-of-range `limit`) → `response_budget_exceeded`. Enforcement mechanics (as-built, #385): the ≤256 KiB context budget is provable because every variable stored field is bounded at its producer — KPI-definition identities (`metric_key`, imported ids) ≤256 B control-character-free at both definition writers, `last_error` ≤2048 B at its writer, the creation-time expected stamp ≤256 keys (a larger relevance union refuses run creation) — and the default call dynamically shrinks its pageable sections (cursors in `truncated`, `{}` = start-of-section) before a final defensive `response_budget_exceeded` gate that no bounded stored state can reach.
...
| `synonym_redirect` (#399, ADR 0101) | `propose_kpi_definition`'s `metricKey` is a curated `kpi_aliases` synonym of an already-populated key. | Not retryable with the same key; reuse the named `definitionId`. |
| `draft_chunk_conflict` (#399, ADR 0102) | A chunked-draft `(draftId, chunkIndex)` replay's content disagrees with the stored chunk (server-computed hash mismatch). | Not retryable at the same index; append at a fresh index or abort the draft. |
| `draft_incomplete` (#399, ADR 0102) | A chunked-draft finalize was refused: zero/gapped chunks, or the assembled total disagrees with the draft's declared `expectedObservations`. | Retryable after appending the missing chunk(s). |
| `draft_superseded` (#399, ADR 0102) | A chunked draft's lease epoch is stale (a takeover happened since it opened) or it was already finalized/aborted. | Not retryable; open a fresh draft. |
...
- `factsAvailable` is true once any facts exist for the resolved period; a per-metric `actualValue` is the **canonical confirmed** fact for that metric+period (joined via `kpi_definitions.metricKey`; preference `final` › `reported` › `consolidated` › `total` over `owners_of_parent`, then id — never the latest-inserted, #496), else null with `outcome = "unknown"`.
...
- `restore_backup({ fileName })`: stages `fileName` for restore on next app relaunch. Verified before staging (`PRAGMA integrity_check` + a `schema_migrations` version-set check, #319) — an unrecognized/corrupt file errors `invalid_input` and stages nothing; verification runs again at relaunch, so a candidate that failed then (or was tampered with meanwhile) never reaches the live database — refused recoverably, reported as a Today attention event instead of aborting startup.
...
- `list_financial_facts(input)`: returns financial fact values, optionally filtered by company, period, or KPI definition — **newest period first** (`IFNULL(period_end_date, fiscal_year || '-12-31')`, then fiscal year), within a period the **canonical fact first** (`CANONICAL_FACT_PREFERENCE_ORDER`: `final` › `reported` › `consolidated` › `total` › `owners_of_parent`), then `metricKey`, then id — never insert order (#496). Consumers that keep the first fact per slot (the Fundamentals matrix, the cross-check prior, metric history, the MCP dossier's 100-fact cap) therefore see the canonical sibling, and the cap keeps the newest periods.
...
- `list_report_documents(companyId)`: returns all captured report documents for one company, **newest disclosure first** (the document's disclosure key: the `/emitent/YYYY-MM/` URL month, else `fetchedAt`, else `createdAt`; ties by id — never insert order, data-model.md § Report Documents / § Model principles, #496). Each `ReportDocument` carries `docKind` — the classified taxonomy value (`periodic_ssf | periodic_jsf | auditor_opinion | presentation | governance | other`, [ADR 0077](adr/0077-trusted-extraction-foundations.md) §1) or `null` when unclassified (a row predating the taxonomy; classification runs at ingestion for new rows). The UI shows it as a per-row kind badge and filters the list by it.
...
Act commands **classified but not exposed** (each justified inline in the registry's tool tables, `src-tauri/src/mcp/registry/`): niche period plumbing (`create/update_financial_period`), `clone_framework`, company-config setters (`set_company_ir_reports_url`, `set_company_sector`, `rename_watchlist`), report-pipeline document machinery (`fetch_report_document`, `extract_report_sections`, `reclassify_report_documents`, `resolve_ir_report`, `extract_report_document_data`, — `capture_report_document` itself is now exposed, T8 below), the video-transcript lifecycle (`create/update/run_video_transcript_job` — the only in-app AI dependency), and the **admin / one-off job triggers** (`backfill_company_health_facts`, `backfill_ownership_extraction`, `run_history_sweep`, `rebuild_fundamentals`, `refresh_gpw_company_registry(_if_stale)`) — whole-corpus rebuilds, quote-history sweeps, registry refresh, and derived-fact backfills the exposed extraction/refresh tools already cover.
...
**`capture_report_document { companyId, url, periodId?, originRef?, title?, attribution? }`** — registers and fetches the document an agent read, by URL, so `record_financial_facts` has a `reportDocumentId` to cite (ADR 0093 dec. 5, epic #285 T8). Always writes `sourceType: "user_url"`; the field itself is not in the input schema — an agent that sends `sourceType` is refused with the standard `deny_unknown_fields` protocol error, never a silent override (an agent must never mint an `espi_attachment`/other ingest-only row). The fetch runs through a dedicated gated policy (`document_fetcher::HttpDocumentFetcher::agent_capture()`), never the unrestricted fetcher every ingest caller (source refresh, backfill, autopilot, structured extraction, the UI capture command, report-diff fetch-on-demand) keeps using unchanged: **https-only** (any other scheme is a typed refusal), an **SSRF guard** — the host's DNS-resolved addresses must all be public (private `10/8` `172.16/12` `192.168/16`, loopback `127/8`/`::1`, link-local `169.254/16`/`fe80::/10`, and unspecified addresses are rejected), re-checked on **every redirect hop** (reqwest follows redirects by default; a naive guard that only checks the original URL is a no-op against a 302 to an internal address), a **content-type allowlist** (`application/pdf` | `text/html` | `application/xhtml+xml`, parameters tolerated) checked on the response header before any body bytes are read — **except** `application/octet-stream` or a missing header, which defers the accept/refuse decision until after the (capped) body is read, then accepts iff a strict offset-0 sniff of the bytes matches (`%PDF-`, `PK\x03\x04`, or `<?xml`/`<!DOCTYPE html`/`<html` right after an optional BOM/whitespace — never the broader corpus-wide container sniff, which scans a 1 KiB window and accepts any leading `<`); every other content type is still refused immediately (#455, ADR 0093 dec. 5 amendment — the maintainer's corpus proves octet-stream is the normal label for mislabelled ESEF/pdf2htmlEX documents) — and a **30 MiB cap** enforced during streaming (never trusting `Content-Length`, which the server can lie about or omit). Idempotent on `(companyId, url)` — a repeat call returns the existing row, **and never downgrades an already-fetched document**: before any fetch, the row's on-disk identity is checked (file present, size and SHA-256 match the stored `contentHash`/`byteSize`) — a `fetched` row that still matches is returned unchanged, a `failed` row whose file matches is healed back to `fetched`, and a `fetched` row whose file is missing or no longer matches is reset to `pending` and refetched. A fetch refusal is never a protocol error: the document row is still created (`fetchStatus: "failed"`, the error recorded) **unless a concurrent capture already stored the file**, in which case that existing fetched document is returned instead (the failure write is a guarded update that never overwrites a `fetched` row); a retry with a corrected URL reuses the same id. Response: the domain `DocumentCaptureResult { documentId, localPath?, success, error? }` verbatim (no MCP-specific reshaping). Registry: `act`, no provenance carrier (the URL itself is the payload) — `writes_disabled` still applies.
